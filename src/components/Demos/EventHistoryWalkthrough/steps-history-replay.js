...
/**
 * "How History Replay provides Durable Execution" narrative, shared across SDKs.
 *
 * @param {object} sdk - `pizzaWorkflow` config from languages/<sdk>.js
 */
export function makeHistoryReplaySteps({ lines }) {
  return [
    {
      number: 1,
    ...
    },
    {
      ...
      note: 'The Worker invokes the Workflow code and runs it one statement at a time. The first few lines total the price of the order and do not interact with the Temporal Service.',
    },
    {
      number: 5,
    ...
    },
    {
      ...
      note: 'When the Activity returns, the Worker notifies the Temporal Service that the Activity Execution is complete. The Temporal Service records an ActivityTaskCompleted Event, which contains the result of the Activity.',
    },
    {
      number: 9,
