...
export function makeCodeToCommandsSteps({ lines, definitionBullets, notes }) {
  // Some SDKs order the sample so that another internal step follows the
  // distance check, such as totaling the price of the order.
  const extraInternal = lines.extraInternal ?? [];
  return numbered([
    {
    ...
    extraInternal.length > 0 && {
      title: notes.extraInternalTitle,
      kind: 'internal',
      lines: extraInternal,
      note: notes.extraInternal,
    },
    {
      title: 'Start a Timer',
      kind: 'command',
