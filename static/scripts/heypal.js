...
(function (window, document) {
  ...
  palAppId = "5e378f8f-07ad-428f-b7d2-73baae362ca7";
  console.log({
    palAppId,
  });

  if (palWidget == null && isDesktop()) {
    ...
    const openWidget = () => {
      ...
      document.getElementById("pal-ball").style.display = "none";
    ...
    const closeWidget = () => {
      widgetStyle.display = "none";
      modalStyle.display = "none";
      document.getElementById("pal-ball").style.display = "block";
    ...
    // widgetToggleButtonStyle.paddingLeft='16px';

    // widgetToggleButtonStyle.paddingRight='16px';
...
// @keyframes shimmer {
//   0% {
//     background: linear-gradient(135deg, transparent 25%, #A0A4EE 50%, transparent 75%);
...
`;
                  ...
                  <div style="display:flex; flex-direction:row; gap:16px; align-items:center; padding-right:16px">
                    <div id='widget-text' style='display: flex; gap: 8px; align-items:center; flex-grow:1; flex-direction:row; padding-left:16px; opacity: 0; animation: fadeInText 1s forwards 2s;'>
                      ...
                      <div id="ai-container" style="position:relative; display: inline-flex;">
                        ...
                        <span id="ai-text" style="padding:2px 8px; font-weight:600; border-radius:4px; background:#D0D4FE; color: #4B4B4B; display:inline-flex; position: relative; z-index: 1;">AI</span>
                      </div>
                    </div>
                    <div id='ball-chevron-container' style='width:40px; height:40px; position:relative;'>
                      <img id="pal-ball" style="position:absolute; top:0; left:0; flex-shrink:0; opacity: 0; animation: fadeInBall 1s forwards, popBall 1s 1s;" src='${BASE_URL}/pal-ball.png' width='40px' height='40px'/>                    
                      <div id='widget-chevron-down-container' style="opacity:1; position:absolute; top:0; left:0; display:none; transform: rotate(0deg); transition: all 1s ease-in-out; align-items: center; justify-content:center; width:40px; height:40px; border-radius:50%; background:#39278F;">
                        <img id='widget-chevron-down' style='flex-shrink:0;' src='${BASE_URL}/widget-chevron-down.png' width='18px' height='18px'/>
                      </div>
                    </div>
                  </div>`;
    ...
    widgetToggleButton.addEventListener("mouseover", () => {
      ...
      document.getElementById("pal-ball").style.animation =
    ...
    widgetToggleButton.addEventListener("mouseout", () => {
      ...
      document.getElementById("pal-ball").style.animation = "fadeInBall 1s forwards 1s, popBall 1s 1s";
