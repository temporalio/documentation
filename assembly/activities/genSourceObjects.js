...
export async function genSourceObjects(config) {
  ...
  const readPath = path.join(config.rootDir, config.tempWriteDir, config.sourcePathsFileName);
  ...
  const writePath = path.join(config.rootDir, config.tempWriteDir, config.sourceObjectsFileName);
