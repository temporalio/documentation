...
Temporal Nexus is in [Public Preview](/evaluate/development-production-features/release-stages#public-preview) for [Temporal Cloud](/cloud/nexus) and [self-hosted deployments](/production-deployment/self-hosted-guide/nexus).
...
![Flow chart showing the states of the Temporal Nexus Circuit Breaker](/img/nexus/circuit-breaker.png)
...
This means its possible for your Nexus handler to be invoked multiple times for a given Nexus Operation.
...
Its not required in all cases, but highly recommended in general.
