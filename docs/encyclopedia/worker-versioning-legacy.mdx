...
Because the new version processes existing [Event Histories](/workflow-execution#event-history), it must adhere to the usual [deterministic constraints](/workflow-definition#deterministic-constraints), and you might need to use one of the [versioning APIs](/workflows#workflow-versioning).
