...
description: Temporal Task Queues are lightweight, dynamically allocated queues that one or more Worker Entities poll for Tasks. Task Queues are used by Workflow Executions and Activity Executions to schedule tasks and enable load balancing across multiple Worker Processes.
slug: /workers
