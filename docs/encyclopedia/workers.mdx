...
Workers execute your Workflow code. The Worker itself is provided by the Temporal SDK, but your application will include code to configure and run it. In general, a Worker is an entity that polls a specific Task Queue on the Temporal Service, executes Workflow code in response, and returns the results of the execution back to the Temporal Server. 
