...
A Task is the context that a Worker needs to progress with a specific [Workflow Execution](/workflows#workflow-execution), [Activity Execution](/activities#activity-execution), or a [Nexus Operation Task Execution](#nexus-operation-task-execution).

There are three types of Tasks:

- [Activity Task](#activity-task)
- [Workflow Task](#workflow-task)
- [Nexus Operation Task](#nexus-operation-task)
...
## What is a Nexus Operation Task? {#nexus-operation-task}

An Nexus Operation Task contains the context needed to proceed with an Nexus Operation Task Execution.
Nexus Operation Tasks largely represent the Nexus Operation Scheduled Event, which contains the data needed to execute an Nexus Operation handler.

## What is a Nexus Operation Task Execution? {#nexus-operation-task-execution}

An Nexus Operation Task Execution occurs when a Worker uses the context provided from the Nexus Operation Task and executes the Nexus Operation handler (also known as the Nexus Operation handler function).

The NexusOperationScheduled Event corresponds to when the Temporal Service puts the Nexus Operation Task into the Task Queue.

The NexusOperationStarted Event corresponds to when the Worker picks up the Nexus Operation Task from the Task Queue.

Either NexusOperationCompleted or one of the other Closed Nexus Operation Events corresponds to when the Worker has yielded back to the Temporal Service.

The API to schedule a Nexus Operation Execution provides an "effectively once" experience, even though there may be several Nexus Operation Task Executions that take place to successfully complete an Nexus Operation.

Once an Nexus Operation Task finishes execution, the Worker responds to the Temporal Service with a specific Event:
...
If you're using the older version of the Worker Versioning pre-release that was released in late 2023, go [here](docs/encyclopedia/worker-versioning-legacy.mdx)
