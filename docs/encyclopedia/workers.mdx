...
A Nexus Task provides the context needed to proceed with an Nexus Task Execution.
...
The API to schedule a Nexus Operation Execution may provide an "effectively once" experience, depending on the implementation of the Nexus Operation handler (which should be idempotent), even though there may be several Nexus Task Executions that take place to successfully complete an Nexus Operation, for example in the case of retries.

Once an Nexus Task finishes execution, the Worker responds to the Temporal Service with a specific Event:
