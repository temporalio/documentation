...
A Nexus Task provides the context needed to proceed with a Nexus Task Execution.
Nexus Tasks contain the data needed to execute various actions related to a Nexus Operation.
...
The `NexusOperationScheduled` Event corresponds to when the Temporal Service puts the Nexus Task to start a Nexus Operation into the Task Queue.
...
A Nexus Operation Execution appears to the caller Workflow as a single RPC, while under the hood the Temporal Service may issue several Nexus Tasks to attempt to start the Operation. Hence, a Nexus Operation Handler implementation should be idempotent. 
...
A Nexus Task Execution completes when a Worker responds to the Temporal Service with either a RespondNexusTaskCompleted or RespondNexusTaskFailed call, or when the task times out. Temporal Service interprets the outcome and determines whether to retry the task or record the progress in a History Event:
