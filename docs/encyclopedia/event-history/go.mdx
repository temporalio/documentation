...
Now that Replay has been covered, this section will explain why Workflow code needs to be
[deterministic](/workflow-definition#deterministic-constraints) in order for Replay to work.

A Workflow's code is deterministic if every execution of its Workflow Definition produces the same Commands in the same
...
To better understand why Workflow code needs to be deterministic, it's helpful to look at a Workflow Definition that violates
it. In this case, this code will walk through a Workflow Definition that breaks the determinism constraint with a random
number generator.

<NonDeterminismDemo sdk="go" />
...
We want to emphasize that although your Workflow code needs to be deterministic, your application itself does not!
...
**Good news**: Your Temporal application can absolutely handle all of these operations. While your Workflow code must be
