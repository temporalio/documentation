...
description: Operations you can perform on an Activity - Pause, Unpause, and Reset.
...
- **Pause doesn't support bulk operations.** Unlike Unpause and Reset, there's no `--query` flag. You can only Pause Activities within a single Workflow, by Activity Id or by Activity type.
