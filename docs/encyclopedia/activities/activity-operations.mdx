...
| [Update Options](#update-options) | Change Activity options (e.g. timeouts, Retry Policy, or Task Queue) without restarting the Activity.  | [`temporal activity update-options`](/cli/command-reference/activity#update-options) |
...
- **CLI and gRPC only.** This operation isn't available in the UI.
...
A Standalone Activity's stored runtime options can be changed.

- The following fields can be changed. The value in parentheses is the `update_mask` entry to provide
  in a CLI or gRPC request:
  - Task Queue name (`task_queue.name`).
  - Schedule-To-Close Timeout (`schedule_to_close_timeout`).
  - Schedule-To-Start Timeout (`schedule_to_start_timeout`).
  - Start-To-Close Timeout (`start_to_close_timeout`).
  - Heartbeat Timeout (`heartbeat_timeout`).
  - Retry Policy (`retry_policy`).
  - Retry Policy initial interval (`retry_policy.initial_interval`).
  - Retry Policy backoff coefficient (`retry_policy.backoff_coefficient`).
  - Retry Policy maximum interval (`retry_policy.maximum_interval`).
  - Retry Policy maximum attempts (`retry_policy.maximum_attempts`).
  - Priority (`priority`).
  - Priority Key (`priority.priority_key`).
  - Fairness Key (`priority.fairness_key`).
  - Fairness Weight (`priority.fairness_weight`).
  - Start Delay while the Activity is still in its delay window (`start_delay`).
- An update mask is required unless `restore_original` is used.
...
- If Schedule-To-Close Timeout is updated:
  - The Schedule-To-Close timeout task is reissued at the new deadline.
...
- If Start-To-Close Timeout or Heartbeat Timeout is updated for a running attempt:
  - The Start-To-Close timeout task is reissued for that attempt.
  - The Heartbeat timeout task is reissued for that attempt.
...
- If Retry Policy is updated:
  ...
  - If the Activity is already waiting before its next retry because a previous attempt failed or timed out and Temporal
    used the Retry Policy to calculate the next dispatch time, the retry deadline is recalculated from the updated
    policy. For example, if the next attempt was waiting 30 minutes because of the previous policy and you lower
    `retry_policy.maximum_interval` to 5 minutes, the next attempt can become eligible at the recalculated 5-minute
    point.
  - A Worker-provided `NextRetryDelay` that is already scheduled isn't recalculated.
- If an unrelated field is updated, the current retry interval isn't recalculated.
- If `restore_original` is set and the Activity is SCHEDULED or PAUSED:
  - The same retry-interval recalculation can apply when the current wait was derived from the Retry Policy.
- If the Activity is SCHEDULED:
  - Dispatch and Schedule-To-Start timeout tasks are reissued.
  - If the Activity is in retry backoff:
...
- If the Activity is PAUSED:
  - The new options are stored, but the Activity isn't dispatched.
  - Updated Retry Policy values can affect when the Activity dispatches after Unpause.
- If the Activity is PAUSE_REQUESTED:
  - Running-attempt timers are reissued so Start-To-Close and Heartbeat timeouts continue to be enforced.
- If the Activity is RESET_REQUESTED:
  - Running-attempt timers are reissued so Start-To-Close and Heartbeat timeouts continue to be enforced.
- If cancellation is pending:
  - The update is still allowed.
  - Updated Start-To-Close and Heartbeat timers can still affect the running attempt.
- If Start Delay is updated:
  - The first dispatch time is recalculated from the Activity's schedule time and the updated Start Delay. Shortening
    Start Delay can make the first dispatch eligible sooner; extending it can defer the first dispatch.
  - The update is allowed only while the Activity is SCHEDULED and the current first dispatch time is still in the future.
  - After the first dispatch time, Start Delay can no longer be updated, even if no Worker has picked up the first
    attempt yet.
...
- If only Start-To-Close Timeout is set, Schedule-To-Close and missing Schedule-To-Start values are set from the run
  timeout when one is set.
- Timeouts can't exceed the run timeout when one is set. Heartbeat Timeout can't exceed Start-To-Close Timeout.
...
The `TemporalPauseInfo` [Search Attribute](/search-attribute) is filterable within a Workflow. `TemporalPauseInfo` is only added
to Workflows, it does not exist for Standalone Activities.
