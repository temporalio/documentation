...
description: Operations you can perform on an Activity - Pause and Unpause.
...
Pause and Unpause are available in Server v1.28.0+. Self-hosted UI requires v2.47.0+.
...
- **A Paused Activity can still time out.** Pause doesn't stop or extend the [Schedule-To-Close Timeout](/encyclopedia/detecting-activity-failures#schedule-to-close-timeout). Use `temporal activity update-options` to adjust the timeout if needed.
...
- **Pause doesn't support bulk operations.** Unlike Unpause, there's no `--query` flag. You can only Pause Activities within a single Workflow, by Activity Id or by Activity type.
...
- **Attempt count and Heartbeat data are preserved by default.** The Activity resumes from where it left off. Use `--reset-attempts` or `--reset-heartbeats` on the CLI to clear these.
...
- **Unpausing doesn't reset the attempt count.** The Activity retries from its current attempt number.
- **A Paused Activity can time out before you Unpause it.** The [Schedule-To-Close Timeout](/encyclopedia/detecting-activity-failures#schedule-to-close-timeout) isn't stopped or extended while Paused. Use `temporal activity update-options` to extend the timeout before Unpausing if needed.
...
There is no record of a Pause or option change in the Workflow's [Event History](/workflow-execution/event#event-history).
...
There's no persistent record that an Activity was Paused or had its options changed.
