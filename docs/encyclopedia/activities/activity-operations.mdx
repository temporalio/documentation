...
API. Activity Operations don't apply to [Local Activities](/local-activity).

Pause, Unpause, Reset, and Update Options are also available, experimentally, for
[Standalone Activities](/standalone-activity) as of Temporal CLI v1.9.0. Omit `--workflow-id` on the CLI to target a
Standalone Activity by Activity ID (and optional Run ID) instead of a Workflow Activity. The rest of this page
describes these operations for Activities running inside a Workflow.
...
- **Pause operates on one Activity at a time, by ID.** Unlike Unpause, Reset, and Update Options, there's no `--query`
  flag. To pause multiple Activities, issue separate commands for each Activity ID.
...
- **The Activity is rescheduled immediately.** Any remaining retry backoff is discarded. The next execution starts right
  away.
- **Attempt count and Heartbeat data are preserved.** The Activity resumes from where it left off. Use [Reset](#reset)
  if you need to restart from attempt 1 or clear Heartbeat details.
...
- **If the Activity is Paused, Reset also Unpauses it.** Use `--keep-paused` to Reset the attempt count without resuming
  execution. With `--keep-paused`, the attempt count and Heartbeat data (if `--clear-heartbeat-details`) are reset, but
  the Activity stays Paused. No retry is scheduled until you [Unpause](#unpause) separately.
...
`--clear-heartbeat-details` and bulk mode via `--query`.
...
- **Heartbeat details are preserved by default.** If your Activity uses Heartbeat details for progress tracking and you
  want a clean restart, pass `--clear-heartbeat-details`.
