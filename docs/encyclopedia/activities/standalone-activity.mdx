...
Standalone Activities are Temporal's [job queue](/evaluate/features/job-queue) - the
...
[Job Queue](/evaluate/features/job-queue) and
...
On Temporal Cloud, enabling Fairness carries a [per-Action surcharge](/evaluate/cloud/actions#fairness).
...
[Public Preview](/evaluate/product-release-stages#public-preview). Request Cancel, Terminate,
...
AWS Lambda support is in [Public Preview](/evaluate/product-release-stages#public-preview) and
GCP Cloud Run is in [Pre-release](/evaluate/product-release-stages#pre-release). See
...
Running a single Activity as a Standalone Activity also costs fewer [Billable Actions](/evaluate/cloud/actions-usage#actions-in-workflows) in
...
- Starting from [Temporal Nexus](/evaluate/features/nexus).
...
Standalone Activities require [Temporal CLI](https://github.com/temporalio/cli/releases) v1.9.1 or higher and [Temporal Server](https://github.com/temporalio/temporal/releases) v1.32.0 or higher.
...
Which should output v1.9.1 or higher, for example:
```
temporal version 1.9.1 (Server 1.32.0, UI 2.53.3)
```
...
Standalone Activities are Generally Available in Temporal Cloud, in all [regions](/evaluate/cloud/regions).

Service Level Objectives and the Service Level Agreement match those for Workflows. See
[Service availability](/evaluate/cloud/service-availability) and [SLA](/evaluate/cloud/sla).
