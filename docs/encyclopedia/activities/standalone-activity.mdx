...
`execute`, `result`, `list`, `count`, `describe`, `cancel`, `terminate`, and `delete`.

[Temporal CLI](https://github.com/temporalio/cli) v1.9.0 or higher adds experimental support for `pause`,
`unpause`, `reset`, and `update-options` on Standalone Activities. See [Activity Operations](/activity-operations)
for what each operation does.
