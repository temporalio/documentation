...
For [synchronous Nexus Operations](/nexus/operations#synchronous-operation-lifecycle) the Nexus Machinery creates a single Nexus Task to start an Operation.
...
To avoid infinite retries and improve semantics, Nexus handlers should return a [specific Nexus error type](/references/failures#nexus-errors).
...
All retryable errors are automatically retried by the Nexus Machinery, as discussed in [Automatic Retries](/nexus/operations#automatic-retries).
...
See [Nexus Operation Failures](/references/failures#nexus-operation-failure) for additional details.
