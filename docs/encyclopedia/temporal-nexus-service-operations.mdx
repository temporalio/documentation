...
Calling a Nexus Operation from a caller Workflow presents as a non-blocking, arbitrary duration operation. This schedules the Operation with the caller’s Temporal Service and does not block caller Workflow execution. 
...
An asynchronous Nexus Operation may take up to 2 months to complete in Temporal Cloud, to allow for callback URL token validation with asymmetric key rotation at those intervals.
...
3. Nexus `Operation handler` in the target cluster processes the Nexus Operation.
   ...
   5. Operation ID is associated with the caller's `NexusOperationFuture` and the `GetNexusOperationExecution` future is unblocked.
...
A single attempt to start a synchronous Nexus Operation may take up to 10 seconds to complete and will then timeout.
...
2. Start Nexus operation on the target cluster is attempted by the caller’s Temporal cluster.
   ...
   2. Retries are processed by the Nexus Machinery in the caller’s Temporal cluster and tracked as part of the caller Workflow, similar to Activities.
3. Nexus Operation handler in the target cluster processes the Nexus `Operation`.
   1. Operation handler pulls operation Nexus Task from Task Queue.
...
Nexus Operation handlers can execute arbitrary code, but unlike Activities they should be relatively short-lived, due to the lack of [heartbeats](https://community.temporal.io/t/best-practices-for-long-running-Activities/934) and a default 10 second NexusOperationStarted timeout. 
Nexus Operation handlers may execute arbitrary code or use a Temporal Client to interact with Temporal APIs.
...
Since the caller's Nexus Machinery will keep trying to start the Operation over and over, the Nexus `Operation` handler should be idempotent just like Activities should be idempotent. 
