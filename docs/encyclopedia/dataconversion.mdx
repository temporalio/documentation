...
<div class="tdiw">
  <div class="tditw">
    <p class="tdit">Data Converter encodes and decodes data</p>
  </div>
  <div class="tdiiw" height="1240">
    <img class="img_ev3q" src="/diagrams/default-data-converter.svg" alt="Data Converter encodes and decodes data" />
  </div>
</div>
...
You can perform some operations on your Workflow Execution using Temporal CLI and the Web UI, such as starting or sending a Signal to an active Workflow Execution using Temporal CLI or canceling a Workflow Execution from the Web UI, which might require inputs that contain sensitive data.

To encode this data, [specify your Codec Server endpoints](#codec-server) with [Temporal CLI](/cli/cmd-options#codec-endpoint) and configure your Web UI to use the Codec Server endpoints.
...
To decode output when using the Web UI and Temporal CLI, use a [Codec Server](#codec-server).
