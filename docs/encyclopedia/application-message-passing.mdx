...
- **Lazy Initialization** -
  ...
  Update-With-Start ensures the cart is created first (the Workflow Execution) before applying the Update (state change of items within the cart).
...
If the Update cannot be delievered, for example, when no Worker is running, the Workflow Execution is still started.
