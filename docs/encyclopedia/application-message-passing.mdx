...
Sending an Update-with-Start, if there is a running Workflow Execution with the given Workflow Id, it will handle the Update. Otherwise, a new Workflow Execution starts and immediately handles the Update.

<RelatedReadContainer>
    <RelatedReadItem path="/develop/go/message-passing#update-with-start" text="Update-With-Start using the Go SDK" archetype="feature-guide" />
</RelatedReadContainer>
