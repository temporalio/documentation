...
- When clients need to [Signal-With-Start](#signal-with-start) atomically (since there is currently no Update-with-Start operation.)
...
- The clients care about the latency of the end-to-end operation and are willing to wait for it to finish or be validated.
...
You can also Signal-With-Start or send a Signal from one Workflow to another.
...
Many developers want their message handlers to run exactly once--to be idempotent--in cases where the same Signal or Update is delivered twice or sent by two different call sites. To do this, you can use an idempotency key.

For Signals, you can pass a unique key among your message arguments and use that to check for duplicate messages in your handler.

For Updates, you can additionally retrieve the Update ID within your handler and use it to check whether the Workflow has already handled the Update. Clients randomize this Update ID by default, which means you'll be deduplicating retries from the same call. If you'd like multiple calls to share an idempotency key, such as when your client itself retries, pass in an Update ID of your own choosing.

:::note

Temporal deduplicates messages for you on the server. However, this doesn't work across [Continue-As-New](/workflows#continue-as-new) boundaries, meaning you would risk processing messages twice for such Workflows if you don't do this yourself.

:::

See the links below for examples in your SDK.
