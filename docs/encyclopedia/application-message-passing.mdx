...
- Allow a failing Activity to exhaust its retries, so that it throws an [Activity Failure](https://docs.temporal.io/references/failures#activity-failure). This will only happen if you change the default Activity [Retry Policy](https://docs.temporal.io/encyclopedia/retry-policies).
