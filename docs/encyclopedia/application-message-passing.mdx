...
You may want your message handlers to process each message exactly once, even if the same Signal or Update is sent multiple times.

Temporal provides server-side deduplication that prevents duplicate processing when:
- A client retries the same message request (for example, due to a network timeout)
- The same Update ID is used from the same call site

However, you need to implement your own deduplication in two scenarios:

1. Workflows that Continue-As-New: Temporal's server-side deduplication doesn't work across [Continue-As-New](/workflows#continue-as-new) boundaries. Without manual deduplication, your Workflow might process the same message twice.

2. Messages sent from different locations: By default, Temporal SDKs generate random Update IDs. This means Temporal only prevents duplicate processing for retries from the same call. If two different parts of your code send the same Update, or if you want to ensure idempotency across multiple client instances, you need to provide your own deduplication.

To implement deduplication, use an idempotency key to track which messages have been processed:

- For Updates: Provide a custom Update ID when sending the Update. Temporal will deduplicate any Updates with the same ID. In your handler, check if this Update ID has already been processed.

- For Signals: Include an idempotency key as part of your Signal arguments. In your Signal handler, check if this key has already been processed before executing the handler logic.

In both cases, maintain a set or list of processed idempotency keys in your Workflow state. Before processing a message, check if its key exists in this collection.
...
- Reject the Update by throwing any exception from your [Validator](https://docs.temporal.io/encyclopedia/workflow-message-passing#update-validators).
