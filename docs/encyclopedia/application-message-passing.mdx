...
In this section, we’ll give you an overview of how messages work with Temporal and cover how to write correct and robust handlers by covering topics like atomicity, guaranteeing completion before the Workflow exits, failures, and idempotency.
...
Here are several common patterns for Signal and Update handlers (but not Queries or [Update Validators](#update-validators)):
...
Synchronous handlers don’t block or kick off any long-running operations. They are guaranteed to run atomically.
...
Sometimes, we need our message handlers to do long-running operations such as executing an Activity. When this happens, the handler will yield control back to [the loop](#message-handler-concurrency). This means that your handlers can have race conditions if you’re not careful.
...
If you don’t need to ensure that your handlers complete, you may specify your handler’s Handler Unfinished Policy as Abandon to avoid needing to do this. However, note that clients waiting for Updates will get Not Found errors if they are waiting for Updates that never complete before the Workflow run completes.

See the links above for how to ensure handlers are finished in your SDK.
...
<RelatedReadContainer>
    ...
    <RelatedReadItem path="/develop/typescript/message-passing#handler-patterns" text="Author message handler patterns in Python" archetype="feature-guide" />
...
When you define an Update handler, you may optionally define a validator. That validator is responsible for accepting or rejecting the Update. You can use validators to verify arguments or make sure the Workflow is ready to accept your Updates.
...
- If it rejects or throws an exception, the client will be informed that it was Rejected, and the Workflow will have no indication that it was ever requested, similar to a Query handler.

:::note

Validators are not allowed to block.
...
<RelatedReadContainer>
    ...
    <RelatedReadItem path="/develop/typescript/message-passing#validate-update" text="Validate updates in Python" archetype="feature-guide" />
...
If you throw any other exception, by default, it will cause a [Workflow Task Failure](references/failures#workflow-task-failures). This means the Workflow will get stuck and will retry the handler periodically until the exception is fixed, for example by a code change or infrastructure coming back online.

#### Exceptions in Updates

When you want to fail the Update and for the client to receive that error:

- Throw any exception from your validator, OR
...
In Go, most exceptions behave like Application Failures in the other SDKs. Panics behave like other exceptions and fail the Signal or Update handler tasks.

#### Idempotency

Most developers want their message handlers to run exactly once in cases where a Signal is delivered twice or sent by two different call sites. You can pass a unique key among your message arguments and use that to check for duplicate messages in your handler.
For Updates, clients can instead pass the ID argument into the Update to serve as an idempotency key and as logging context. This can be retrieved within your handler and used to check whether the Update has already been handled.
...
<RelatedReadContainer>
    ...
    <RelatedReadItem path="/develop/typescript/message-passing#handle-signal" text="Handle Signals in Python" archetype="feature-guide" />
...
<RelatedReadContainer>
    ...
    <RelatedReadItem path="/develop/typescript/message-passing#handle-update" text="Handle Updates in Python" archetype="feature-guide" />
...
Author queries using these language guides.
...
<RelatedReadContainer>
    ...
    <RelatedReadItem path="/develop/typescript/message-passing#handle-queries" text="Handle Queries in Python" archetype="feature-guide" />
