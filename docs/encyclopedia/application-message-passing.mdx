...
To enable sending Updates, set the [frontend.enableUpdateWorkflowExecution](https://github.com/temporalio/temporal/blob/main/common/dynamicconfig/constants.go) and [frontend.enableUpdateWorkflowExecutionAsyncExcepted](https://github.com/temporalio/temporal/blob/main/common/dynamicconfig/constants.go) dynamic config values to `true`.
...
Here are several common patterns for write operations, Signal and Update handlers. They don't apply to read operations, i.e. Queries or [Update Validators](#update-validators)):
...
#### Exceptions in message handlers in the Go SDK

In Go, most exceptions behave like [Application Failures](/references/failures#application-failure) in the other SDKs. Panics behave like other exceptions and fail the Signal or Update handler tasks.
