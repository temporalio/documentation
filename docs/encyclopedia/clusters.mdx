...
sidebar_position: 8
description: This guide provides a comprehensive overview of Temporal Services.
...
This guide provides a comprehensive overview of Temporal Services.
...
- [Temporal Service deployment guide](/self-hosted-guide)
...
<div class="tdiw">
  <div class="tditw">
    <p class="tdit">Frontend Service</p>
  </div>
  <div class="tdiiw" height="1040">
    <img class="img_ev3q" src="/diagrams/temporal-frontend-service.svg" alt="Frontend Service" />
  </div>
</div>
...
- Calls from a remote related to [Multi-Cluster Replication](#multi-cluster-replication)
...
Although the total number of History Shards remains static for the life of the Temporal Service, the number of History Service processess can change.
...
Temporal Services have operated successfully using anywhere from 1 to 128K History Shards, with each Shard responsible for tens of thousands of Workflow Executions.
...
<div class="tdiw">
  <div class="tditw">
    <p class="tdit">Matching Service</p>
  </div>
  <div class="tdiiw" height="980">
    <img class="img_ev3q" src="/diagrams/temporal-matching-service.svg" alt="Matching Service" />
  </div>
</div>
...
<div class="tdiw">
  <div class="tditw">
    <p class="tdit">Worker Service</p>
  </div>
  <div class="tdiiw" height="740">
    <img class="img_ev3q" src="/diagrams/temporal-worker-service.svg" alt="Worker Service" />
  </div>
</div>
...
The Temporal Persistence store is a database used by [Temporal Services](#temporal-server) to persist events generated and processed in your Temporal Service and SDK.
...
<div class="tdiw">
  <div class="tditw">
    <p class="tdit">Persistence</p>
  </div>
  <div class="tdiiw" height="620">
    <img class="img_ev3q" src="/diagrams/temporal-database.svg" alt="Persistence" />
  </div>
</div>
...
- In Temporal Server v1.21 and later, standard Visibility is no longer in development, and we recommend migrating to a [database that supports Advanced Visibility features](/self-hosted-guide/visibility). The Visibility configuration for Temporal Services has been updated and Dual Visibility is enabled. For details, see [Visibility store setup](/self-hosted-guide/visibility).
...
Temporal's Archival feature is considered **experimental** and not subject to normal [versioning and support policy](/temporal-service#versions-and-support).
...
## What is Temporal Service configuration? {#temporal-service-configuration}
...
On self-hosted Temporal Services, configure mTLS in the `tls` section of the [Temporal Service configuration](/references/configuration#tls).
...
On self-hosted Temporal Services, you can restrict access to Temporal Service endpoints by using the `clientCAFiles` or `clientCAData` property and the [`requireClientAuth`](/references/configuration#tls) property in your Temporal Service configuration.
...
On self-hosted Temporal Services, you can specify `serverName` in the `client` section of your mTLS configuration to prevent spoofing and [MITM attacks](https://en.wikipedia.org/wiki/Man-in-the-middle_attack).
...
- [Set up Grafana with Temporal Cloud observability to view metrics](/cloud/metrics/prometheus-grafana#data-sources-configuration-for-temporal-cloud-and-sdk-metrics-in-grafana)

On self-hosted Temporal Services, expose Prometheus endpoints in your Temporal Service configuration and configure Prometheus to scrape metrics from the endpoints.
...
- [Temporal Service OSS metrics reference](/references/temporal-service-metrics)
...
When necessary, for higher availability, Temporal Service operators can failover to any of the backup Clusters.

Temporal's Multi-Cluster Replication feature is considered **experimental** and not subject to normal [versioning and support policy](/temporal-service).

Temporal automatically forwards Start, Signal, and Query requests to the active Temporal Service.
...
However, they might see a lag due to replication delay when querying the Workflow Execution state from a standby Temporal Service.
...
This means that data across temporal-service is **not** strongly consistent.
...
When performing failover for a Namespace from one Temporal Service to another Temporal Service, the version attached to the Namespace will be changed by the following rule:
...
When a Temporal Service is trying to mutate a Workflow Execution History, the version will be checked.
A Temporal Service can mutate a Workflow Execution History only if the following is true:
...
- Temporal Service A comes with initial version: 1
- Temporal Service B comes with initial version: 2
- Shared version increment: 10

T = 0: Namespace α is registered, with active Temporal Service set to Temporal Service A
...
T = 1: namespace β is registered, with active Temporal Service set to Temporal Service B
...
T = 2: Namespace α is updated to with active Temporal Service set to Temporal Service B
...
T = 3: Namespace β is updated to with active Temporal Service set to Temporal Service A
...
View in both Temporal Service A & B
...
Since Temporal is AP, during failover (change of active Temporal Service Namespace), there can exist cases where more than one Temporal Service can modify a Workflow Execution, causing divergence of Workflow Execution History. Below shows how the version history will look like under such conditions.
...
- Temporal Service A comes with initial version: 1
- Temporal Service B comes with initial version: 2
- Temporal Service C comes with initial version: 3
- Shared version increment: 10

T = 0:

View in both Temporal Service B & C
...
T = 1: adding event with event ID == 4 & version == 2 in Temporal Service B
...
T = 1: namespace failover to Temporal Service C, adding event with event ID == 4 & version == 3 in Temporal Service C
...
T = 2: replication task from Temporal Service C arrives in Temporal Service B
...
T = 2: replication task from Temporal Service B arrives in Temporal Service C, same as above
...
Temporal Multi-Cluster Replication relies on asynchronous replication of Events across Clusters, so in the case of a failover it is possible to have an Activity Task dispatched again to the newly active Temporal Service due to a replication task lag.
This also means that whenever a Workflow Execution is updated after a failover by the new Temporal Service, any previous replication tasks for that Execution cannot be applied.
This results in loss of some progress made by the Workflow Execution in the previous active Temporal Service.
...
Your application should have retry logic in place so that the Activity gets retried and dispatched again to a Worker after the failover to the new Temporal Service.
...
Due to the nature of Multi-cluster Replication (for example, Workflow Execution History events are replicated asynchronously) different Runs (same Namespace and Workflow Id) can arrive at the target Temporal Service at different times, sometimes out of order, as shown below:

```
| ------------- |          | ------------- |          | ------------- |
| Temporal Service A |  | Network Layer |  | Temporal Service B |
| --------- || ------------- |          | ------------- |
        |                          |                          |
...
Because Run 2 appears in Temporal Service B first, Run 1 cannot be replicated as "runnable" due to the rule `at most one Run open` (see above), thus the "zombie" Workflow Execution state is introduced.
A "zombie" state is one in which a Workflow Execution which cannot be actively mutated by a Temporal Service (assuming the corresponding Namespace is active in this Temporal Service). A zombie Workflow Execution can only be changed by a replication Task.
