...
increases exponentially, until it either succeeds or is canceled. When a subsequent attempt succeeds, execution
continues normally: a Workflow that called the Activity resumes as if the failure never occurred, and for a
[Standalone Activity](/standalone-activity), the Activity Execution completes and returns its result to the Client.
...
These defaults are the same whether the Activity is a Workflow Activity or a [Standalone Activity](/standalone-activity)
— a Retry Policy is a property of the Activity Task Execution itself, not of what started it.
