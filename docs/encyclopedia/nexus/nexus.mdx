...
- [Temporal Operation Handler](/nexus/temporal-operation-handler) - Implement Operations with a single handler type, and get bidirectional linking across the Namespace boundary.
- [Nexus Client Code Generator](/nexus/client-code-generator) - Generate typed models, validators, and Service definitions for Go, Java, Python, and TypeScript from one schema.
- [Nexus Standalone Activity](/nexus/standalone-activity) - Back an Operation with a single durable step instead of a Workflow.
- [Microservice Development Walkthrough](/develop/java/nexus/development-walkthrough) - Build a Nexus Service end to end, adding one capability at a time. This walkthrough is not in the navigation yet, so this link is its entry point.
