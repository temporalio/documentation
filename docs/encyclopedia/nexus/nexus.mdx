...
A new way of building Nexus Services is in pre-release. It combines a contract-first workflow, where one schema generates typed models and Service definitions for every language, with a single handler type that can back an Operation with a Workflow, an Update, or an Activity.
