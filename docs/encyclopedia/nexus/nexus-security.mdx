...
The wrapper approach changes the static type of the operation input, so it only compiles when the caller uses `ExecuteOperation` with a **string operation name**.
A typed reference declared as `nexus.Operation[Input, Output]` constrains the input through Go generics, and the wrapped value will not satisfy that constraint.
The call fails at compile time, not at runtime.
