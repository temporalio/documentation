...
```csharp
TemporalOperationHandler.FromHandleFactory<UpdateAddressInput, AddressOutput>(
    async (context, client, input) =>
        await client.StartWorkflowUpdateAsync<OrderWorkflow, AddressOutput>(
            $"order-{input.OrderId}",
...
Call `startActivity` when the work is a single durable step. The Activity runs with no parent Workflow, so the options require an Activity Id and at least one timeout. Java also requires a Task Queue; the other SDKs default it to the Task Queue the Operation is running on. See [Nexus Standalone Activity](/nexus/standalone-activity).
