...
Workflows can be thought of as stateful web services that can receive messages. The Workflow can have message handlers, akin to endpoints, that react to incoming messages together with the Workflow's current state.
...
| **Concurrency and throughput** | You don’t want to limit the number of messages processed concurrently by a single Workflow. | You don’t need more concurrent Updates per Workflow than the allowed limits for [Cloud](/evaluate/cloud/limits#per-workflow-execution-update-limits) or [Self-Hosted](/self-hosted-guide/defaults). |
