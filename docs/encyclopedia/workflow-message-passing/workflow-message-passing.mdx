...
- Queries are efficient–they never add entries to the [Workflow Event History](/workflow-execution#event-history), whereas an Update would (if accepted).
...
- You could write your read operation as an Update, which will give you better efficiency and latency, though it will write an entry to the [Workflow Event History](/workflow-execution#event-history).
