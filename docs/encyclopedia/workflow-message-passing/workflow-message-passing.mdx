...
- You don't want to limit the number of messages that can be processed concurrently by a single Workflow.
...
- You don't require more concurrent Updates per Workflow than the permitted limits for [Cloud](/cloud/limits#per-workflow-execution-update-limits) or [Self-Hosted](/self-hosted-guide/defaults).
