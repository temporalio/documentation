...
- If no other Namespace is specified, the Temporal Cluster uses the Namespace "default" for all Temporal SDKs and temporal CLI.
...
- **Namespace Configuration:** Various configuration options like the [Retention Period](/clusters#retention-period) and the [Archival](/clusters#archival) destination are configured per Namespace through a special CRUD API or through [`temporal`](/cli/cmd-options#namespace).
...
On self-hosted Temporal Cluster, you can register your Namespaces using temporal CLI (recommended) or programmatically using APIs. Note that these APIs and temporal CLI commands will not work with Temporal Cloud.
...
- In your Cluster setup, create your Namespaces, including the default, in your setup script.
  ...
  - If deploying through the [Temporal Helm charts](https://github.com/temporalio/helm-charts), you can create the "default" Namespace by using temporal CLI; for example, `temporal operator namespace create --namespace default`
...
- Use the `temporal operator namespace create` or `temporal operator namespace update` command with the `--retention` modfiier to register your Namespaces, one at a time, and set the Retention Period on each.
  ...
  - [How to create a new Namespace using temporal](/cli/operator#create)
  - [How to create a new Namespace using the Go SDK](/dev-guide/go/features#register-namespace)
  - [How to create a new Namespace using the Java SDK](/dev-guide/java/features#register-namespace)
...
On self-hosted Temporal Cluster, you can manage your registered Namespaces using temporal CLI (recommended) or programmatically using APIs.
<!--Technically correct but is this confusing for people since you can do this for non-self-hosted:-->
...
- Update information and configuration for a registered Namespace on your Temporal Cluster:
  ...
  - With temporal CLI: [`temporal operator namespace update`](/cli/operator#update)
...
- Get details for a registered Namespace on your Temporal Cluster:
  ...
  - With temporal CLI: [`temporal operator namespace describe`](/cli/operator#describe)
...
- Get details for all registered Namespaces on your Temporal Cluster:
  ...
  - With temporal CLI: [`temporal operator namespace list`](/cli/operator#list)
...
- With temporal CLI: [`temporal operator namespace delete`](/cli/operator#delete).
...
- [How to list Namespaces in a Cluster using temporal CLI](/cli/operator#list)
- [How to view (describe) Namespace metadata and details using temporal CLI](/cli/operator#describe)
...
- [How to register a Global Namespace](/cli/operator#register)
