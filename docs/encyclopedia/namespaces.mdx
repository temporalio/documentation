...
## What is a Multi-region Namespace? {#multi-region-namespace}

A Multi-region Namespace is a [Namespace](/namespaces) that exists across Clusters when [Multi-Cluster Replication](/clusters#multi-cluster-replication) is set up.

- [How to register a Multi-region Namespace](/tctl-v1/namespace#register)
- [How to change the active Cluster for a Multi-region Namespace](/tctl-v1/namespace#update)

The Multi-region Namespace feature enables Workflow Executions to progress through another Cluster in the event of a failover.

A Multi-region Namespace may be replicated to any number of Clusters, but is active in only one Cluster at any given time.

For a failover to be successful, Worker Processes must be polling for Tasks for the Multi-region Namespace on all Clusters.

A Multi-region Namespace has a failover version.
...
Unlike regular Namespaces, which provide at-most-once semantics for an Activity Execution, Multi-region Namespaces can support only at-least-once semantics (see [Conflict resolution](/clusters#conflict-resolution)).
