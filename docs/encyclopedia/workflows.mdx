...
- [How to send a Signal using Temporal CLI](/cli/workflow#signal)
...
- [How to send a Query using Temporal CLI](/cli/workflow#query)
...
- [How to operate Schedules using Temporal CLI](/cli/schedule)
...
In Temporal CLI, for example, an interval is specified as a string like `45m` to mean every 45 minutes, or `6h/5h` to mean every 6 hours but at the start of the fifth hour within each period.

In Temporal CLI, a calendar expression can be specified as either a traditional cron string with five (or six or seven) positional fields, or as JSON with named fields:
...
(This feature isn't currently exposed in Temporal CLI or the Temporal Web UI.)
...
(Note: this isn't currently exposed in Temporal CLI or the web UI.)
