...
If you need to start a Workflow for a particular implementation only if it hasn't started yet, pay close attention to your Retention Period. If this becomes unwieldy, consider using [Workflow message passing](/encyclopedia/workflow-message-passing) instead of trying to start Workflows atomically.
