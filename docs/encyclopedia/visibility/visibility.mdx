...
The `GROUP BY` clause is only supported in the Count API. We only support grouping by `ExecutionStatus`. We plan to support group by any search attribute in a future release.
