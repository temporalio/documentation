...
Visibility is a search index that Temporal updates asynchronously as executions change. Changes appear after a short propagation delay, usually a couple of seconds, and longer when the system is under load. Visibility is built for finding and filtering across many executions, not for reading the current state of one execution. When you need the authoritative, up-to-date state of a specific Workflow Execution, use `DescribeWorkflowExecution` instead of a Visibility query.
