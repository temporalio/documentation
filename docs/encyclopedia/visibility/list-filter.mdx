...
Temporal supports aliasing of [default search attributes](/temporal-service/search-attribute#default-search-attribute) with the `Temporal` prefix. This allows custom search attributes that would previously collide with reserved search attributes.
...
When you have custom search attributes that conflict with default search attributes, aliasing respects your custom definitions:

- **Custom attributes take precedence**: If you've defined a custom search attribute that clashes with a default search attribute, the default search attribute will be accessible with `TemporalMySearchAttribute` and your search attribute with `MySearchAttribute`.
