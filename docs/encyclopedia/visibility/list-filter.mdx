...
Temporal supports aliasing of [default Search Attributes](/search-attribute#default-search-attribute) with the `Temporal` prefix. This makes it possible to define custom Search Attributes that use the same names as default attributes without causing collisions.
...
This means that if you define a custom Search Attribute with the same name as a default Search Attribute, the non `Temporal` prefixed name will refer to your custom attribute. You can still search with the default Search Attribute by using the `Temporal` prefix.

For example, if you have a custom Search Attribute named `WorkflowId`, List Filters using the following Search Attributes will return different results:

```sql
-- If you have a custom Search Attribute named 'WorkflowId'
-- This will use your custom attribute, not the default Search Attribute
WorkflowId = 'custom-workflow-id-123'

-- The original system attribute still works by using the Temporal prefix
TemporalWorkflowId = 'system-workflow-id-456'
```

`WorkflowId` will refer to your custom Search Attribute, while `TemporalWorkflowId` will refer to the default Search Attribute.

If you don't have a custom Search Attribute that uses the same name as a default Search Attribute, the Search Attribute and the `Temporal`-prefixed alias will return the same results. For example, if you don't have a custom Search Attribute `WorkflowVersioningBehavior` defined, List Filters using the following Search Attributes will return the same results:
