...
Avoid using sensitive data as a Search Attribute name (key) as well.
Attribute names are visible in Namespace configuration, query expressions, and Temporal UI, which increases the risk of exposure.
If a Search Attribute key containing sensitive information is compromised, it may reveal details about the data model or the nature of the data being processed.
This can result in violations of data protection regulations such as GDPR, HIPAA, or SOC 2.

If you need to correlate Workflow Executions with sensitive identifiers, store a non-sensitive lookup key (such as an opaque identifier or hash) as the Search Attribute value, and resolve it to the sensitive data in your own secure data store.
...
- Search Attributes are not encrypted in the system.
  ...
  See the [warning above](#search-attribute) for details.
