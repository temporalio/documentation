...
Storing a list of values in a non-list Search Attribute type (for example, multiple integers in an **Int** attribute or multiple strings in a **Keyword** attribute) is deprecated and will be removed in a future release.
...
If you are storing lists in **Int** attributes, consider either migrating to **KeywordList** (storing integers as strings, with exact-match queries only) or splitting into multiple individual **Int** attributes.

Temporal Cloud is actively enforcing this restriction.
New namespaces are already blocked from using this pattern, and existing namespaces that rely on it will need to migrate.
