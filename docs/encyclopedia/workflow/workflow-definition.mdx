...
title: What is a Temporal Workflow Definition?
...
The execution semantics of a Workflow Execution include the re-execution of a Workflow Function, which is called a [Replay](/workflow-execution#replays).
The use of Workflow APIs in the function is what generates [Commands](/workflow-execution#command).
Commands tell the Temporal Service which Events to create and add to the Workflow Execution's [Event History](/workflow-execution#event-history).
