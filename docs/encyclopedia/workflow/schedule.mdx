...
- [Schedule](#schedule)
  ...
  - Policies (Overlap Policy, Catchup Window, Pause-on-failure)
...
For triggering a Workflow Execution at a specific one-time future point rather than on a recurring schedule, the [Start Delay](/workflow-execution#delay-workflow-execution) option should be used instead of a Schedule.
...
It is important to note that the [status](/workflow-execution#status) of the subsequent run is marked as `Continued-As-New` and not as `Completed`.
