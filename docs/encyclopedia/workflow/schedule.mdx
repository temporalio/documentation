...
`ListSchedules` and `CountSchedules` are served by the Visibility subsystem. They share the [Visibility API rate limit](/evaluate/cloud/limits#visibility-api-rate-limit) and are eventually consistent, so a newly created or deleted schedule may not appear in the list right away. For more detail, see [Operations that use Visibility](/visibility#operations-that-use-visibility).
