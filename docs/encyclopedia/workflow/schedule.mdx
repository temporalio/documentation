...
- [How Workflow Pause interacts with Schedules](#workflow-pause)
...
A Paused Workflow Execution is still an open Workflow Execution.
For a Schedule, that means the Paused Workflow Execution counts as the currently running execution when the Schedule evaluates its [Overlap Policy](#overlap-policy).
...
Backfilled Actions are still subject to the Overlap Policy, so a Paused Workflow Execution can cause Backfilled Actions to skip, buffer, or wait unless you choose a policy that allows or replaces overlap.
...
A [Paused Workflow Execution](#workflow-pause) is still running for the purposes of the Overlap Policy.
