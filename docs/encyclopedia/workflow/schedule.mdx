...
By default, intervals align to the Unix epoch (January 1, 1970 at midnight UTC), so without a phase offset, a 7 day interval would only ever fire on dates that happen to fall on that epoch-aligned boundary.

You would use a phase offset when you need your recurring Schedule to fire at a specific, human-meaningful time like, every 10 days at 5pm UTC on a particular date, rather than accepting the arbitrary time the epoch alignment produces.
