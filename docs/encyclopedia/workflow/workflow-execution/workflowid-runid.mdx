...
Workflow Ids are stored in plain text, are **not** processed by a custom [Payload Codec](/dataconversion#payload-codec), and are visible in the Temporal Web UI, CLI output, Event History, and system logs.
