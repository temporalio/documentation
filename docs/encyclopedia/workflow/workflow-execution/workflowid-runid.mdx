...
Workflow Ids are stored in plain text and are **not** processed by a custom [Payload Codec](/dataconversion#payload-codec).
They are visible in the Temporal Web UI, CLI output, Event History, and system logs.
...
Do not use values such as social security numbers, email addresses, credit card numbers, or other personally identifiable information (PII) directly as a Workflow Id.
Doing so risks exposing sensitive data to anyone with Namespace access and may result in violations of data protection regulations such as GDPR, HIPAA, or SOC 2.
