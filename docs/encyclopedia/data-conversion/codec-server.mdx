...
Because both use the word "Codec," it is easy to confuse a Codec Server with a [Payload Codec](/payload-codec). They
serve different purposes:

| | Payload Codec | Codec Server |
| --- | --- | --- |
| **What it is** | A component in your [Data Converter](/dataconversion) that transforms Payloads (bytes to bytes). | An HTTP server that exposes your Payload Codec over the network. |
...
A Codec Server wraps a Payload Codec. You implement the transformation logic once in a Payload Codec, then host that
logic in a Codec Server so the Web UI and CLI can use it remotely.
