...
external store and replaces it with a lightweight reference. Payloads below the threshold stay inline in the Event
History. When the Temporal Service dispatches Tasks to the Worker, the Worker retrieves the reference payloads from
external storage and loads them into memory to execute the Task.
