...
| Default choice | Amazon S3, Google Cloud Storage, Azure Blob Storage. Durable object stores designed for arbitrary blob sizes. We provide a first-party S3 driver for the [Go](/develop/go/data-handling/external-storage#store-and-retrieve-large-payloads-with-amazon-s3) and [Python](/develop/python/data-handling/external-storage#store-and-retrieve-large-payloads-with-amazon-s3) SDKs. |
|---|---|
| **Use-case specific** | Google Cloud Bigtable, Redis, and similar low-latency stores. Must be configured for durability, and payloads must fit within the store's size constraints. |
| **Development only** | Local disk. Not durable or accessible across Workers. |
