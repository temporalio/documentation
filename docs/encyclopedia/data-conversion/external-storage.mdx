...
[Retry Policy](/encyclopedia/retry-policies) controls the timing of that retry. Design your storage driver and backing
store to tolerate transient failures on their own, for example with the redundancy features described below, since
each failure still costs a full task attempt rather than being retried in place.
