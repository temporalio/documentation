...
- **Data governance.** While Temporal supports end-to-end client-side encryption, some organizations prefer to store
  ...
  regardless of size.
- **Resilience to regional failures.** External Storage can be configured for cross-region durability so that payloads
  remain available during a regional outage. See [Durable External Storage](#durable-external-storage).
...
retrieve payloads. To protect against regional or provider failures, use your storage provider's redundancy features,
such as cross-region replication and multi-region routing.
