...
- **Data governance.** While Temporal supports end-to-end client-side encryption, some organizations prefer to store
  ...
  of size.
- **Resilience to regional failures.** When backed by Amazon S3, External Storage can be configured for cross-region
  durability so that payloads remain available during a regional outage. See
  [Durable External Storage with Amazon S3](#durable-s3).
...
If you use the S3 storage driver, you can make External Storage tolerant of regional failures by combining
[Cross-Region Replication (CRR)](https://docs.aws.amazon.com/AmazonS3/latest/userguide/replication.html) with an
[S3 Multi-Region Access Point (MRAP)](https://aws.amazon.com/s3/features/multi-region-access-points/). This complements
Temporal Cloud's [High Availability](/cloud/high-availability) offering (see
[Relationship to Temporal Cloud High Availability](#external-storage-and-temporal-cloud-ha) below to decide whether you
need it).

### The problem

S3 buckets are tied to a single AWS region. Writing to a single bucket creates a single point of failure if that region
or bucket becomes unavailable.

### How CRR and MRAP help

AWS provides two technologies for mitigating S3 failure:

- **S3 Cross-Region Replication (CRR)** asynchronously replicates objects from a source bucket to one or more destination
  buckets.
- **S3 Multi-Region Access Points (MRAP)** provide a single global endpoint that routes requests to the lowest-latency
  available regional bucket.

Use CRR for durability across regions, and MRAP as the bucket endpoint in your driver configuration.

To get started:

1. Create an S3 bucket in each failover region.
2. Enable versioning on all buckets (required for CRR).
3. Configure CRR and the replication IAM role and policy.
4. Create a [Multi-Region Access Point](https://aws.amazon.com/s3/features/multi-region-access-points/) that includes all
   regional buckets.
5. If you need an explicit replication SLA, enable
   [Replication Time Control (RTC)](https://docs.aws.amazon.com/AmazonS3/latest/userguide/replication-time-control.html).
6. Configure the External Storage S3 driver to use the MRAP ARN in place of the bucket name.

If a bucket or region becomes unavailable, Workers using the MRAP automatically fail over to the closest healthy bucket.

### Considerations

**Replication lag (eventual consistency).** CRR and MRAP do not make S3 strongly consistent across regions. After a
write, a read in another region may temporarily miss the object. To mitigate:
...
**Replication SLA.** By default, CRR has no SLA on replication time. If you need stronger guarantees, enable
[Replication Time Control](https://docs.aws.amazon.com/AmazonS3/latest/userguide/replication-time-control.html).

**Cost.** S3 replication and versioning can significantly increase storage costs.

### Relationship to Temporal Cloud High Availability {#external-storage-and-temporal-cloud-ha}

Temporal Cloud's [High Availability](/cloud/high-availability) offering and External Storage durability are independent
concerns. Use the table below to decide whether you should enable CRR and MRAP.

|                                                | Temporal Service (single region)                                                                | Temporal Cloud High Availability (multi-region)                                                          |
| ---------------------------------------------- | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| **S3 External Storage without CRR and MRAP**   | Both the Temporal Service and External Storage are regional single points of failure.           | The Service can fail over across regions, but External Storage may still be a regional point of failure. |
| **S3 External Storage with CRR and MRAP**      | External Storage tolerates S3 regional failure, but the Temporal Service may still be single-region. | Best end-to-end resilience across regional failures.                                                 |

In any multi-region External Storage setup, plan for replication lag and added cost from versioning and replication.
