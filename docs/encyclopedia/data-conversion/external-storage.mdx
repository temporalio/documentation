...
- **Amazon S3, Google Cloud Storage, Azure Blob Storage:** Default choice for durable payload storage. We provide a
  ...
  and a first-party Google Cloud Storage driver for the [TypeScript](/develop/typescript/best-practices/data-handling/external-storage#store-and-retrieve-large-payloads-with-google-cloud-storage) SDK.
...
:::info Storage failures are not retried within a Task attempt
