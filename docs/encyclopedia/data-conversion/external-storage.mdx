...
Start with an object store such as Amazon S3, Google Cloud Storage, or Azure Blob Storage unless you have a specific reason to use a different system, such as lower latency or existing infrastructure constraints.

A production storage system should:

- Store payload data durably and retain it for the full Workflow lifetime plus the Namespace retention period. See [Lifecycle management](#lifecycle) for details.
...
- **Amazon S3, Google Cloud Storage, Azure Blob Storage:** Default choice for durable payload storage. We provide a first-party S3 storage driver for the Go and Python SDKs.
