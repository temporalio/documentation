...
Temporal integrates with tracing libraries such as [OpenTelemetry](https://opentelemetry.io/) and [OpenTracing](https://opentracing.io/).
...
Tracing can be enabled via installing an interceptor on the Temporal Client or Worker in the different SDKs. See the
samples linked below to enable tracing in the supported SDKs:
