...
Temporal Cloud namespaces include additional capabilities, such as [API key](/cloud/api-keys) and [mTLS authentication](/cloud/certificates), [built-in role-based access controls](/cloud/manage-access/roles-and-permissions#namespace-level-permissions), [separate rate limits](/evaluate/cloud/limits#namespace-level), [high availability replication](/cloud/high-availability), [namespace tags](/cloud/namespaces#tag-a-namespace), and [namespace descriptions](/cloud/namespaces#describe-a-namespace).
...
- **Access control**: In self-hosted Temporal, use a custom [Authorizer](/self-hosted-guide/security#authorization) on your Frontend Service to restrict who can access each Namespace.
  Temporal Cloud provides [role-based access controls](/cloud/manage-access/roles-and-permissions#namespace-level-permissions) instead.
- **Inter-Namespace communication**: Namespaces are isolated by default.
  [Nexus](/nexus) is the controlled path for Workflows in one Namespace to call operations in another.
- **Multi-tenancy**: A single Namespace is still multi-tenant.
  ...
  For architectural guidance, see [Multi-tenant application patterns](/best-practices/multi-tenant-patterns).
