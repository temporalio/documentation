...
The Worker Controller Instance (WCI) is a system Workflow that manages Serverless Worker invocations. One WCI Workflow
runs per Worker Deployment Version that has a compute provider configured. The WCI runs in the same Namespace as your
Worker Deployment.

The WCI monitors Task Queues bound to the Worker Deployment Version and invokes the Lambda function when Tasks arrive.
The WCI also receives signals from the Matching Service when sync match failures occur, allowing it to respond to new
Tasks immediately rather than waiting for the next poll cycle.
