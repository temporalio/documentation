...
| High Availability | Namespaces with [Multi-region or Multi-cloud Replication](/cloud/high-availability) are not failover-aware for Serverless Workers. The compute provider configuration (for example, a Lambda ARN) is scoped to a single region, so after a failover the WCI keeps invoking the original region's Worker instead of one in the new active region. |
