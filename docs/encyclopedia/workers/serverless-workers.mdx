...
- **Invocation deadline > longest Activity runtime + shutdown deadline buffer.** Set on the compute provider to give
  ...
  [Activity Heartbeats](/detecting-activity-failures#activity-heartbeat) to record the state of the Activity execution
  so that the next retry can pick up where it left off.
