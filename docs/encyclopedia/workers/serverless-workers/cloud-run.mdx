...
Set it with `--gcp-cloud-run-scale-down-stabilization-duration` on `temporal worker deployment create-version` or `update-version`, which requires Temporal CLI v1.8.3 or later.
