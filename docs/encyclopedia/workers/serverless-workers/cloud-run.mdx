...
Because an instance runs an ordinary long-lived Worker, Cloud Run needs no provider-specific handler or package, unlike AWS Lambda.
Any Temporal SDK can run on a Cloud Run Worker Pool.
The one addition to a standard Worker is [Worker Versioning](/worker-versioning), which Serverless Workers require, so an SDK needs Worker Versioning support to run here.
Packaging the Worker into a container image is where the per-language differences show up, and each SDK's Cloud Run page covers them.
