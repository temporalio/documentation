...
These differences lead to the following practical choices:
...
| You want to run a standard long-running Worker and define its idle and drain behavior. | You want the Lambda Worker integration to manage the Worker around each invocation and its deadline. |
