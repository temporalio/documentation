...
Generally partitions will act as FIFO queues, so even sync matches (Tasks that can be dispatched immediately) are not delivered before async matches (older tasks).
