...
2. A Task Queue name must be set when creating a Worker Entity and when running a Worker Process:
   ...
   - [How to run a development Worker using the Go SDK](/develop/go/workers/basics)
...
5. A Task Queue name can be provided when creating a Nexus Endpoint.
   ...
   - [How to run a Nexus Worker using the Go SDK](/develop/go/nexus/service-handler#register-a-nexus-service-in-a-worker)
