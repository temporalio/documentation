---
id: glossary
title: Glossary
description: The following terms have specific definitions within the context of the Temporal Platform.
tags:
  - reference
---

<!-- prettier-ignore -->
import * as WhatIsTheTemporalPlatform from '../concepts/what-is-the-temporal-platform.md'

The following terms have specific definitions within the context of the <preview page={WhatIsTheTemporalPlatform}>Temporal Platform</preview>.

- [Activity](/docs/concepts/what-is-an-activity)
- [Activity Execution](/docs/concepts/what-is-an-activity-execution)
- [Activity Heartbeat](/docs/concepts/what-is-an-activity-heartbeat)
- [Activity Id](/docs/concepts/what-is-an-activity-id)
- [Activity Task](/docs/concepts/what-is-an-activity-task)
- [Activity Task Execution](/docs/concepts/what-is-an-activity-task-execution)
- [Advanced Visibility](/docs/concepts/what-is-advanced-visibility)
- [Archival](/docs/concepts/what-is-archival)
- [Child Workflow Execution](/docs/concepts/what-is-a-child-workflow-execution)
- [Command](/docs/reference/commands)
- [Continue-As-New](/docs/concepts/what-is-continue-as-new)
- [Data Converter](/docs/concepts/what-is-a-data-converter)
- [Event](/docs/concepts/what-is-an-event)
- [Event History](/docs/concepts/what-is-an-event-history)
- [List Filter](/docs/concepts/what-is-a-list-filter)
- [Local Activity](/docs/concepts/what-is-a-local-activity)
- [Namespace](/docs/concepts/what-is-a-namespace)
- [Parent Close Policy](/docs/concepts/what-is-a-parent-close-policy)
- [Query](/docs/concepts/what-is-a-query)
- [Retry Policy](/docs/concepts/what-is-a-retry-policy)
- [Run Id](/docs/concepts/what-is-a-run-id)
- [Schedule-To-Close Timeout](/docs/concepts/what-is-a-schedule-to-close-timeout)
- [Schedule-To-Start Timeout](/docs/concepts/what-is-a-schedule-to-start-timeout)
- [Search Attribute](/docs/concepts/what-is-a-search-attribute)
- [Side Effect](/docs/concepts/what-is-a-side-effect)
- [Signal](/docs/concepts/what-is-a-signal)
- [Standard Visibility](/docs/concepts/what-is-standard-visibility)
- [Start-To-Close Timeout](/docs/concepts/what-is-a-start-to-close-timeout)
- [Task](/docs/concepts/what-is-a-task)
- [Task Queue](/docs/concepts/what-is-a-task-queue)
- [Task Token](/docs/concepts/what-is-a-task-token)
- [Temporal](/docs/concepts/what-is-temporal)
- [Temporal Platform](/docs/concepts/what-is-the-temporal-platform)
- [Temporal SDK](/docs/concepts/what-is-a-temporal-sdk)
- [Worker](/docs/concepts/what-is-a-worker)
- [Workflow](/docs/concepts/what-is-a-workflow)
- [Workflow Definition](/docs/concepts/what-is-a-workflow-definition)
- [Workflow Execution](/docs/concepts/what-is-a-workflow-execution)
- [Workflow Execution Timeout](/docs/concepts/what-is-a-workflow-execution-timeout)
- [Workflow Id](/docs/concepts/what-is-a-workflow-id)
- [Workflow Id Reuse Policy](/docs/concepts/what-is-a-workflow-id-reuse-policy)
- [Workflow Run Timeout](/docs/concepts/what-is-a-workflow-run-timeout)
- [Workflow Task](/docs/concepts/what-is-a-workflow-task)
- [Workflow Task Execution](/docs/concepts/what-is-a-workflow-task-execution)
- [Workflow Task Timeout](/docs/concepts/what-is-a-workflow-task-timeout)
- [Workflow Type](/docs/concepts/what-is-a-workflow-type)
