...
If [Child Workflow](/workflows#child-workflow) is set to `Start` and `RequestCancel`, remove one of these attributes.
...
This error indicates that the Payload has exceeded the [Signal's](/workflows#signal) available input size.
...
This error indicates that the [Workflow](/workflows) has reached capacity for pending [Child Workflows](/workflows#child-workflow).
...
Therefore, the [Workflow Task](/workers#workflow-task) was failed after attempting to add more [Signals](/workflows#signal) to an external Workflow.
...
`UnhandledCommand` can happen when the Workflow is receiving a high number of [Signals](/workflows#signal).
