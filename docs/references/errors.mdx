...
This error indicates that the Payload has exceeded the [Signal's](/encyclopedia/application-message-passing#signals) available input size.
...
Therefore, the [Workflow Task](/workers#workflow-task) was failed after attempting to add more [Signals](/encyclopedia/application-message-passing#signals) to an external Workflow.
...
`UnhandledCommand` can happen when the Workflow is receiving a high number of [Signals](/encyclopedia/application-message-passing#signals).
