...
Update the [Temporal SDK](/encyclopedia/architecture/temporal-sdks) to the most recent release.
...
See [Per Workflow Nexus Operation Limits](/evaluate/cloud/limits#per-workflow-nexus-operation-limits) for details.
...
This error indicates that the concurrent [poller count](/develop/worker-performance/task-queues#poller-count) has been exhausted.
