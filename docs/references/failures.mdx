...
Nexus Operation handlers can avoid retrying by setting an Application Failure's non_retryable flag to true. When a [non-retryable error](/#nexus-errors) is returned from a Nexus handler, the overall Nexus Operation Execution is failed and the error is returned to the caller’s Workflow Execution as a **Nexus Operation Failure.**
