...
description: Discover the range of Commands Workers can issue to the Temporal Cluster after Workflow Task Execution, from Completing Workflow Execution to Start Timer and Signal External Workflow Execution.
...
A [Command](/workflows#command) is a requested action issued by a [Worker](/workers#worker) to the [Temporal Cluster](/clusters) after a [Workflow Task Execution](/workers#workflow-task-execution) completes.
...
This Command is triggered by a call to [Signal](/encyclopedia/application-message-passing#signals) another Workflow Execution.
