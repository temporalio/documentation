---
id: cluster-metrics
title: Temporal OSS Temporal Service metrics reference
sidebar_label: Temporal Service metrics
description: The Temporal Service emits a range of metrics to help operators get visibility into the Cluster’s performance and set up alerts.
...
A Temporal Service emits a range of metrics to help operators get visibility into the Cluster’s performance and to set up alerts.

All metrics emitted by the Temporal Service are listed in [defs.go](https://github.com/temporalio/temporal/blob/master/common/metrics/defs.go).

For details on setting up metrics in your Temporal Service configuration, see [Temporal Service](/references/configuration#global).
...
This metric is a good indicator for connection issues between Temporal Service and the persistence store.
Example:
