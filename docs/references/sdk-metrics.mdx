...
- `RequestTooLarge`: The whole Workflow Task completion exceeded the Namespace limit on the size of a single request, so the Worker proactively failed the Workflow Task as non-retryable rather than send a request the Temporal Service would reject.
