...
The information on this page is relevant to [Temporal SDKs](/encyclopedia/architecture/temporal-sdks).

See [Cloud metrics](/cloud/metrics) for metrics emitted by [Temporal Cloud](/evaluate/cloud/overview).

See [Temporal Service metrics](/references/cluster-metrics) for metrics emitted by a [self-hosted Temporal Service](/temporal-service).

Some SDKs emit metrics beyond what this reference lists.
...
- `PayloadsTooLarge`: A Payload in the Activity Task response exceeded the Namespace size limit, so the Worker proactively failed the Activity Task as retryable. See [Activity Payloads too large](/troubleshooting/execution-failures#activity-payloads-too-large).
...
- `timeout`: The Local Activity exceeded its schedule-to-close or start-to-close timeout.
...
The Schedule-To-Start time of a Nexus Task.
The schedule time is taken from when the corresponding request hit the Frontend service to when the SDK started processing the task.
