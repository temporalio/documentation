...
- `internal_sdk_error`: There was an unexpected internal error within the SDK while handling the Nexus task. Indicates a
  bug in the SDK.
- `handler_error_{TYPE}`: The user handler code returned an error with a predefined error specified in the [Nexus
...
- `operation_failed`: The user handler code has indicated that the operation has completed as canceled. In Go, this maps
