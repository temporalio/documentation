...
| `component.nexusoperations.useSystemCallbackURL`       | String  | (New with version 1.30.X) Defines the URL template used to construct Nexus callback endpoints. (Only required for the experimental external endpoint target feature or servers older than 1.30.)     | `false`                                                                 |
| `component.nexusoperations.callback.endpoint.template` | String  | (Legacy as of version 1.30.X) Defines the URL template used to construct Nexus callback endpoints. (Only required for the experimental external endpoint target feature or servers older than 1.30.) | `https://$PUBLIC_URL:7243/namespaces/{{.NamespaceName}}/nexus/callback` |
| `component.callbacks.allowedAddresses`                 | Object  | (Legacy as of version 1.30.X) Defines the security allow-list of callback URL patterns that the server will accept; used to restrict what callback endpoints can be invoked.                         | (See below sub-properties)                                              |
| `component.callbacks.allowedAddresses.Pattern`         | String  | (Legacy as of version 1.30.X) Defines which callback URLs are permitted to be called by the server. (the patterns is a wildcard)                                                                     | `*`                                                                     |
| `component.callbacks.allowedAddresses.AllowInsecure`   | Boolean | (Legacy as of version 1.30.X) Enables whether insecure (non-TLS/HTTP) URLs matching the pattern are permitted; should be false in production when HTTPS is supported.                                | `true`                                                                  |