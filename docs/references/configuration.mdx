...
- `httpPort`: Port for the [HTTP API](/self-hosted-guide/server-frontend-api-reference#http-api). Set to a non-zero value to enable the HTTP API on the frontend service. When enabled, the HTTP API translates HTTP requests to gRPC calls and serves OpenAPI specs. Defaults to `0` (disabled).
