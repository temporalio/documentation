...
sidebar_label: Temporal Service configuration
description: Much of the behavior of a Temporal Service is configured using the `development.yaml` file.
...
Used by gossip protocol to communicate with other hosts in the same Temporal Service for membership info.
Use IP address that is reachable by other hosts in the same Temporal Service.
If there is only one host in the Temporal Service, you can use 127.0.0.1.
Check `net.ParseIP` for supported syntax, only IPv4 is supported.

### metrics

Configures the Temporal Service's metric subsystem.
...
_Required_ - The number of history shards to create when initializing the Temporal Service.

**Warning:** This value is immutable and will be ignored after the first run.
Please ensure you set this value appropriately high enough to scale with the worst case peak load for this Temporal Service.
...
_Required_ - The name of the primary data store definition that should be used to set up [Visibility](/clusters#visibility) on the Temporal Service.

### secondaryVisibilityStore

_Optional_ - The name of the secondary data store definition that should be used to set up [Dual Visibility](/visibility#dual-visibility) on the Temporal Service.
...
- `enableHostVerification` - _boolean_ - `true` to verify the hostname and server cert (like a wildcard for Cassandra Temporal Service). This option is basically the inverse of `InSecureSkipVerify`. See `InSecureSkipVerify` in http://golang.org/pkg/crypto/tls/ for more info.
...
`clusterMetadata` contains the local Temporal Service information. The information is used in [Multi-Cluster Replication](/clusters#multi-cluster-replication).
...
- `currentClusterName` - _required_ - the name of the current Temporal Service. **Warning:** This value is immutable and will be ignored after the first run.
...
- `failoverVersionIncrement` - the increment of each Temporal Service version when failover happens.
- `masterClusterName` - the master Temporal Service name, only the master Temporal Service can register/update namespace. All clusters can do namespace failover.
- `clusterInformation` - contains the local Temporal Service name to `ClusterInformation` definition. The local Temporal Service name should be consistent with `currentClusterName`. `ClusterInformation` sections consist of:
  - `enabled` - _boolean_ - whether a remote Temporal Service is enabled for replication.
...
- `membershipPort`: Port used to communicate with other hosts in the same Temporal Service for membership info.
  Each service should use different port.
  If there are multiple Temporal Services in your environment (Kubernetes for example), and they have network access to each other, each Temporal Service should use a different membership port.
...
- `state`: State for Archival setting. Supported values are `enabled`, `disabled`. This value must be `enabled` to use Archival with any Namespace in your Temporal Service.
  - `enabled`: Enables Archival in your Temporal Service setup. When set to `enabled`, `URI` and `namespaceDefaults` values must be provided.
  - `disabled`: Disables Archival in your Temporal Service setup. When set to `disabled`, the `enableRead` value must be set to `false`, and under `namespaceDefaults`, `state` must be set to `disabled`, with no values set for `provider` and `URI` fields.
...
- To enable Archival in your Temporal Service configuration:

  ```yaml
  # Temporal Service-level Archival config enabled
  archival:
...
- To disable Archival in your Temporal Service configuration:

  ```yaml
  # Temporal Service-level Archival config disabled
  archival:
    history:
      state: "disabled"
      enableRead: false
...
- `policy`: Supported values are `noop`, `selected-apis-forwarding`, and `all-apis-forwarding`.
  ...
  - `selected-apis-forwarding`: Sets up forwarding for the following APIs to the active Temporal Service based on the Namespace.
    ...
    - `QueryWorkflow`
  - `all-apis-forwarding`: Sets up forwarding for all APIs on the Namespace in the active Temporal Service.
...
Configuration for setting up file-based [dynamic configuration](/clusters#dynamic-configuration) client for the Temporal Service.
