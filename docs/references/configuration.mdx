...
`clusterMetadata` contains the local Temporal Service information. The information is used in [Multi-Cluster Replication](/temporal-service#multi-cluster-replication).
...
- `masterClusterName` - the master Temporal Service name, only the master Temporal Service can register/update namespace. All temporal-service can do namespace failover.
...
Configuration for setting up file-based [dynamic configuration](/temporal-service#dynamic-configuration) client for the Temporal Service.
