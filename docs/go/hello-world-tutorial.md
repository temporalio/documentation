...
There are two ways to start a Workflow, via the Temporal CLI or Temporal SDK. In this tutorial we use the SDK to start the Workflow which is how most Workflows are started in live environments. Additionally, the call to the Temporal server can be made [synchronously or asynchronously](/docs/go/how-to-spawn-a-workflow-execution-in-go). Here we do it synchronously, so you will see the caller wait for the result of the Workflow.
