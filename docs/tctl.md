...
```
docker run --network=host --rm temporalio/tctl:1.0.0 --namespace samples-namespace namespace describe
```
...
`--tls_cert_path=<certificate file path>` command line argument allows to pass a certificate for server to validate the client (`tctl`) identity. Requires `--tls_key_path` to also be provided.

`--tls_key_path=<private key file path>` command line argument allows to pass a private key ofr secure communication with the server. Requires `--tls_key_path` to also be provided.
