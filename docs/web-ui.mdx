...
On self-hosted Temporal Services, manage your users, metrics, and logging in your [server configuration](/references/configuration).

<!--
AB: Commenting because this is redundant now? Also this needs to be updated for self-hosted Temporal Services.
...
- Temporal Services: Temporal Service information -->

### Archive

On self-hosted Temporal Services, Archive shows [Archived](/Temporal Services#archival) data of your Workflow Executions on the Namespace.
...
The UI supports both a [Codec Server endpoint](/production-deployment/data-encryption#web-ui) and the `tctl` plugin port.
