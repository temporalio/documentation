...
AI Agents present new uses for Temporal, such as maintaining state over long periods and letting a human step in when needed.
...
For an implementation walkthrough with compensation ordering and code samples, see the [Saga Pattern](/design-patterns/saga-pattern) reference.
