...
Need a responsive Workflow? Think about use cases like these:
...
- Your eCommerce shopping cart Workflow needs to know when a new item is added. **Update** it to add the item and receive back the current items to render on the next page.

Temporal provides Signals, Queries, and Updates to allow rich interactivity with your running Workflows.

**Signals**: Signal to send messages asynchronously to a running Workflow, perhaps changing its state or controlling its flow in real-time.

**Queries**: Query to monitor or debug the internal state of your Workflow in real-time.
