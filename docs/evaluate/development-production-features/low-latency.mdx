---
id: low-latency
title: Low latency - Temporal feature
description: Discover how Temporal's design and features ensure low latency for your applications. 
sidebar_label: Low latency
keywords:
  - low latency
  - temporal performance
  - temporal cloud
  - self-hosted temporal
  - workflow latency
  - temporal benchmarks
  - temporal sdk metrics
  - optimized workflow performance
  - latency-sensitive applications
  - temporal cloud advantages
tags:
  - temporal-latency
  - temporal-cloud
---

import { RelatedReadContainer, RelatedReadItem } from '@site/src/components/related-read/RelatedRead';

Temporal provides features that significantly reduce latency, making your applications faster, more performant, and more efficient.
In the world of modern applications, low latency is crucial for ensuring minimal delay in Workflow Executions.

Temporal Cloud includes architectural advancements like a custom persistence layer, which reduce latency effectively.
Benchmarks have shown that application-side latency is lower when using Temporal Cloud. 
This low-latency architecture ensures rapid Workflow Execution and responsiveness, critical for time-sensitive applications and high-performance systems.

Temporal Cloud’s architecture includes a custom persistence layer with three key components:

- **Better Sharding:** Distributes load across multiple databases, preventing bottlenecks. 
Better sharding of persistent state across multiple databases allows for independent resizing, improving scalability and handling high-traffic events without delay.
- **Write-Ahead Log (WAL):** Aggregates updates before writing to the database, reducing write latency. 
This log stores writes in an append-only format, reducing latency and database size by batching updates before writing to the database.
- **Tiered Storage of Workflow Event History:**  Offloads completed Workflow Event Histories, improving database efficiency. Moving completed Workflow Event Histories to an object store reduces database load, improving efficiency and latency for running Workflows.

Temporal Cloud provides lower latency, making it suitable for latency-sensitive, large-scale, or business-critical applications.

<RelatedReadContainer>
  <RelatedReadItem path="https://learn.temporal.io/blog/exploring-temporal-cloud-automation-features" text="Exploring Temporal Cloud Automation Features" archetype="blog-post" />
  <RelatedReadItem path="https://learn.temporal.io/blog/high-availability-disaster-recovery" text="High Availability and Disaster Recovery with Temporal Cloud" archetype="blog-post" />
  <RelatedReadItem path="https://learn.temporal.io/blog/estimating-cost-of-temporal-cloud" text="Estimating the Cost of Temporal Cloud" archetype="blog-post" />
  <RelatedReadItem path="https://learn.temporal.io/blog/migrating-to-temporal-cloud" text="How to Migrate Your Self-Hosted Service to Temporal Cloud" archetype="blog-post" />
  <RelatedReadItem path="https://learn.temporal.io/webinar/custom-persistence-layer" text="Custom Persistence Layer of Temporal Cloud" archetype="webinar" />
  <RelatedReadItem path="https://docs.temporal.io/cloud/service-availability#latency" text="Temporal Cloud’s Latency SLO" archetype="slo" />
  <RelatedReadItem path="https://www.youtube.com/watch?v=SQv9ot-jB6o&list=PLl9kRkvFJrlREHL7fiEKBWTp5QuFeYS2r&index=5" text="Replay Conference Talk: What’s Cloud Got to Do with It?" archetype="conference-talk" />
</RelatedReadContainer>
