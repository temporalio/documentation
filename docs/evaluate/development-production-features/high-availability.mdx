---
id: high-availability
title: High availability - Temporal Cloud and Self Hosted production feature
description: Temporal Cloud's Multi-region Namespaces offer automated failover, synchronized data replication, and high availability for workloads requiring disaster-tolerant deployment and 99.99% uptime. Use Global Namespace for self-hosted. 
sidebar_label: High availability
tags:
  - production-deployment
  - production-features
  - temporal-cloud
  - high-availability
keywords:
  - availability
  - explanation
  - failover
  - high-availability
  - multi-region
  - multi-region namespace
  - namespaces
  - temporal-cloud
  - term
---
import { RelatedReadContainer, RelatedReadItem } from '@site/src/components/related-read/RelatedRead';

Temporal offers high availability as a feature for both Temporal Cloud and self-hosted deployments. These features ensures your applications remain operational even in the face of regional outages or service disruptions.

- [Multi-region Namespaces for Temporal Cloud](/evaluate/development-production-features/multi-region-namespace): Temporal Cloud's Multi-region Namespaces offer automated failover, synchronized data replication, and high availability for workloads requiring disaster-tolerant deployment and 99.99% uptime.
- [Global Namespace for Self-Hosted](/evaluate/development-production-features/global-namespace): For self-hosted Temporal deployments, Global Namespace provides enhanced availability across multiple Temporal Services, with automatic replication of metadata and Workflow Execution state.



<RelatedReadContainer>
    <RelatedReadItem path="/cloud/multi-region" text="Multi-region Namespaces for Temporal Cloud" archetype="encyclopedia" />
    <RelatedReadItem path="/namespaces#global-namespace" text="Global Namespace for self-hosted deployments" archetype="encyclopedia" />
    <RelatedReadItem path="/cloud/pricing" text="Multi-region Pricing" archetype="encyclopedia" />
    <RelatedReadItem path="/cloud/multi-region#routing" text="PrivateLink routing for Multi-region" archetype="encyclopedia" />
</RelatedReadContainer>
