...
- **Soft limits** can be adjusted by Temporal Cloud based on usage or upon request.
  Some limits are automatically tracked and changed by Temporal Cloud as your usage increases.
  For example, as your number of Namespaces or the rate of Actions per second (APS) grows, Temporal Cloud will gradually allocate more resources to meet that demand.
  For all soft limits, you can also request adjustments by reaching out to [Support](https://support.temporal.io), whether they are automatically managed or not.
...
The size limit for any given [Event History](/workflow-execution#event-history) transaction is 4 MB.
...
- **Blob size limit** for Payloads, including Workflow context and each Workflow and Activity argument and return value:
  ...
  - The max size limit for any given [Event History](/workflow-execution#event-history) transaction is 4 MB.
...
See the Nexus Encyclopedia entry for [additional details](/workflow-execution#workflow-execution-nexus-operation-limits).
