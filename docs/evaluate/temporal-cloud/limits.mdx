...
A single Workflow Execution can have a maximum of 30 in-flight Nexus Operations and 30 total Nexus Operations.
Public Preview doesn't yet remove completed Nexus operations from mutable state. 
After the Workflow Execution reaches that limit, it will process no more Nexus Operations.
