...
Identifiers such as Workflow Id, Workflow Type, and Task Queue names have a maximum length of 1,000 bytes, Unicode characters may use multiple bytes.
characters may take up more than a byte each.
