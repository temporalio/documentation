...
Credits provide an additional discount schedule for billable [Actions](#action-and-operations) and [storage](#storage).
Credits do not expire.

Credits applied to a Temporal Cloud Account unlock volume-based discounts.
Discounts are applied separately to each Namespace so pricing may not be uniform between Namespaces. This table shows our credit tiers and thresholds for each calendar month of use:
...
When you purchase Temporal Cloud with credits your account automatically unlock tiered price volume discounts.
...
Consider an account [eligible for volume discounts](#credit-system) that uses two Namespaces.
