...
- **Workflow started**.
  Occurs via client start, [Continue-As-New](/workflow-execution/continue-as-new), [Child Workflow](/child-workflows) start, or for any [Signal-With-Start](/sending-messages#signal-with-start). Also occurs with [Update-With-Start](/sending-messages#update-with-start) if the Workflow was not already started.
...
- **Signal sent**.
  An Action occurs for every [Signal](/sending-messages#sending-signals), whether sent from a Client or from a Workflow. Note that one total Action is created for [Signal-With-Start](/sending-messages#signal-with-start) regardless of whether the Workflow was started.
...
- **Update received by Worker**.
  An Action occurs for every successful [Update](/sending-messages#sending-updates) and every [rejected](/handling-messages#update-validators) Update. This includes for [Update-With-Start](/sending-messages#update-with-start), and is in addition to the start Action in the case when the Workflow was started.
...
- **Start Child Workflow Execution initiated**.
  When the parent Workflow durably records the intent to start a Child Workflow, it results in one Action.
- **Child Workflow Execution attempted**.
  When the server attempts to start the Child Workflow, it results in one Action.
