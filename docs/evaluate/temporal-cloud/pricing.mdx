...
- **Workflow Update received**. An Action occurs when an [Update](/encyclopedia/workflow-message-passing#sending-updates) is received by a Worker and either processed by the Update handler, or rejected by an [Update Validator](/encyclopedia/workflow-message-passing#update-validators).
