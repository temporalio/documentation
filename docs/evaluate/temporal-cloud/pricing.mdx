...
* Nexus Operation scheduled. The caller Workflow starting a Nexus Operation results in one Action on the caller Namespace.  
* Nexus Operation canceled. The caller Workflow canceling a Nexus Operation results in one Action on the caller Namespace.  
* The underlying Temporal primitives such as Workflows, Activities, and Signals are created by a Nexus Operation handler result in the normal Actions for those primitives billed to the handler’s namespace. 
This includes retries for underlying Temporal primitives like Activity retries.  
