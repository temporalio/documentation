...
Once you have an account, you can view your cost and billing details in the account directly.
For more information, view [Billing and Cost](/cloud/billing-and-cost).
...
:::tip OPERATIONS THAT RESULT IN ACTIONS
...
If you need to store closed Workflow history for longer than the maximum 90-day period on Temporal Cloud, we recommend leveraging **Export**.
...
Each account is required to select a support plan.
You'll find a complete description of our available plans in our [Support](https://docs.temporal.io/cloud/support) documentation.
...
Account Owners and Finance Admin can view their detailed billing data at any time in the [Usage and Billing dashboards](/cloud/billing-and-cost) in Temporal Cloud.

:::tip Two pricing options for Temporal Cloud

Temporal Cloud offers two payment choices: pay-as-you-go and prepaid Temporal Credits.
With pay-as-you-go, you're invoiced each month.
With Temporal Credits, you pre-pay your Temporal Cloud spend and invoiced only when you run out of Temporal Credits.
Prepaid Temporal Credits allows for discounted pricing (details below).
Both models bill for three components: Actions, storage, and support.
...
This section shows how you're billed each calendar month and gives examples for each pricing style.

### Pay-as-you-go Action pricing {#payg-action-pricing}
...
This rate applies across all Namespaces and is pro-rated per Namespace.

| Actions | &#36;25 per one million |
| ------- | ------------------------|

**Example**

If you use fewer than one million Actions per month in a Namespace, your bill for that Namespace's Actions will be less than &#36;25 for that calendar month.
...
### Pay-as-you-go storage pricing {#payg-storage-pricing}

You're billed for your Namespaces' Active and Retained Storage for each calendar month.
...
Say you consume 720 GBh of Active Storage and 3,600 GBh of Retained Storage in one calendar month your bill is:
...
### Pay-as-you-go support pricing {#payg-support-pricing}

Support pricing offers [two tiers](/cloud/support#support):

- Basic Support is priced at the maximum of &#36;200/mo or 10% of Temporal Cloud consumption.
- Premium Support is priced at the maximum of &#36;2,000/mo or 10% Temporal Cloud consumption.
Temporal cloud consumption is based on the combined Actions and Storage cost.

**Example**

Say that you are signed up for Basic Support with &#36;20 of Actions and &#36;31.75 of Storage consumed in one month.
Using this example, your Basic Support costs are:

```
Max of $200 or 10% * ($20 (Actions) + $31.75 (Storage)) = $5.18, so $200.
```

### Calculate your monthly pay-as-you-go costs {#payg-monthly-costs}

Your total invoice each calendar month is the combination of Temporal Cloud consumption (Actions and storage) and support.

**Example**

For these examples in this section, your monthly pay-as-you-go bill will be:

```
$20 (Actions) + $31.75 (Storage) + $200 (Support) = $251.75
```

## Prepaid Temporal Credits Pricing {#prepaid}

**How does prepaid Credit pricing work for Actions, storage, and support?**


## <div style={{backgroundColor: '#ffff00',padding: '0px 15px',borderRadius: '5px',border: '1px solid #cccccc',display: 'inline-block'}}>**REST OF UNUSED MATERIAL**</div>
...
If you purchase Temporal Cloud credits, your account automatically unlocks tiered volume pricing for Active Storage.
Volume prices are calculated on a calendar month basis and reset every calendar month.
Volume tiers are measured in gigabyte-hours.
Each Active storage tier applies per Namespace:
...
### Volume pricing for Actions

**How do I get discounted pricing for Actions?**

Credits applied to a Temporal Cloud Account unlock volume-based pricing.
This table shows our volume price tiers and thresholds for each calendar month of use:
...
Volume tiers are applied separately to each Namespace so pricing may not be uniform between Namespaces.

:::tip HOW TO PURCHASE CREDITS

You can purchase Temporal Cloud credits by contacting our team at [sales@temporal.io](mailto:sales@temporal.io).

:::

### Calculating Action volume prices {#action-volume-prices}

**How does tiered volume pricing work?**

When you purchase credits for Temporal Cloud, your account automatically unlocks tiered volume pricing.
Volume prices are calculated on a calendar month basis and reset every calendar month.

Here's an example that demonstrates how this works.
Consider a Temporal Cloud account paid for with credits, so it's eligible for volume discounts.
This account uses two Namespaces.
In this example, one Namespace consumes 100 million Actions in a calendar month and one consumes 500 million Actions during the same month.
Here's how the billing works for these two Namespaces, and how the account is invoiced for the month.

- The first Namespace using 100 million Actions is billed at the &#36;23.25 per million Actions tier:

  ```
  100M Actions ⨉ $23.25 per million Actions = $2,325 (Namespace total)
  ```

- The second Namespace with 500 million Actions is billed at the &#36;23.25 per million Actions tier for the first 300 million Actions and at the &#36;18.80 per million Actions tier for the remaining 200 million Actions:
...
### Calculating Storage volume prices {#storage-volume-prices}

**How do I get discounted pricing for Storage?**
...
Consider a Temporal Cloud account paid for with credits, so it's eligible for volume discounts.
This account uses two Namespaces.
In this example, one Namespace consumes 5,000 GBh of active storage during a calendar month one consumes 14,000 GBh of active storage during the same month.
Here's how the billing works for these two Namespaces, and how the account is invoiced for the month.

- The first Namespace is charged at the &#36;0.039 per GBh tier:

  ```
  5,000 GBh ⨉ $0.039 per GBh = $117 (total Active Storage cost for first Namespace)
  ```

- The second Namespace is charged at the &#36;0.039 per GBh tier for the first 7,000 GBh, and at the &#36;0.031 per GBh tier for the remaining 7,000 GBh:

  ```
  7,000 GBh ⨉ $0.039 per GBh = $273
  7,000 GBh ⨉ $0.031 per GBh = $217
  $273 + $217 = $490 (total Active Storage cost for second Namespace)
  ```

- The account is charged $753 for the month:
...
Services and support pricing is outlined in the table below.
...
**What are Temporal Cloud credits?**

A Temporal Cloud credit is a unit used for pre-payment of Temporal services.
Each credit is equal to $1.
Credit usage is computed each calendar month and deducted from your credit balance.

**Are there advantages to purchasing Temporal Cloud credits?**

Credits provide an additional discounted pricing schedule for billable Actions and storage.
The credits applied to a Temporal Cloud account include volume-based pricing for each Namespace.
