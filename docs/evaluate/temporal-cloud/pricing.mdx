...
:::tip Scheduled workflows and batch jobs contribute to action costs:

Some pending entities accrue **action costs** even before they execute:

- [**Scheduled Workflows**](https://docs.temporal.io/schedule): Each schedule counts as an action as long as it exists, regardless of whether it has triggered any workflows yet.
- [**Batch Jobs**](https://docs.temporal.io/cli/batch): Batch executions that have been created but not started also incur action costs for their metadata and configuration.

These pending actions remain billable until deleted. To optimize your action usage and reduce costs, consider cleaning up unused schedules and batch jobs proactively.
...
:::tip Storage costs are also affected by pending workflows, including:

- [Schedules](https://docs.temporal.io/schedule): Each scheduled workflow that is yet to be triggered contributes to storage usage.
- [Batch jobs](https://docs.temporal.io/cli/batch): Batch workflow executions that are created but not yet started also consume storage.\
  These pending states are retained by the Temporal Cloud service until execution, and their metadata and configurations contribute to overall storage consumption. For cost optimization, consider managing the lifecycle of these entities carefully.
