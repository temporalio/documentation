...
- The underlying Temporal primitives (Workflows, Activities, Signals, …) created by a Nexus Operation handler (directly or indirectly) result in the normal Actions for those primitives billed to the handler’s Namespace. 
