...
Running a single Activity as a Standalone Activity rather than wrapping it in a Workflow costs fewer [Billable Actions](/evaluate/cloud/actions-usage#actions-in-workflows) on Temporal Cloud, and for short jobs it's lower latency, because there are fewer Worker round-trips.
...
## Release stages and limitations

Standalone Activities are Generally Available, including Start Delay, Request Cancel, Terminate, and Delete. These capabilities are in [Public Preview](/evaluate/product-release-stages#public-preview):
...
The `TerminateExisting` conflict policy is not supported yet. Use `Fail` or `UseExisting` instead. See [Limitations](/standalone-activity#limitations) for the full list.
