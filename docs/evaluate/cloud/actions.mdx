...
description:
  Which operations count as billable Actions in Temporal Cloud, by category, with the metric name and Event History type
  for each one.
...
Actions are the primary unit of consumption-based pricing for Temporal Cloud. They track billable operations within the
Temporal Cloud Service, such as starting Workflows, recording a Heartbeat, or sending Signals.

Most Actions fall into one of these categories:
...
[Event History](/evaluate/cloud/actions-usage#actions-in-workflows) and [Usage](https://cloud.temporal.io/usage) Dashboards,
and the [Billing API](/cloud/billing-api). Use Action types and categories to estimate usage, find the Action types
driving it, track spikes, and troubleshoot errors. For example, you can see whether a new Workflow Type on a Namespace
is driving a usage increase, or find the aggregate usage of Activity Heartbeats on a Namespace.

Which Action Types you see depends on the endpoint. Action Categories and Action Types are not available in the Billing
API.

Usage and billing data carry the most complete Actions data, while Event History and metrics support detailed
_estimates_. [History Event Types](/workflow-execution/event) are listed for transparency, but they do not always map
1:1 to Action Types. The following sections list each category, its Action types, and where each one is available.
...
- **Activity started or retried**. Occurs each time a [**Workflow Activity**](/workflow-activity) or [**Standalone Activity**](/standalone-activity) is started or retried.
  - De-duplicated Standalone Activity starts that return an already-running Activity (sharing an Activity ID) do _not_
    count as an Action, unless the start request performs an Activity options update, like attaching a callback to the running Activity — for example,
    within a Nexus handler using the `USE_EXISTING` conflict policy.
- **Standalone Activity pause, reset, and update options**. Each count as one Action. Unpause does _not_ count as an Action.
  See [Activity Operations](/activity-operations).
...
A [Schedule](/schedule) starts a Workflow at a particular time. Each execution of a Schedule accrues three Actions:
...
Track Actions usage at the granularity you need. See [Billing and Usage](/cloud/billing-and-usage) for the available
views.
