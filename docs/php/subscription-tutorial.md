...
4. At any point while subscription (or trial) is ongoing, be able to cancel subscription with sending a **sorry email**.
...
The whole task *"create a subscription"* looks too complicated, thus we need to break it into small manageable peaces.
...
Consider activity implementation as implementation details of you business logic (workflow), and thus we don't cover 
them in this tutorial. When building this subscription workflow we will walk through the business process and 
use only activity interface. It is up to you to implement all other details.

:::

Method `subscribe(string $userID)` will contain all the business logic. First, we send an email that the trial period 
...
Using timer is safe because if the workflow crashes, then after restart it will continue right from the crash and
not from the scratch. It means that if the workflow has been waiting for 29 days and then crashes, it will be
able to recover and continue waiting with those 29 days. It will not be possible when using native PHP `sleep()` function.

:::

## Receive Cancellations

Per Requirement 2, users can cancel during the trial. Once the trial period or subscription is cancelled we should
email the user. How can we implement subscription cancellation? There are several ways to do it, but the simplest is 
just to cancel the workflow. It is obvious here, that we need a separate console command for cancellation. Let' create one:
...
This command accepts `$userId` as an input argument, then fetches the workflow with id of `'subscription:' . $userID` and
tries to cancel it. That's it, very simple. Now, we need to handle cancellation within the running workflow. Once the
running workflow is cancelled `CancelledFailure` exception is thrown. We can catch it and send an email like this:
...
Here we catch `CanceledFailure` and continue with activity to send a sorry email. Why do we use `Workflow::asyncDetached()`
instead of plain `yield`? We use a "native way" to cancel a business process (a workflow) here. When workflow is 
cancelled all internal coroutines will be also cancelled. The email should be sent even if the main workflow is already
closed. Thus, we need to run it into a detached coroutine, that doesn't belong to the workflow. `Workflow::asyncDetached()`
does this job: everything inside the callback will be executed inside the detached coroutine, that doesn't belong to the
calling workflow. Exactly what we need here. Having that actually we can handle any cancellations: trial or monthly subscription. So, let's continue and finally implement subscription
workflow.
...
If we assume that the subscription period is 30 days, and it should last until it is manually cancelled, then we 
need an "endless loop" here. We "endlessly" wait for 30 days and charge monthly fee. Also, don't forget about the
trial period at the beginning.
...
On the next iteration we again wait for 30 days, charge monthly fee and send email. The last thing we need to do is
to handle subscription cancellation. At this moment we only send "sorry" email, but we definitely will have a dedicated
subscription service, which should be notified once the user cancels his or her subscription. 
...
The beauty of using Temporal is that a relatively complex business process is written with a few lines 
of code. There is no rocket science in the code of our `SubscriptionWorkflow`, instead the code is very
straight-forward. Workflow code provides us with a high-level view of the business process without digging
into details.
