...
export async function recordProcessorWorkflow(recordId: string): Promise<void> {
  ...
  // Read the parent's Workflow ID from this Workflow's own metadata. The
  // parent's Workflow ID is stable across Continue-as-New, so signaling by
  // Workflow ID (no run ID) always reaches the current run.
  const parentId = workflowInfo().parent?.workflowId;
  if (!parentId) throw new Error("recordProcessorWorkflow must run as a Child Workflow");
  // Ignore NOT_FOUND — the parent's final run may have already completed.
  try {
    const parent = getExternalWorkflowHandle(parentId);
...
class RecordProcessorWorkflow:
    ...
    async def run(self, record_id: str) -> None:
        ...
        assert parent is not None, "must run as a Child Workflow"
        # Ignore NOT_FOUND — the parent's final run may have already completed.
        try:
...
func RecordProcessorWorkflow(ctx workflow.Context, recordID string) error {
	...
	// Read the parent's Workflow ID from this Workflow's own metadata. The
	// parent's Workflow ID is stable across Continue-as-New, so signaling by
	// Workflow ID (empty run ID) always reaches the current run.
	parent := workflow.GetInfo(ctx).ParentWorkflowExecution
	if parent == nil {
		return errors.New("RecordProcessorWorkflow must run as a Child Workflow")
	}

	// Ignore not-found — the parent's final run may have already completed.
	err := workflow.SignalExternalWorkflow(ctx, parent.ID, "", CompletionSignal, recordID).Get(ctx, nil)
...
public class SlidingWindowWorkflowImpl implements SlidingWindowWorkflow {
    ...
    private void startChild(String recordId, String parentId) {
        ...
        // The child reads the parent's Workflow ID with Workflow.getInfo().getParentWorkflowId().
