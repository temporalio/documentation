...
Each child executes as an independent Workflow with its own Workflow ID, event history (51,200 event limit), and lifecycle.
...
Each child is a first-class Workflow with its own ID for tracking, its own 51,200-event history limit, and its own execution timeout configuration.
