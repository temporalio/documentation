...
If you are new to Temporal Cloud, see the [pricing documentation](/cloud/pricing#action) to learn more and familiarize yourself with [what results in a billable Action](/evaluate/cloud/actions) in Temporal Cloud.
...
[Actions](/evaluate/cloud/actions) encompass Workflow operations, Activity Executions, Signals, Queries, and other interactions with Temporal.
...
[Child Workflows cost 2 Actions](/evaluate/cloud/actions#workflow) compared to an Activity's 1 Action.
...
Running the same single Activity in a wrapper Workflow costs 1 Action for the Activity and 1 Action for the Workflow, not including Activity heartbeats, retries, or other billable [Actions](/evaluate/cloud/actions#activity).
...
Therefore, multiple Local Activities that run back-to-back only [count as a single billable action](/evaluate/cloud/actions#activity), whereas each regular Activity counts as a billable action.
...
See the [Temporal Cloud Action Documentation](/evaluate/cloud/actions#workflow) for details.
...
2. **Monitor comprehensively**: Leverage the [Usage dashboard](/evaluate/cloud/actions-usage#usage) in the Cloud UI to track the impact on Actions and Storage after optimizations are made
