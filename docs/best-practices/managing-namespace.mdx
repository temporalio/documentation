...
- [APS limits](/evaluate/cloud/limits#actions-per-second) and rate limiting
...
See [Multi-tenant application patterns](/best-practices/multi-tenant-patterns) for those designs.
...
Use an optional [Namespace description](/cloud/namespaces#describe-a-namespace) for a one-line purpose note: workload,
owner context, or why the Namespace exists.

Keep structured classification in [Tags](#tagging). Description is for humans reading the Namespace. Tags are for
filter, report, and automation dimensions such as `team` and `environment`.

Do not put secrets, confidential data, or PII in a description. Like Tags and [Search Attributes](/search-attribute),
descriptions are not encrypted.
