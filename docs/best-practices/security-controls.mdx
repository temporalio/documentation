...
Only Cloud UI activity that sends a request to Temporal Cloud resets the inactivity timer. Mouse movement, scrolling, or
reading information that has already loaded does not reset the timer.

The Cloud UI does not display a warning or countdown before the session expires.

The 12-hour maximum applies regardless of activity.
