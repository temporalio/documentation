...
[Create a support ticket](/evaluate/cloud/support#support-ticket) and provide your
...
To disable Strict UI Session Mode, [create a support ticket](/evaluate/cloud/support#support-ticket). Active sessions retain the
...
Track the expiration dates of your client and [Certificate Authority certificates](/cloud/certificates). Temporal Cloud trusts the uploaded CA; if it expires, all client authorizations will fail. Establish and automate a certificate rotation schedule (for example, rotate client certificates quarterly and CA certificates annually, well before expiry). Temporal supports uploading a new CA certificate alongside the old one, so rotation doesn't interrupt client connections. Always test new certificates in a staging environment if possible.
