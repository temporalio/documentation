...
Each section of a knowledge hub maps to the metrics above.
The following outline shows what to include, organized by the developer journey from evaluation through production
operations:

| Section | What it covers | Metrics it drives |
| :--- | :--- | :--- |
| **Temporal overview** | What Temporal is, why your organization chose it, and business value metrics. | Knowledge Hub traffic |
| **Decision framework** | Qualifying questions, good and bad use cases, and alternative recommendations. | Knowledge Hub traffic, support question rate |
| **Getting started** | 30-minute quickstart with environment setup, starter template, and first Workflow locally and on Temporal Cloud. | Time to first Workflow |
| **Learning paths** | Structured courses from foundational to advanced, tailored by persona. Links to [Temporal's free training](https://learn.temporal.io/). | Time to first Workflow, time to first Workflow in production |
| **Architecture and standards** | Namespace conventions, connectivity requirements, and Worker deployment standards. | Time to Workflow in production |
| **Cost guidance** | Billable Actions, storage tiers, and cost-saving tips. | Support question rate |
| **Shared responsibility** | Ownership matrix between Platform and Application teams across IAM, infrastructure, development, deployment, observability, and operations. | Support question rate |
| **Design patterns** | Curated Workflow patterns with descriptions and code sample links. | Time to Workflow in production, support question rate |
| **Troubleshooting and escalation** | Observability tools, runbooks for common issues, escalation path, SLAs, and example alert definitions. | Support question rate |
| **Support and FAQs** | Support tier, ticket submission, Temporal account contacts, expert-led session types, and frequently asked questions. | Support question rate |
