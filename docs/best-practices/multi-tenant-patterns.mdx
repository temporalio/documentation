...
This pattern uses [Task Queue Fairness](/develop/task-queue-priority-fairness#task-queue-fairness) to manage multi-tenant workloads within a single Task Queue. Each tenant is assigned a fairness key, and fairness weights control its share of Task dispatches when multiple tenants have backlogged Tasks.

You can also set per-fairness-key rate limits to cap an individual tenant's dispatch rate.
...
|---|---|---|---|---|
| **Isolation** | Task Queue level | Weighted dispatch | Activity-level only | Complete |
| **Noisy neighbor protection** | Strong | Dispatch-based | Activity-level | Full, with separate rate limits |
