...
Reports can be generated with hourly, daily and monthly granularities, each of which have their own data ranges.
...
Data from the current billing month is subject to change and only finalized once the billing month is closed.
...
| SKUID | A unique identifier that represents a specific SKU | business-actions |
