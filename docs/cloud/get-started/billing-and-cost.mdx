...
- Coming soon
    - Usage API: Systematically access usage information by Namespace down to hourly granularity, enriched with Tags, Projects, Action Categories and Action Types. 
    Use this API to identify workflow optimizations and track detailed usage patterns.
    - Billable Actions Metrics: Coming soon to the [OpenMetrics endpoint](/cloud/metrics/openmetrics), Billable Action Metrics will enable highly detailed analysis. 
    Going beyond the Usage API metrics will include labels for Namespaces, Workflow Types, Action Categories, Action Types down to the minute granularity.
