...
Note: a domain can span multiple accounts, but each individual email address belongs to one account only — users needing access to multiple accounts must use a different email address per account.
