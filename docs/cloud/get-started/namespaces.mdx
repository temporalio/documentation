...
<Tabs>
    ...
    <TabItem value="temporal-cloud" label="Temporal CLI">
...
<TabItem value="namespace-temporal-cloud" label="Temporal CLI">

See the [`temporal cloud namespace create`](/cli/command-reference/cloud/namespace#create) command reference for details.

</TabItem>

<TabItem value="namespace-cloud-ops" label="Cloud Operations API">
...
[pre-release](/evaluate/product-release-stages) and doesn't support
...
## How to add a Namespace description in Temporal Cloud {/* #describe-a-namespace */}
...
:::caution Do not include sensitive or confidential information

Do not include secrets, credentials, confidential data, or personally identifiable information (PII) in a Namespace
description.

Like [Tags](#tag-a-namespace) and [Search Attributes](/search-attribute), Namespace descriptions are not encrypted and
are not processed by a custom [Payload Codec](/payload-codec#payload-codec). Anyone with Namespace Read access can see
the description in the Cloud UI, CLI, and Cloud Ops API.

:::

### Limits

- Optional. Create and update succeed without it.
- At most 255 printable ASCII characters plus whitespace.
- Mutable, including clear-to-empty.
- Does not change Namespace runtime behavior.
- Stored on the Cloud control plane (`NamespaceSpec.description`). It is not the open source `NamespaceInfo.Description`
  field, and Temporal Cloud does not sync the two. Do not use `temporal operator namespace` to manage Cloud descriptions.
...
- Set or update: same as [Update Namespace](/cloud/manage-access/roles-and-permissions#namespace-level-permissions)
  (Account Owner, Account Admin, or Namespace Admin). Not restricted to Account Owner the way Tags are.
- View: anyone with Namespace Read access.

<Tabs>

<TabItem value="description-webui" label="Web UI">

On create, fill **Description** on the Create Namespace page.

On an existing Namespace, use **Add description** or the edit control on the Namespace overview header. Saving an empty
description clears it.

</TabItem>

<TabItem value="description-cloud-ops" label="Cloud Operations API">

Create: include `spec.description` on
...
copy the current `spec` and `resourceVersion`, change only `description`, then
[`UpdateNamespace`](https://saas-api.tmprl.cloud/docs/httpapi.html#tag/namespaces/POST/cloud/namespaces/%7Bnamespace%7D)
with the full spec. Omitting `description` on update clears it.

```
curl -sS -X POST "$BASE/cloud/namespaces/$NAMESPACE" \
  -H "$H_AUTH" -H "$H_VER" -H "Content-Type: application/json" \
  -d '{
    "namespace": "'"$NAMESPACE"'",
    ...
    "spec": {
      ...
      "retentionDays": 30,
      "apiKeyAuth": { "enabled": true },
      "description": "Elevate project migration from AWS to GCP"
    }
  }'
```
...
Requires Cloud Ops API version `v0.21.0` or later. See [Cloud Ops API](/ops).

</TabItem>

<TabItem value="description-cli" label="Temporal CLI">
