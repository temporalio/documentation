...
This availability objective applies only to the Cloud Ops API. It is not a contractual service-level agreement and does not change the Temporal Cloud [SLA](/evaluate/cloud/sla). Temporal Service operations, including Workflow and Activity execution, are covered separately by the Temporal Cloud SLA.
