...
description:
  Connect a SAML 2.0 identity provider to your Temporal Cloud account for single sign-on, included on all Temporal Cloud plans.
...
SAML is included on all Temporal Cloud plans. See the [pricing page](/cloud/pricing) for details.
...
- Provide a stable subject `NameID` that uniquely identifies each user.
- Include the user's email address and name as attributes.

The following example shows how to apply the same configuration to an additional identity provider. It does not
...
[Create a support ticket](/evaluate/cloud/support#support-ticket) that includes the following information:
