...
- Use a stable, non-transient subject `NameID`.
- Include attributes named `email` and `name`.
...
shown above. Configure the application with an RSA signing certificate, a stable subject `NameID`, the required `email`
and `name` attributes, and access for the appropriate users or groups. If users will launch Temporal Cloud from the
PingOne application portal, make the application visible there.
...
and Assertion Consumer Service URL shown above. Configure the connection with an RSA signing certificate, a stable
subject `NameID`, and the required `email` and `name` attributes.
