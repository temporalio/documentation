...
the Namespace-level, each access principal is assigned one or more Namespace-level permissions. Some account-level
roles, such as Account Owner and Global Admin, automatically have Namespace-level permissions on all Namespaces in the
account.
