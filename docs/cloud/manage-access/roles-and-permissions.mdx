...
The Metrics Read-Only role is intended for Service Accounts used by observability integrations. It grants access to the
[Temporal Cloud OpenMetrics endpoint](/cloud/metrics/openmetrics) but no Cloud Ops API or Namespace permissions. An
Account Owner or Global Admin must manage the Service Account and its API keys.
