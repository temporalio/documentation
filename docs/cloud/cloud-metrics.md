...
To view and manage third-party integration settings, your user account must have the Global Admin [Role](/cloud/account-setup/users#account-level-roles).
...
For more information, see [Set up Grafana with Temporal Cloud](#prometheus-and-grafana-setup).
...
- Root CA certificates and end-entity certificates. See [Certificate requirements](/cloud/account-setup/certificates#certificate-requirements) for details.
...
- [Global Admin privileges](/cloud/account-setup/users#account-level-roles) to the Temporal Cloud account.
- [CA certificate and key](/cloud/account-setup/certificates#) for the Observability integration. You will need the certificate to set up the Observability endpoint in Temporal Cloud.
...
3. Add your metrics queries:
   - For Temporal Cloud metrics, expand the **Metrics browser** and select the metrics you want to see.
     ...
     The documentation on [Cloud metrics](#available-performance-metrics) lists metrics emitted from Temporal Cloud.
