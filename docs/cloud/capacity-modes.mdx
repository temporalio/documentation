...
See [Throttling behavior](/evaluate/cloud/limits#throttling-behavior) for more details.
...
Each Namespace has an Actions per second (APS), Requests per second (RPS), and Operations per second (OPS) limit that scales automatically with usage. Your APS limit never falls below its [default limit](/evaluate/cloud/limits#actions-per-second). If Temporal Support has manually set your Namespace's limit, that value becomes your floor in place of the default, and it persists across capacity mode changes.
...
Your throughput limit will never fall below the [default limit](/evaluate/cloud/limits#actions-per-second) for your Namespace. 
...
* The [default limit](/evaluate/cloud/limits#actions-per-second)
* The lesser of:
...
If you need capacity beyond what is self-serviceable or available in a region, please [file a support ticket](/evaluate/cloud/support#ticketing) indicating the limit, region, and timeframe that the capacity is needed. 
...
For the metrics to watch and how to alert on utilization, see [Provisioned capacity utilization](/cloud/service-health#provisioned-capacity-utilization).
...
### Set capacity modes from the CLI
...
* `--capacity-mode provisioned` sets a fixed capacity allocation for the Namespace.
* `--capacity-value` sets the provisioned throughput value in TRUs (Temporal Resource Units).

To switch to on-demand capacity, omit `--capacity-value`:
