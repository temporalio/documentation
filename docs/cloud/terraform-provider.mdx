...
```bash
# Replace <your-secret-key> with the token output from `temporal cloud apikey create-for-me` or `tcld apikey create`.
...
```bash
temporal cloud namespace get -n "<your-namespace>.<your-account-id>"
```
...
```bash
temporal cloud nexus endpoint get --name "<your-nexus-endpoint-name-without-account-suffix>"
```
