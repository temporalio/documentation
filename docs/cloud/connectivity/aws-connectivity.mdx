...
flowchart LR
    ...
    subgraph HPRIM["<b>us-east-1</b> (active)"]
      HWA["<b>Worker(s)</b><br/>Server name:<br/>my-namespace.my-account.tmprl.cloud"]:::worker
      HVPA["<b>VPC Endpoint</b><br/>vpce-…-us-east-1"]:::endpoint
      HNSA["<b>Replica</b><br/>(Active)"]:::ns
      HWA --> HVPA
      HVPA -->|PrivateLink| HNSA
    end
    subgraph HSEC["<b>us-west-2</b> (passive)"]
      HWB["<b>Worker(s)</b><br/>Server name:<br/>my-namespace.my-account.tmprl.cloud"]:::worker
      HVPB["<b>VPC Endpoint</b><br/>vpce-…-us-west-2"]:::endpoint
      HNSB["<b>Replica</b><br/>(Passive)"]:::ns
      HWB --> HVPB
      HVPB -->|PrivateLink| HNSB
    end
