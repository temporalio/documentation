...
`UpdateNamespace` replaces the entire spec and requires the latest `resource_version`, so read the current Namespace first:
...
Then post the whole `NamespaceSpec` back, appending `$CR_ID` to `connectivityRuleIds`:
...
- Do not omit fields from `spec` in Step 2. Proto3 update is full-replace — any field left out will be cleared. Building the body from the GET response avoids surprises.
...
Then post the whole `NamespaceSpec` back, appending `$CR_ID` to `connectivity_rule_ids`:
...
- Do not omit fields from `spec` in Step 2. Proto3 update is full-replace — any field left out will be cleared. Building the body from the `GetNamespace` response avoids surprises.
...
If an unforeseen event requires Temporal to add or remove a published range, Temporal gives every Stable IPs user six months of advance notice, through both targeted communication and the [Temporal Cloud changelog](https://temporal.io/change-log).

Temporal never routes traffic to IP addresses that are not on the published list.
