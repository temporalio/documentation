...
:::warning Verify that Private Hosted Zones do not block failover routing

If you use Route 53 Private Hosted Zones (PHZs) for PrivateLink, ensure that your PHZs only override DNS for regions where you have a PrivateLink endpoint configured.
A broad PHZ (for example, one that covers `<account>.tmprl.cloud`) can intercept the Namespace Endpoint DNS resolution and prevent Workers from following the CNAME to the failover region.

If your Workers cannot connect after a failover, check whether a PHZ is overriding DNS for the failover region and pointing it to a local address that doesn't exist.
