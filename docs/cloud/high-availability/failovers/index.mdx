...
- **Operations that had not yet replicated at the moment of failover.** A Workflow start, Signal, Update, or other
  ...
  Namespace according to the Conflict Resolution process.
    - Note that Conflict Resolution can only recover data from a functioning Temporal Server. If the active server never
recovers, the Workflow API calls that fall within the 1 minute RPO may be permanently lost. Such a case would require
