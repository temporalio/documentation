...
After a Temporal-managed failover, Temporal automatically fails back to the original region once the region is healthy. After a user-triggered failover, the Namespace stays in the replica region until a user triggers another failover. See [failback options](/cloud/high-availability/failovers/manage#failbacks) for details on managing failback behavior.
