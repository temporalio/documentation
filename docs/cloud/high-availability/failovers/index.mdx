...
non-graceful failover, replication lag causes  temporary setback in Workflow progress. At the moment of non-graceful failover:

- Operations that had already replicated remain durable in the replica.
- Operations that had not yet replicated (i.e., that are still in the replication backlog) are reconciled when the region recovers.
...
In a graceful failover, Temporal Cloud drains the replication backlog to zero and pauses traffic before
