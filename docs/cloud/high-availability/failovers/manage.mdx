...
The `<target_region>` must be the ID of a region (example: `aws-us-east-1`) where the Namespace has a replica that is
...
- `region` (required): The [region code](/evaluate/cloud/regions) of the region to failover to. Must be a region where the
