...
but the compute provider configuration (for example, a Lambda ARN) is scoped to a single region. After a failover, the
Worker Controller Instance keeps invoking the original region's Worker instead of one in the new active region. See
