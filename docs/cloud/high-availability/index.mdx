...
Multi-region Replication and Multi-cloud Replication are in [General Availability](/evaluate/development-production-features/release-stages#general-availability).

Same-region Replication is in [Public Preview](/evaluate/development-production-features/release-stages#public-preview).
...
For Multi-region and Multi-cloud Replication, the replica region must be on the same continent as the active region.
Not every region supports every replication type.
See [Regions](/cloud/regions) for the supported replica regions for each active region.
