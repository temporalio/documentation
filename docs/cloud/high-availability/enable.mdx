...
To deploy Worker fleets in both regions that stay idle in the passive region until failover, see [Active / Hot](/cloud/high-availability/deployment-models#active-hot).
...
temporal cloud namespace ha update \
    ...
    --passive-poller-forwarding disable
```

Set the flag to `enable` to re-enable forwarding.
