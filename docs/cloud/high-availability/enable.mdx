...
Not all replication options are available in all regions. For Multi-region and Multi-cloud Replication, the replica region must be on the same continent as the active region. See the [region documentation](/cloud/regions) for the supported replica regions for each active region.
