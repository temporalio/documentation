...
Executions.
 ...
 The replica region must be on the same continent as the primary region. Because of that, not all replication options are available in all Temporal Cloud regions. See the [region documentation](/cloud/regions) for the supported replica regions for each active region.
