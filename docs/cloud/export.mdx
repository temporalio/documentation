...
description: Workflow History Export in Temporal Cloud lets users export Closed Event Histories to an object storage for compliance and analytics. Configure via Cloud UI or tcld.
slug: /cloud/export
...
Workflow History Export allows users to export closed Event Histories from Temporal Cloud to cloud object storage (AWS S3 or GCP GCS), enabling:

- Compliance and audit trails of complete Event History data in [proto format](https://github.com/temporalio/api/blob/master/temporal/api/export/v1/message.proto)
