...
* mTLS certificate
* cluster configuration json output for each cluster
* type of visibility store for each cluster
* customer search attributes for each cluster/namespace
* metrics for each namespace

Use the following template for reporting cluster/namespace mappings.

```yaml
customer:
  account_id: "temporal cloud account id"

clusters:
  - cluster_name: "cluster1"
    namespaces:
      - source_namespace: "ns1"
        cloud_namespace: "ns1.account"
        cloud_region: "changeme"
      - source_namespace: "ns2"
        cloud_namespace: "ns2.account"
        cloud_region: "changeme"  
