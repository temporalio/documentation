...
2. For server versions 1.22.6 - 1.23.x, apply this [extra required setting](#special-dynamic-configuration-for-version-1226---123x).
...
One the self-hosted setup has been verified, the next step will be to perform test migrations.
...
### Special dynamic configuration for Version 1.22.6 - 1.23.x

Temporal versions 1.22.6 and 1.23.x include support for stream-based replication, but it is disabled by default. Since
those releases, stream-based replication has been validated as more reliable than the poll-based replication that
remained the default in 1.22.6 and 1.23.
