...
1. Risk of change - No matter what, clients must be updated to use the new cloud-side Namespace. The big difference is that
   automated migration enables replication between self-hosted and cloud during the migration process. This replication allows some flexibility
   in transferring clients (if you miss a few clients, replication will provide some cover).If you are unsure of the impact of switching clients
   during a manual migration, then you may opt for the automated approach.  
...
4. Closed Workflows - Automated migration will migrated closed Workflows where manual migration will not.
...
[manually migrate](/cloud/migrate/manual) any workflows into their new cloud-side target Namespaces prior to starting an auto-migration.
