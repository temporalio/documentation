...
Specify the name of a [Signal](/encyclopedia/application-message-passing#signals). This modifier is required when `--batch_type` is `signal`.
...
Pass input for the [Signal](/encyclopedia/application-message-passing#signals). Input must be in JSON format.
