...
Specify the name of a [Signal](/encyclopedia/sending-messages#sending-signals). This modifier is required when `--batch_type` is `signal`.
...
Pass input for the [Signal](/encyclopedia/sending-messages#sending-signals). Input must be in JSON format.
