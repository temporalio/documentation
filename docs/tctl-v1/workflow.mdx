...
description: Explore the wide range of tctl Workflow commands for efficient management of Workflow Executions in a Temporal Service.
...
The `--query` flag is supported only when [Advanced Visibility](/visibility#advanced-visibility) is configured with the Temporal Service.
...
Specify the [Start-To-Close Timeout](/activities#start-to-close-timeout) of the [Workflow Task](/workers#workflow-task) in seconds.
