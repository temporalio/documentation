...
sidebar_label: admin
description: Explore the wide range of tctl admin commands that let you run admin operations on a Temporal Service.
...
The `tctl admin cluster` command runs the administrator-level operations on a given Temporal Service.
...
The `tctl admin cluster add-search-attributes` command allows Search Attributes to be added to a Temporal Service.
Custom Search Attributes can be used to make a Temporal Service more identifiable.
...
Use this command to add custom Search Attributes to your Temporal Service:
...
:::note
If you are adding custom Search Attributes to a Temporal Service running from the `docker-compose-es.yml` file in the [temporalio/docker-compose](https://github.com/temporalio/docker-compose) repo, make sure to increase the Docker memory to more than 6 GB.
:::
...
The `tctl admin cluster describe` command provides information for the current Temporal Service.

The following modifier changes the behavior of the command:

#### --cluster_value

The name of the remote Temporal Service within the current Temporal Service.

This modifier is optional, and can default to the return of current Temporal Service information.

### get_search_attributes

The `tctl admin cluster get_search_attributes` command retrieves existing Search Attributes for a given Temporal Service.
...
The `tctl admin cluster list` command lists Temporal Service information on the given Temporal Service.
...
The `tctl admin cluster remove_remote_cluster` command removes remote Temporal Service information on the given Temporal Service.

The modifier below changes the behavior of the operation:

#### --cluster

The name of the remote Temporal Service to remove.

### remove_search_attributes

> The Temporal tctl documentation covers version 1.17 of the Temporal CLI.

The `tctl admin cluster remove-search-attributes` command removes custom Search Attribute metadata from a Temporal Service.
This operation has no effect on Elasticsearch index schema.

Use the following command to remove a [Search Attribute](/visibility#search-attribute) from a Temporal Service's metadata:

```bash
tctl admin cluster remove-search-attributes --name <SearchAttributeKey>
```

Only custom Search Attributes can be removed from a Temporal Service's metadata.
Default Search Attributes cannot be removed.

Removing a Search Attribute removes it from the Temporal Service's metadata but does not remove it from the Elasticsearch index.
...
The `tctl admin cluster upsert_remote_cluster` command adds or updates remote Temporal Service information in the current Temporal Service.

#### --frontend_address

The remote Temporal Service frontend address.

#### --enable_connection

Enables remote Temporal Service connection.
...
The total amount of shards for the Temporal Service.
...
The `tctl admin membership list_db` command lists the Temporal Service items in a targeted membership.
...
ShardId managed by the Temporal Service.
...
Temporal Service for the shard to use.
...
The `tctl admin workflow refresh_tasks` command updates all [Tasks](/workers#task) in a [Workflow](/workflows), provided that the command can fetch new information for Tasks.
