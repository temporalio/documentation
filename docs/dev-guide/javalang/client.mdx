...
This protocol supports the two-way communication needed to fully orchestrate Temporal applications.
...
Your code doesn't use gRPC directly.
...
A Worker Factory uses a client to build a Worker, as Workers must be able to communicate with the Temporal Service.
In the following example, the new Worker listens to 'YourTaskQueue' and can manage 'YourWorkflow' tasks (implemented in 'YourWorkflowImpl').
You should also configure your Workers to manage Activity tasks.
...
Worker code is normally invoked from a 'main' method, where the JVM starts program execution.
Starting the factory will block.
This enables the Worker application to continue running as it polls for new tasks on its Task Queue.
