...
- [Choose a development Temporal Service](/dev-guide/java/project-setup#choose-dev-temporal-service): Discover which development Temporal Service you should choose
...
- [How to create a Temporal Client in Java](/dev-guide/java/foundations#connect-to-a-dev-temporal-service): To initialize a Workflow Client, create an instance of a `WorkflowClient`, create a client-side `WorkflowStub`, and then call a Workflow method (annotated with the `@WorkflowMethod` annotation).
- [How to connect to Temporal Cloud](/dev-guide/java/foundations#connect-to-temporal-cloud): Use a compatible mTLS CA certificate and mTLS private key and your Cloud Namespace to connect to Temporal Cloud.
...
- [How to Heartbeat an Activity](/dev-guide/java/features#activity-heartbeats): An Activity Heartbeat is a ping from the Worker that is executing the Activity to the Temporal Service.
...
- [How to use Visibility APIs](/dev-guide/java/observability#visibility): The term Visibility, within the Temporal Platform, refers to the subsystems and APIs that enable an operator to view Workflow Executions that currently exist within a Temporal Service.
