...
If Webpack fails to create the bundle, the SDK will throw an error and emit webpack logs using the SDK's [logger](/dev-guide/typescript/observability#logging).
...
Or, if you are seeing stripped-down Workflow types in the Temporal Service's Web UI like this:
...
It can be due to your bundler stripping out Workflow function names.
Temporal relies on those names to set the "Workflow Type" in the Service Web UI.
To prevent the build process from removing Workflow function names,  modify the webpack configuration file ( `webpack.config.js`) by setting `keep_fnames` to `true` in the `TerserPlugin` configuration section:
...
See [esbuild docs](https://esbuild.github.io/api/#keep-names) for more information.
</TabItem>
</Tabs>
