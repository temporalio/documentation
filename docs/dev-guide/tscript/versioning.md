...
The Temporal Platform requires that Workflow code is [deterministic](/workflows#deterministic-constraints) in nature.
...
Patching APIs let you change Workflow Definitions without causing [non-deterministic behavior](/workflows#non-deterministic-change) in current long-running Workflows.
