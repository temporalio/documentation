...
description: Explore using Signals in Temporal with the TypeScript SDK to send messages to Workflows, with details on defining, sending, and handling Signals, including customization options.
...
* [Develop with Queries](#what-are-queries)
* [Defining Signals and Queries statically and dynamically](#static-and-dynamic-signals-and-queries)
...
<!--Jwahir suggests updating this for better fluency. Patrick reminds: this is reference material, why can be surfaced somewhere else. The engineers will maintain. Why content from Angela in Slack. When adding net new content, the review from Engineers will be substantial.-->
Two steps enable you to define and handle your Signal so it works with your Workflow code.
...
See also: [Common problems with Signals and their Workarounds](https://github.com/temporalio/edu-interacting-with-workflows-typescript-content/blob/main/signaling-your-workflows/common-issues-with-signals.md)
...
There can be only one handler for any Signal type; calling `setHandler` multiple times for the same Signal type overwrites the existing handler.
...
<!--come back and review this and reword-->
The [`WorkflowHandle.signal`](https://typescript.temporal.io/api/interfaces/client.WorkflowHandle#signal) method synchronously awaits the signal sending?
...
<!--Reword for we voice-->
Another reason we use the getExternalWorkflowHandle method is to ensure that Workflows remain deterministic.
...
Temporal provides a solution for this.
...
<!--In the reference guide, reference any primitives (for example, the encyclopedia). Try to add links to each of these terms in this (long list)-->
...
## Define Signals and Queries statically or dynamically {#static-and-dynamic-signals-and-queries}
