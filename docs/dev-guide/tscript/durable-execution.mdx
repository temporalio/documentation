...
1. **[Intrinsic non-deterministic logic](/dev-guide/typescript/durable-execution#intrinsic-non-deterministic-logic):** This occurs when Workflow state or branching logic within the Workflow gets determined by factors beyond the SDK's control.
2. **[Non-deterministic code changes](/dev-guide/typescript/durable-execution#durability-through-replays):** When you change your Workflow code and deploy those changes while there are still active Workflow Executions relying on older code versions.
...
<!--
[Start a new Workflow](/dev-guide/go/project-setup#start-workflow) and after it is complete [download the Event History as a JSON object](/dev-guide/typescript/durable-execution#retrieve-event-history).
-->
