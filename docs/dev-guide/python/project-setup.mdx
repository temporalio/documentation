...
This section covers how to use a terminal, a code editor, and a development Temporal Service to create a Namespace, write a single Activity Workflow, run a Worker that talks to your development Temporal Service, run a Workflow using the CLI, add a testing framework, and view Workflows in the Web UI.
...
- [Use a self-hosted environment such as Docker](#self-hosted-temporal-temporal-service)

Read more in the [Choose a development Temporal Service](#choose-dev-temporal-service) section on this page.
...
## Choose a development Temporal Service {#choose-dev-temporal-service}

**Which development Temporal Service should you choose?**

We recommend choosing a development environment based on your requirements.

The source code for the Temporal Server (the orchestrating component of the Temporal Service) is licensed under the MIT open source license. So, in theory, anyone can take the Temporal Server code and run their Temporal Platform in any number of creative ways.
...
- [Self-hosted Temporal Service](#self-hosted-temporal-temporal-service)
...
If you are eager to to set up Temporal Service-level metrics for performance tuning, we recommend using a self-hosted Temporal Service or Temporal Cloud.
...
In other words, Temporal Cloud is perfect if you are ready to run at scale and don’t want the overhead of managing your own self-hosted Temporal Service.
...
For example, running a self-hosted Temporal Service lets you try different databases, view Temporal Service metrics, use custom [Search Attributes](/visibility#search-attribute), and even play with the [Archival](/clusters#archival) feature.
...
Now run the Worker on the same network as the Temporal Service containers using the following command:
...
**How to start a Workflow with the Temporal CLI when using a Self-hosted Temporal Service**
