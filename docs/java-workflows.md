...
Again, it is important to note that this code directly implements the business logic, and if any of the invoked operations (aka [Activities](/docs/java-activities)) take a long time, the code is not going to change.
...
Though, unlike [Activities](/docs/java-activities), Workflow timeouts are available primarily to protect the system from "runaway" Workflows that may end up consuming too many resources, and not intended to be used as a part of the business logic.
...
[Activities](/docs/java-activities) are made available to handle that kind of logic, and thus retrying Workflows is rare.
...
The only required Workflow options parameter is the name of a [Task Queue](/docs/java-task-queues).
Read the [Task Queues concept page](/docs/java-task-queues) for a better overview.
...
If you are using [Signals](/docs/java-signals) with the Go SDK, you should make sure to do an asynchronous drain on the Signal channel or the Signals will be lost.
