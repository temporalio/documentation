...
- [How to set up a Codec Server](#codec-server)

The Web UI and tctl both use a default [Data Converter](#data-converter), which is capable of serialization only.
...
Make sure to [enter a valid URL and port](#codec-server) for the codec endpoint when configuring the Codec Server.
