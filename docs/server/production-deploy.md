...
While a lot of effort has been made to easily run and test the Temporal Server in a development environment (see the [Quick install guide](/application-development/foundations#run-a-dev-cluster)), there is far less of an established framework for deploying Temporal to a live (production) environment.
...
See the [Cluster concept guide](/clusters) for more details.
...
- **Internal services**. The next layer will be scaling the 4 internal services of Temporal ([Frontend, Matching, History, and Worker](/clusters#temporal-server)).
