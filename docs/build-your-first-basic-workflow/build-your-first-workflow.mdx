...
Before you begin, setup your local development environment:

<CallToAction 
  href="/quickstarts"
  buttonText="Quickstart Guide"
  description="Run through the Quickstart to get your setup complete."
/>
...
1. **A Workflow** is a function or method that defines the overall flow of your application. It's written in your programming language of choice and supported by your chosen SDK. 
...
2. **Send a Workflow Execution request** to the Temporal Service: The Temporal Service accepts your request but does not run the code. The Workers communicate with the Temporal Service, accept tasks related to that request, and then fulfill them by running the code. The results are reported back to the Temporal Service, which tracks the progress of the Workflow Execution.
...
Mismatched Task Queue names don't result in an error, but the Worker won't receive any tasks so the Workflow Execution will not progress.
