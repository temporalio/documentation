...
import { MissionStatus } from "@site/src/components/MissionStatus";
...
<MissionStatus items={[
  { id: 'crash-test', number: '1', label: 'Crash Recovery Test', status: 'pending' },
  { id: 'bug-fixing', number: '2', label: 'Live Bug Fixing', status: 'pending' },
  { id: 'advanced', number: '3', label: 'Advanced Missions', status: 'pending' }
]} />
...
<TemporalProgress steps={[
  { id: 'crash-recovery', label: 'Crash Recovery', status: 'active' },
  { id: 'bug-fixing', label: 'Live Bug Fixing', status: 'pending' }
]} />
...
**Advanced Experiment**: Try killing the Worker at different points during execution. Start the Workflow, kill the Worker during the withdrawal, then restart it. Kill it during the deposit. Each time, notice how Temporal maintains perfect state consistency.

**Bonus points**: Check the Web UI while the Worker is down - you'll see the Workflow is still "Running" even though no code is executing.
:::

## Experiment 2 of 2: Live Bug Fixing

<TemporalProgress steps={[
  { id: 'crash-recovery', label: 'Crash Recovery', status: 'completed' },
  { id: 'bug-fixing', label: 'Live Bug Fixing', status: 'active' }
]} />

**The Challenge**: Inject a bug into your production code, watch Temporal retry automatically, then fix the bug while the Workflow is still running. This is every developer's dream scenario.
...
## Mission Debrief: What You Just Accomplished

**Congratulations!** You've experienced firsthand why Temporal is a game-changer for reliable applications. Here's what you proved:
...
<TemporalProgress steps={[
  { id: 'crash-recovery', label: 'Crash Recovery', status: 'completed' },
  { id: 'bug-fixing', label: 'Live Bug Fixing', status: 'completed' },
  { id: 'mission-complete', label: 'Mission Complete!', status: 'completed' }
]} />

## Advanced Training Missions

Ready for more chaos engineering? Try these advanced scenarios:
