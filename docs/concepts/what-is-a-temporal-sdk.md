...
The Temporal SDKs emit a set of metrics, mostly from Temporal Client usage and Worker Processes.
...
Time it took to complete activity execution, from the time core generated the activity task, to the time lang responded with a completion (failure or success).
...
Total latency of successfully finished local activities from schedule to completion
...
Number of [Local Activity Executions](/docs/concepts/what-is-a-local-activity).
...
A workflow task did not find a cached WorkflowWorker
...
Current cache size in number of workflows
...
Count of workflows forced from the cache intentionally
...
Count of number of times a request to invoke an activity that is not registered with the worker
...
A worker was registered/created/started
