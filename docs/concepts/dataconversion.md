...
A Codec Server is an HTTP/HTTPS server that uses a [custom Payload Codec](/self-hosted-guide/data-encryption#) to decode your data remotely through endpoints.
...
When you apply custom encoding with encryption or compression on your Workflow data, it is stored in the encrypted/compressed format on the Temporal Server. For details on what data is encoded, see [Securing your data](/self-hosted-guide/data-encryption#).
