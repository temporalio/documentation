...
Temporal [Multi-Cluster Replication](/concepts/what-is-multi-cluster-replication) relies on asynchronous replication of Events across Clusters, so in the case of a failover it is possible to have an Activity Task dispatched again to the newly active Cluster due to a replication task lag.
