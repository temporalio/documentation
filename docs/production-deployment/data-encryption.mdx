...
To query your Codec Server from the Temporal Web UI, enable Cross-Origin Resource Sharing (CORS) on your Codec Server.

Enable the following CORS headers to allow requests from the Temporal Web UI:

- `Access-Control-Allow-Origin`
- `Access-Control-Allow-Methods`
- `Access-Control-Allow-Headers`

For example, for Temporal Cloud Web UI hosted at https://cloud.temporal.io, configure the following:
...
The following example shows how to configure CORS in a TypeScript Codec Server using Express and the `cors` middleware package:
...
async function main({ port = 8888 }) {
  ...
  app.use(cors({ 
    allowedHeaders: ['x-namespace', 'content-type', 'authorization'] 
  }));
  
  // Express application setup
  app.use(express.json());
  
  // Define your /encode and /decode endpoints here
  // ...
...
Configure CORS early in your application setup, before defining any routes. This ensures all incoming requests are checked against your CORS policy before any route handlers execute.

The `allowedHeaders` option specifies which HTTP headers can be included in requests to your server. For Codec Servers, include:

- `x-namespace`: Custom header sent by Temporal Web UI to identify the Namespace
- `content-type`: Standard header for JSON requests
- `authorization`: Required if your Codec Server implements authentication
...
Restrict access to your Codec Server because it can decode potentially sensitive data with a single API call.

##### Localhost and ingress restriction

Depending on your infrastructure and risk levels, restricting HTTP ingress to your Codec Server may be sufficient, such as by using a VPN like [WireGuard](https://www.wireguard.com/) or deploying it on a private network.

The Temporal Web UI can communicate with a Codec Server that is only accessible on `localhost`, so this is a legitimate security pattern. This works well when users access the Web UI and Codec Server from the same machine or within the same secure network.

However, if your Codec Server is exposed to the internet at all, you need an authentication solution.

##### Implementing authentication

When implementing authentication, consider user management. If multiple users need to interact with your Codec Server to decode packages, do not share the same authentication route. Even with token-based authentication, you may need to distinguish between users for audit purposes.

If you already use an organization-wide authentication provider, integrate it with your Codec Server. A Codec Server is a standalone HTTP server, so you can use existing libraries and protocols for authentication, including:

- **OAuth 2.0-based identity services**: Auth0, Okta, Google Auth, Microsoft AD, Amazon Cognito
- **JWT (JSON Web Tokens)**: Temporal Cloud uses JWTs for authorization
- **Custom authentication**: Implement your own authentication logic based on your organization's requirements
