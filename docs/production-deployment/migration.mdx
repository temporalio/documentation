...
description: Migrate from a self-hosted Temporal Cluster to Temporal Cloud with guidance on migration requirements, steps, and considerations for Temporal Client connection code, Workflow Executions, and more.
...
Both a [Signal](/workflows#signal) and a [Query](/workflows#query) will be executed during the course of the migration.
...
- Consider the implications for [security and access to your cluster](/cloud/security).
