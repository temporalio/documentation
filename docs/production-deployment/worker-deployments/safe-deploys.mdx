...
You'll need to add a few additional configuration parameters to your Workers to toggle on Safe Deploys.
There are three new parameters, with different names depending on the language:

- `UseVersioning`: This enables the Safe Deploys functionality for this Worker.
...
This requires the [Temporal Worker Controller](/production-deployment/worker-deployments/kubernetes-controller).
