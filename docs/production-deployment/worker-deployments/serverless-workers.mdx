...
- A Temporal Service (Temporal Cloud or self-hosted with the worker controller component)
...
This requires an IAM role in your AWS account that Temporal can assume.
...
aws iam create-role \
  ...
  --assume-role-policy-document '{
    ...
    "Statement": [{
      "Effect": "Allow",
      "Principal": {"AWS": "<TEMPORAL_ROLE_ARN>"},
      "Action": "sts:AssumeRole",
      "Condition": {
        "StringEquals": {
...
| Value | Description |
|---|---|
| `TEMPORAL_ROLE_ARN` | The Temporal Service role that assumes your invocation role. For Temporal Cloud, this is provided in your Namespace configuration. For self-hosted, this is the IAM identity running the worker controller. |
| `YOUR_EXTERNAL_ID` | A unique identifier that Temporal presents when assuming the role. For Temporal Cloud, this is provided in your Namespace configuration. |
...
When the task lands on the Task Queue with no active pollers, Temporal invokes your Lambda function.
The Worker starts, picks up the task, and processes it.
...
- **AWS CloudWatch Logs:** The Lambda function's log group (`/aws/lambda/my-temporal-worker`) should show invocation logs.
