...
Worker Versioning provides the following benefits for [blue-green or rainbow deployments](#deployment-systems):
...
- Using **Workflow Pinning**, you can declare each Workflow type to have a **Versioning Behavior**, either Pinned or
  ...
  - An **Auto-Upgrade** Workflow will automatically move to a new code version as you roll it out, specifically its
    ...
    Deployment Version and need to be kept replay-safe manually, that is with
...
:::caution Before you enable Worker Versioning

Worker Versioning has [limits](/evaluate/cloud/limits#worker-versioning-level) on the number of deployments, versions, and Task Queues. Review those limits before enabling versioning.

In particular, do not enable Worker Versioning for worker-specific Task Queues — versioning is not relevant for such queues, and it will unnecessarily exhaust the [per-version Task Queue limit](/evaluate/cloud/limits#max-task-queues-in-deployment-version-limits).
