...
There are various ways in which one run of a workflow can start other workflow runs.
The following section names each of those scenarios and describes how versioning behavior and/or version are inherited.

A workflow can start a new run by:
...
- Being [Reset](https://docs.temporal.io/workflow-execution/event#reset)
...
When an AutoUpgrade workflow starts a new run, the version is never inherited.

When a Pinned Parent workflow starts a child, the child will inherit the parent's version. So the first workflow task of
the child will execute in the same version as its parent. If the child is also Pinned, the user can enjoy the guarantee
that the parent-child tree will run in the same version. If the child workflow is annotated as Auto-Upgrade, the workflow's
behavior will change to Auto-Upgrade after the first task is completed.

If the child's task queue is not in the same Worker Deployment as the parent, then the child will not inherit, and it will
start on the Current Version of its task queue.

When an Auto-Upgrade Parent workflow starts a child, the child will inherit no initial versioning behavior, and will start
on the current version of its Worker Deployment like all other new workflow executions.

---

When a Pinned workflow Continues-As-New, the Pinned version is inherited across the Continue-As-New chain.

---
Versioning Override:
    // Children, crons, retries, and continue-as-new will inherit source run's override if the override is pinned
    // and if the new workflow's Task Queue belongs to the override version.

Non-Override Pinned Behavior and Version:
