...
If you need more control over when you sunset workers and builds from one version to another, you should consider using Workflow Pinning. The Pinned Workflow Type doesn't need to be patched as they run on the same worker and build until all of the tasks are complete. Otherwise, if your Worker and Workflows are new, we suggest not providing a `DefaultVersioningBehavior`.
