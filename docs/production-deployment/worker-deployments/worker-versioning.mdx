...
Versions. Refer to the [Worker Versioning Activity behavior docs](/worker-versioning#actvity-behavior-across-versions)
for more details.
...
:::note When you change the behavior to Auto-Upgrade, the Workflow will resume work on the Workflow's Target Version. So
if the Workflow's Target Version is different from the earlier Pinned Version, you should make sure you
...
Handle different reasons differently. For version upgrades, use the `AUTO_UPGRADE` versioning behavior option. For
history size reasons, you may want to continue without upgrading to stay on the same version.
