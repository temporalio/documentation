...
There may be times when you need to migrate your Workflow from Pinned to Auto-Upgrade because you support rainbow deployments now or there has been another update and you no longer want a specific Workflow using the pinned behavior.

There are a few steps you need to take in order to make that change. Since Auto-Upgrade Workflows need to be replay-safe, you need to [patch](/workflow-definition#workflow-patching) your Workflow code to ensure it supports multiple versions.

Next, you need to update the versioning behavior for your Workflow from Pinned to Auto-Upgrade. You can use `temporal workflow update-options` for this.
...
If there are still Tasks running in the Workflow, you should consider waiting for them to drain from the Workflow before doing this migration to avoid version issues.
