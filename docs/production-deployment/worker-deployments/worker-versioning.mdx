...
func (w *Workflows) ContinueAsNewWithVersionUpgradeV1(
  ...
  for {
    // Trigger a WFT when timer expires to refresh the continue-as-new-suggested flag.
...
- **Lazy moving only:** Workflows must be invoked by executing a step to receive the Continue-as-New suggestion. Sleeping Workflows
  won't be proactively get the Continue-as-New suggested information (planned for a future release).
