...
Pinned Workflows get stuck on versions and Workflows prevent versions from draining, long-running Workflows could be preventing drainage. You could move the Workflow to a new build, but that would just push the problem to the next build.
...
If you want to move all of your Workflows of a certain type to this new configuration, you can do it with this command:
...
:::note
[Patching](/patching#patching) will be necessary going forward once you move to Auto-Upgrade behavior, but you don't need to patch the code before you switch from Pinned behavior. You will eventually need to patch your newly Auto-Upgrade Workflows since they need to be replay-safe to supports multiple versions after the migration.
:::
