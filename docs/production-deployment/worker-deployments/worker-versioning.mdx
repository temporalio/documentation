...
:::note
When you change the behavior to Auto-Upgrade, the Workflows will resume work on the Workflow's Target Version. So if the Workflow's Target Version is different from the previous Pinned version of the Workflows, you should make sure you [patch](/patching#patching) the Workflows. Patching ensures your Workflows are replay-safe to support multiple versions after the migration. Otherwise, the Workflow will can get stuck on the Workflow's Target Version.
:::
