...
- Every Workflow has to declare a [versioning behavior](/worker-versioning#versioning-behaviors), or the Worker needs to set a
...
This writes the following profile to your Temporal CLI configuration file, which you can also edit directly in the TOML:
...
[Artifact Registry](https://cloud.google.com/artifact-registry/docs). The Worker should:
...
For guidance on writing a Worker and setting these options, see the Worker documentation for
your specific SDK.
...
manifest below. Set the manual instance count to `1`. Temporal scales the pool up when Tasks arrive and back to zero when
the invocation window ends.

Save the following manifest as `worker-pool-spec.yaml` and replace the values marked with `# replace`:
...
metadata:
  ...
  name: <your-worker-pool-name> # replace
  namespace: '<your-gcp-project-number>' # replace
spec:
  instanceSplits:
    - latestRevision: true
      percent: 100
  template:
    ...
    spec:
      ...
      serviceAccountName: <your-invocation-service-account>@<your-gcp-project>.iam.gserviceaccount.com # replace
      containers:
        - image: <your-worker-image> # replace
          name: worker-1
          args:
            ...
            - --deployment-name=<your-worker-deployment-name> # replace
            - --deployment-build-id=1.0
            - --run-id=<your-run-id> # replace
          resources:
            limits:
              cpu: '2'
              memory: 512Mi
...
yq -o=json worker-pool-spec.yaml > "$POOL.json"
...
deployment name your Worker registers with in the earlier [Deploy the Worker to a Cloud Run worker pool](#deploy-worker-pool) section.
