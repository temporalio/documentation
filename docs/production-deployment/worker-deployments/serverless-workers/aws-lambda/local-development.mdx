...
description:
  ...
  with no AWS account.
slug: /production-deployment/worker-deployments/serverless-workers/aws-lambda/local-development
...
the same way you would for [self-hosted setup](/production-deployment/worker-deployments/serverless-workers/aws-lambda/self-hosted-setup#enable-worker-controller).
...
[Enable the Worker Controller](/production-deployment/worker-deployments/serverless-workers/aws-lambda/self-hosted-setup#enable-worker-controller)
...
steps, but use the LocalStack function ARN and pass `--aws-lambda-skip-role-and-external-id`. The CLI otherwise
requires an assume-role ARN and External ID whenever you set a function ARN; this flag skips that client-side check and
defers to the server, which accepts an ARN-only configuration because you started it with
`require_role_and_external_id=false`. Don't pass the assume-role flags together with it — the CLI rejects that
combination.
...
- To configure a self-hosted Temporal Service to invoke Lambda functions in AWS, see
  [Self-hosted setup](/production-deployment/worker-deployments/serverless-workers/aws-lambda/self-hosted-setup).
- If a Workflow does not progress or the Lambda is not invoked, see
  [Troubleshoot Serverless Workers](/troubleshooting/serverless-workers/aws-lambda).
...
| `create-version` fails with `missing required AWS Lambda provider detail: role`. | Add `--aws-lambda-skip-role-and-external-id`. The CLI requires an assume-role ARN and External ID whenever you set a function ARN unless this flag is present, and rejects passing both. |
