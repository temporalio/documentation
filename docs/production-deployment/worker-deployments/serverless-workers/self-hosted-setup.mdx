...
|---|---|
| `TemporalIamRoleArn` | The ARN of the IAM role or user that the Temporal Service runs as. This is the identity the server uses to call `sts:AssumeRole`. |
...
The stack output `RoleARN` contains the IAM role ARN to use when creating the Worker Deployment Version.
