...
Install dependencies into a local directory for packaging:

```bash
pip install --target ./package temporalio
```

Copy your application code into the package directory:

```bash
cp lambda_function.py my_workflows.py my_activities.py ./package/
```
