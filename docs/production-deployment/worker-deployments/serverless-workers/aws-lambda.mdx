...
your AWS account that grants `lambda:InvokeFunction` and `lambda:GetFunction` on your Worker Lambda functions. The trust
policy on the role includes an External ID condition to prevent
