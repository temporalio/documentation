...
Build a fat JAR (uber-jar) with all dependencies using your build tool (Gradle or Maven). For example, with Gradle's
Shadow plugin:
...
Package the JAR into a zip file:

```bash
cp build/libs/my-worker-all.jar function.jar
zip function.zip function.jar
```
...
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `--function-name` | Name of the Lambda function.                                                                                                   |
| `--runtime`       | Lambda runtime. Use `java21` or another supported Java version.                                                                |
| `--handler`       | Fully qualified class name of your `RequestHandler` implementation.                                                            |
| `--memory-size`   | Java Workers typically need more memory than other runtimes. Start with `512` and adjust based on CloudWatch memory metrics.    |
