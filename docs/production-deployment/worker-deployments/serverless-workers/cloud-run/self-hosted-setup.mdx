...
Cloud Run uses the `rate-based` algorithm, not the `no-sync` algorithm that AWS Lambda uses. A Cloud Run Worker Pool is a
set of long-lived instances, so the WCI resizes it from arrival and backlog rates. `no-sync` only applies to providers
that are invoked per Task, and pairing it with `gcp-cloud-run` is rejected.
...
{/* TODO(lenny): The server resolves credentials through ambient ADC, per the description of
workercontroller.compute_providers.gcp.first_delegate_as_base in temporal-auto-scaled-workers wci/client/config.go.
Still confirm whether a service-account key file is a recommended fallback. Not dogfooded. */}
