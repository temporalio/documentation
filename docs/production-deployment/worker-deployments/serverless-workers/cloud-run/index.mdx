...
| `--service-account` | The runtime service account the Worker Pool instances run as. It needs read access to the secrets referenced below. This is distinct from the invoker service account in [Step 3](#configure-iam), which Temporal impersonates to scale the pool. |
...
separate runtime service account you set in [Step 2](#create-worker-pool).
