...
The runner needs no baseline role to run the Worker. Cloud Run collects `stdout` and `stderr` into
[Cloud Logging](https://cloud.google.com/run/docs/logging) through its own infrastructure, and the Cloud Run service
agent, not the runner, pulls the container image. Grant the runner only what your code reaches:
