...
can run on a Worker Pool. The one addition is Worker Versioning, which Serverless Workers require. The tabs below cover
...
- [Terraform](https://developer.hashicorp.com/terraform/install) installed. Temporal provides the IAM setup as a
  Terraform module.
- A Temporal SDK with Worker Versioning support. Use the tabs to select your language and the rest of the page will
  update accordingly.
...
For the Java Worker setup specific to Cloud Run, see
...
For the .NET Worker setup specific to Cloud Run, see
...
For the Ruby Worker setup specific to Cloud Run, see
...
covers every Workflow the Worker registers. Setting it to `Some(VersioningBehavior::Unspecified)` is an error at
startup.
...
For the Rust Worker setup specific to Cloud Run, see
...
`node:22-slim` ships without a CA certificate bundle, and the Rust core of the TypeScript SDK reads TLS roots from the operating system's store. Without the `ca-certificates` step above, the Worker fails at startup with `TransportError: tonic::transport::Error(Transport, NativeCertsNotFound)`.
