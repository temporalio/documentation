...
The scaling flags are optional, and the defaults suit most Worker Pools. If your Workers run long-running Activities,
raise `--gcp-cloud-run-scale-down-stabilization-duration` (default `90s`) so the pool does not shrink while those
Activities are still executing. On `create-version`, that flag has to be passed together with
`--gcp-cloud-run-min-instances`, `--gcp-cloud-run-max-instances`, `--gcp-cloud-run-initial-instances`, and
`--gcp-cloud-run-utilization-target`. You can also change it later on its own with `temporal worker deployment
update-version`. Both require Temporal CLI v1.8.3 or later. See
[Scale-in](/serverless-workers/cloud-run#scale-in).
