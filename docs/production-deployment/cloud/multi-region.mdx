...
The audit log entry for Failover uses the `"operation": FailoverNamespaces"` event.
...
* When Temporal triggers failovers, the audit log updates with details.
  Look specifically for `"operation": "FailoverNamespaces"` in the logs.
...
The `sa-east-1` region listed here is not yet available for use with multi-region Namespaces at this time.
