...
Temporal Cloud's multi-region Namespace (MNS) feature adds push-button high availability for your Temporal Service.
MNS add seamless failover capabilities to your production work, ensuring continuous operation without manual intervention​​.
...
At failover, routing is completely transparent for multi-region customers unless you use PrivateLink.
That is, you don’t need to make any changes on your end.
If you are a PrivateLink customer, this section describes how to configure multi-region routing for multi-region Namespaces.
...
* setting override DNS; and 
* ensuring network connectivity between the two regions.
...
By using a subzone for `region.<tmprl_domain>`, you can override the regional DNS to make sure traffic is routed internally.
