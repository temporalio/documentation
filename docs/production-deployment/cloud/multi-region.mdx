...
* Multi-region Namespaces offer a 99.99% contractual SLA for workloads with strict high-availability requirements.
  ...
  In the event of a regional failure, Temporal Cloud automatically fails over the MRN Namespace to the secondary region. 
* Single-region Namespaces offer a 99.9% contractual Service Level Agreement ([SLA](/cloud/sla)).
...
Even though our contractual SLA for single-region Namespaces is 99.9%, Temporal Cloud provides 99.99% service availability for _all_ Namespaces, both single- and multi-region.
