...
Temporal supports authentication to multi-region Namespaces using [API keys](/cloud/api-keys) or [mTLS](/cloud/certificates).
...
We recommend multi-region Namespaces to connect using the gRPC endpoint format `<namespace>.<account>.tmprl.cloud:7233` regardless of authentication method.
This allows automated regional failover without needing to switch endpoints.
