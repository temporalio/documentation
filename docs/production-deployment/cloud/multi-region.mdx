...
For these reasons, this feature provides Temporal Cloud's highest levels of regional service reliability.

This feature guide covers the following topics:

- [Why choose high availability multi-region Namespaces?](/cloud/multi-region#multi-region-intro)
...
- [Manage your multi-region Namespaces](/cloud/multi-region#management)

## Why choose multi-region Namespaces? {#multi-region-intro}
...
Temporal's multi-region failovers enable existing Workflow Executions to keep running and new Executions to be initiated.
...
Multi-region Namespaces in Temporal Cloud allow two replicated Namespaces (one active, one standby) to accept requests, but not simultaneously.
...
Multi-region Namespaces support standby recovery.
...
:::info
- Customers who wish to test the set-up of their multi-region Namespace can manually trigger failovers using the Web UI page, the 'tcld' CLI utility, or code their own solutions using our APIs.
...
When using multi-region Namespaces, the replication between regions is asynchronous.
...
## Manage your multi-region Namespaces {#management}

Temporal enables you to create and manage your multi-region Namespaces using the Temporal Cloud Web UI interface, the command line 'tcld' CLI utility, and by issuing API commands.
Use the following capabilities to create, upgrade, and discontinue your multi-region Namespaces.

* [Create multi-region Namespaces](/cloud/multi-region#create)
* [Upgrade single-region Namespaces to multi-region](/cloud/multi-region#add-regions)
...
:::
### Create multi-region Namespaces {#create}

The following sections show how to create multi-region Namespaces.
Multi-region provides multi-region deployment backed by Temporal's data replication and active-standby features.
...
### Upgrade single-region Namespaces to multi-region {#add-regions}
