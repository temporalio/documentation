...
- Open Workflows continue in the standby region with minimal interruption and no data loss.
...
- Creating a new multi-region Namespace or adding a new region to an existing Namespace is not yet supported by Temporal Cloud’s Terraform provider.  
