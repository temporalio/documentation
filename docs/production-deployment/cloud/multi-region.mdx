...
Temporal Cloud's multi-region Namespace feature provides our highest levels of service availability, with 4 9s.
Our Service Level Agreement ([SLA](https://docs.temporal.io/cloud/sla)) provides 99.99% availability and 99.9% guarantee against service errors.
...
* Multi-region Namespaces provide our highest level of availability.
* Single-region Namespaces work best for workloads that don't require stringent availability guarantees.

In single-region use, Temporal clients connect to a single Namespace in one deployment region.
Single-region Temporal Cloud Namespaces offer 99.99% availability.
Our contractual Service Level Agreement ([SLA](https://docs.temporal.io/cloud/sla)) provides a 99.99% guarantee against service errors.
For many applications, this provides sufficient availability.

In comparison, our multi-region Namespaces support standby recovery for even higher availability, providing a 99.99% guarantee against service errors.
...
Consider an incident or outage that elevates error rates or latency in the active region of a multi-region Namespace.
Under these circumstances, Temporal Cloud will initiate a Namespace failover.
...
There may be a short period of time during the incident, the [replication lag](/cloud/multi-region#replication-lag), when some data is unavailable.
When the active region is unable to reach the standby region, that data will be unavailable until the network issue is fixed and data has finished replicating.
...
Single-region Namespaces provide _at-most-once_ semantics for Activities execution (when [Maximum Attempts](https://docs.temporal.io/retry-policies#maximum-attempts) is set to 0).
...
**How do you create, enable, and manage my multi-region Namespace?**

Temporal enables you to create and manage your multi-region Namespace using the Temporal Cloud Web UI and the command line 'tcld' CLI utility.
...
* When making your request you must let us know which region you want the Namespace to land in after removing the secondary region.
...
* `temporal_cloud_v0_replication_lag_bucket`: the bucketed replication lag histogram over a time interval
* `temporal_cloud_v0_replication_lag_sum`: the total replication lag over a time interval
* `temporal_cloud_v0_replication_lag_count`: the number of lag incidents over a time interval

The following samples demonstrate how you can use these metrics to explore replication lag.

**Per-second rate of change**

This sample calculates the 99th percentile of per-second rate of change in replication lag over the last minute by Namespace:
...
This sample calculates the average replication lag per operation over the last minute, for each Namespace:
...
| Region | PrivateLink VPC Endpoint | DNS Record Override |
|---|---|---|
...
| `us-east-1` | `com.amazonaws.vpce.us-east-1.vpce-svc-0b06bbbd81fd41222` | `aws-us-east-1.region.tmprl.cloud` |
| `us-east-2` | `com.amazonaws.vpce.us-east-2.vpce-svc-01b8dccfc6660d9d4` | `aws-us-east-2.region.tmprl.cloud` |
| `us-west-2` | `com.amazonaws.vpce.us-west-2.vpce-svc-0473f53028c65d8fb` | `aws-us-west-2.region.tmprl.cloud` |
