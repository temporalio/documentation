...
There are a few caveats:
   1. For long-lived entity Workflows, new Nexus requests will be routed to the new target Namespace and the long-lived entity-Workflow will still be running on the previous Namespace.
   ...
   3. A cancel Nexus Operation request from a caller Workflow will be routed via the Nexus Endpoint to the new target Namespace, which may not have the underlying Workflow.
   4. In these scenarios our team can provide guidance on different migration strategies.