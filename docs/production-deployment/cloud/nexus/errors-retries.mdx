...
The Nexus Machinery breaks up the [Nexus Operation lifecycle](/nexus/services-operations#operation-lifecycle) into one or more [Nexus Tasks](/workers#nexus-task) that a Nexus handler is responsible for processing.
For [synchronous Nexus Operations](/nexus/services-operations#synchronous-operations-lifecycle) the Nexus Machinery creates a single Nexus Task to start an Operation.
Additional Nexus Tasks may be used for [asynchronous Nexus Operations](/nexus/services-operations#asynchronous-operation-lifecycle), for example to cancel a long-running Operation.
