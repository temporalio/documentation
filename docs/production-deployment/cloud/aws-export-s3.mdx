...
2. (optional) An IAM role that has write permission to the above S3 bucket.
...
:::note Why does Temporal Cloud provision multiple internal IAM roles to trust for Export?

Temporal Cloud creates multiple intermediary IAM roles for export operations primarily for security purposes. 
