...
import ToolTipTerm from '@site/src/components/info/ToolTipTerm';
...
Use `tcld namespace delete-region` to remove a <ToolTipTerm term="replica" /> to an existing Temporal Cloud [Namespace](/namespaces).
...
The region to remove from the existing Namespace.
