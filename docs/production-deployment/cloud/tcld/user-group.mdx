...
multiple namespace roles to the group. Value is the form of `<namspaceid>-<role>` where the namspace ID is the full ID of the namespace and role is one of `admin`, `read`, or `write`. Example: `mynamespace.abc123-read` adds the read role for the `mynamespace.abc123` namespace.
...
Will remove the given namespace roles instead of replacing all existing roles already assigns. This allows namespace roles to be removed without knowing what roles are already assigned to the group.
