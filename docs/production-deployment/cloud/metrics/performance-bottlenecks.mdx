...
- CPU-intensive work: Performing CPU-intensive operations in your Workflow Task is can lead to slow execution.
...
- Complex Workflow logic: Workflow code with complex logic or computationally intensive operationssuch as scheduling many concurrent child Workflows or Activities, can increase replay latency.
...
2. Optimize your Activity implementat to reduce latency, especially with external services or database interactions.
...
The [`temporal_worker_task_slots_available{worker_type=”ActivityWorker”}`](/references/sdk-metrics#worker_task_slots_available) metric indicates the number of available slots for executing Tasks on a Worker.
This metric may go to zero for several reasons:
...
his can help you understand the load on your Temporal service and how often your Workers are polling for Activity Tasks.
...
Temporal Workers relies on caching to optimize performance by reducing the overhead of fetching Workflow state from the history.
...
This means that if the Worker needs to process more Tasks for the evicted Workflow Execution, it will have to fetch the entire Event History from the Temporal Service.
