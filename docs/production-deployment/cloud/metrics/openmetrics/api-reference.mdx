...
The Temporal Cloud OpenMetrics API provides actionable operational metrics about your Temporal Cloud deployment. This is a scrapeable HTTP API that returns metrics in OpenMetrics format, suitable for ingestion by Prometheus-compatible monitoring systems.
...
Metrics descriptions are also available programmatically via the `/v1/descriptors` endpoint.
...
<CaptionedImage
    ...
    title="Creating a Service Account with Metrics Read-Only Role"
/>
...
* **`region`**: The AWS/GCP region where the metric originated  
...
* `limited`: Response truncated due to size limits (30k metric data points max)  
  * use namespace or metric filtering to reduce the response size  
* `unknown`: Completeness cannot be determined (possibly due to regional issues or timeouts)   
  * Clients are encouraged to retry
...
| Header | Description |
| ----- | ----- |
| `X-Completeness` | Indicates if response is complete: `complete`, `limited`, or `unknown` |
...
2. Import the [Grafana dashboard](https://github.com/temporalio/dashboards/blob/master/cloud/temporal_cloud_openmetrics.json) and configure your prometheus datasource
...
Examples for these integrations \+ more live [here](https://github.com/temporal-community/cloud-metrics-scrape-examples).
...
Note: In Prometheus, the `params` config can be set to match the same behavior as above
...
If using prometheus, it can be configured to drop metrics with a specific label or even rename specific label values to reduce the cardinality.
...
| ----- | ----- | ----- |
| 10k datapoints per metric | Response may be truncated | Use namespace/metric filtering |
| 30k total metrics per scrape | Response may be truncated | Use namespace/metric filtering |
