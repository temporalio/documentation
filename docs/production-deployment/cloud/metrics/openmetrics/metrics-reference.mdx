...
* **Value Metrics**: The most recent aggregate value within some lookback window(e.g., backlogs, limits)  
...
| `region` | AWS/GCP region where the metric originated |
...
| ----- | ----- |
| `operation` | The name of the operation |
| `is_background` | Whether background operation: `true` or `false` |
| `namespace_mode` | The namespace mode |
