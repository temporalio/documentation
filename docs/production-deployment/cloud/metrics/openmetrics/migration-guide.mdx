...
The OpenMetrics endpoint is available in  [Public Preview](/evaluate/development-production-features/release-stages#public-preview) for testing and validation. The existing query endpoint remains fully operational and supported.
...
```
[Now - Public Preview Available]
        ↓
[TBD - General Availability (GA)]
        ↓
[GA + 6 months - Query Endpoint Deprecated]
        ↓
[Sunset - Query Endpoint Removed]
```

* **Now**: OpenMetrics endpoint available in Public Preview for testing  
* **During Public Preview**: Both endpoints run in parallel \- no changes required  
* **At GA**: OpenMetrics endpoint becomes production-ready  
* **GA \+ 6 months**: Query endpoint deprecated and eventually removed  
* **Action Required**: Complete migration before the 6-month deprecation window ends
...
Metrics from OpenMetrics are already rates, therefore certain Prometheus functions no longer make sense. See below (not an exhaustive list):
...
**Before (Query Endpoint)**: Calculate percentiles using `histogram_quantile()`
...
For folks that want to get started immediately, Temporal Cloud metrics can be directly integrated into Datadog by configuring the Datadog agent to scrape the OpenMetrics endpoint.  Example for that lives [here](https://github.com/temporal-community/cloud-metrics-scrape-examples/tree/main/datadog/openmetrics).
...
Below is a template for mapping metrics from the old query endpoint to the new OpenMetrics endpoint. Note that all metrics follow the pattern of `v0` → `v1` version change, and the fundamental difference is the shift from cumulative counters to pre-computed rates for the majority of the metrics. Note that labels below are only new labels added to the metrics. For the complete list of labels, see the metrics reference page.
...
See [metrics reference](/production-deployment/cloud/metrics/openmetrics/metrics-reference) for up-to-date list of all available metrics and their full descriptions.
...
Namespace filtering can be used to ensure that metrics are scraped for relevant Namespaces, thereby reducing cardinality.
...
Or you can even relabel certain label values in order to ensure to keep significant ones, for example, it’s possible to rename non-important task queues to “unknown” while retaining important ones.
...
* Consider using PromQL `or` operator: `metric_v1 or metric_v0`

### Q: Are there limits to how frequently I can scrape or how much data will be returned?

The limits are documented on the main OpenMetrics endpoint documentation page.

### Q: Why are some metrics missing from my scrapes? I don’t see all the metrics documented

The OpenMetrics endpoint only returns metrics that were actually generated during the one-minute aggregation window. This is different from the query endpoint which might return zeros.
