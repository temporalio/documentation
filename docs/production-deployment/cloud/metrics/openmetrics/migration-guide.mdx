...
No. The metrics will be approximately the same but due to aggregation differences and windowing, it is not expected for values to match exactly between the two endpoints. 
