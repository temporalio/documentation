...
Temporal provides 2 different sources of metrics.
Each source provides users options for metrics, levels of granularity and filtering, monitoring-tool integrations, and configuration.
...
Before implementing observability for Temporal Cloud, decide what you need to measure for your use case.

You can implement Temporal Cloud observability for two different use cases:
...
When measuring the health and performance of Temporal backed applications and key business processes, you should typically rely on Temporal SDK metrics as a source of truth.
This is because Temporal SDKs provide metric values calculated from your business process implementation.
By using SDK Metrics, users gain visibility into the health of their application from the perspective of their application, not from the perspective of the Temporal Service. 

SDK metrics are for monitoring individual workers and your code's behavior; Cloud/Server metrics are for monitoring Temporal behavior.
When used together, Temporal Cloud and SDK metrics help users measure the health and performance of their full Temporal infrastructure, including the Temporal Cloud Service and user supplied Temporal Workers. 
