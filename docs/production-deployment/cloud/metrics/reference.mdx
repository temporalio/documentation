...
Metrics are scraped every 30s, which gets exposed to the metrics endpoint with a 1 min lag.
The endpoint has a set resolution of 15s, which shows the same value twice because of the scrape interval.
