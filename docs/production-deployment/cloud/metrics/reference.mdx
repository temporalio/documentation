...
* **[Metrics capture](#capture)**:
  Gather metrics using a Prometheus-style endpoint so they can be visualized using Prometheus and Grafana or exported to observability platforms like Datadog.
...
* SDK metrics start with the phrase `temporal_`.
  These metrics are distinct from Temporal Cloud metrics.
...
* [How to set up Grafana with Temporal Cloud observability](/cloud/metrics/prometheus-grafana).
* [How to monitor Worker Health with Temporal Cloud Metrics](/production-deployment/cloud/worker-health)
* [How to monitor Service Health with Temporal Cloud Metrics](/production-deployment/cloud/service-health)
...
Metrics for all Namespaces in your account are available from the [metrics endpoint](#capture).
