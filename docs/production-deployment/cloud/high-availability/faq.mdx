...
- **[Graceful failover**](/cloud/replication/how-it-works#graceful-failover):
...
- **[Forced failover**](/cloud/replication/how-it-works#forced-failover):
  In this mode, a replica immediately activates in the standby Namespace.
  Events not replicated due to [replication lag](/cloud/replication/best-practices#metrics) will undergo [conflict resolution](/cloud/replication/how-it-works#conflict-resolution) upon reaching the new active Namespace.

  This mode prioritizes _availability_ over consistency.

- **[Hybrid failover**](/cloud/replication/how-it-works#hybrid-failover) (Default mode):
...
Read more about [automatic failovers](/cloud/replication/how-it-works#health-checks).
...
Otherwise, it is typically a matter of seconds or even less and can be monitored through  [external metrics](/cloud/replication/best-practices#metrics).

**Q: Can Workflows execute events a second time in the standby Cluster due to replication lag?**

Yes. This is explained in the [conflict resolution](/cloud/replication/how-it-works#conflict-resolution) section of our documentation.
...
Yes, replication lag is a [metric](/cloud/replication/best-practices#metrics) that we expose.
...
This is [documented](/cloud/replication/best-practices#metrics).
...
Customers should decide whether to add logic to handle this or manually fix affected Workflows if they believe the risk is low. Other known limitations have been [documented](/cloud/replication/how-it-works#workflow-execution-replication) around causality and so forth.
