...
When you enable these features, Temporal Cloud automatically synchronizes your data between the primary and a <Components.ToolTipTerm term="replica"/>, keeping them in sync.
In case of an incident or an outage, Temporal will automatically <Components.ToolTipTerm term="fail over" src="failover" /> your Namespace from the primary to the replica.
...
Returning control from the replica to the primary is called a <Components.ToolTipTerm term="failback" />.
...
Our recovery point objective (<Components.ToolTipTerm term="RPO" />) is near-zero.
...
Our recovery time objective (<Components.ToolTipTerm term="RTO" />) is 20 minutes or less per incident.
