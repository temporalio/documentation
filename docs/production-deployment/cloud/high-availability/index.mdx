...
Failback [may be disabled](/cloud/high-availability/failovers#disabling-temporal-initiated) by modifying your Namespace settings.
...
Our recovery point objective (<ToolTipTerm term="RPO" tooltip="RPO is the maximum acceptable duration during which transactional data might be lost from the service." />) is near-zero.
...
Our recovery time objective (<ToolTipTerm term="RTO" tooltip="RTO sets a maximum time and service level objective, after which, service must be restored following a disruption." />) is 20 minutes or less per incident.
