...
This replication ensures that any Workflow state or History change is written to all three AZs before sending an acknowledgment to the Client.
...
Each cell, along with its physical and software resources, is a failure domain or "isolation domain".
When a shared dependency or common infrastructure component -- such as the database or networking -- fails within a cell, all Namespaces within that cell are affected.

High Availability features improve reliability and reduce risk by deploying a primary and its replica to different cells, that is separate isolation domains.
If one domain experiences an incident, a simple failover mitigates the impact.
...
For these critical use cases, Temporal Cloud offers a suite of _High Availability (HA) features_ with a 99.99% contractual SLA.
