...
You can test the failover of your Namespace by manually triggering a failover using the UI or the 'tcld' CLI utility.
...
- When working with multi-region Namespaces, your CNAME may change.
  ...
  This change doesn't affect standard Namespaces as they are **deployed within a single region**.
...
Temporal Cloud’s High Availability features use asynchronous replication between the primary Namespace and the replica.
Workflow updates in the primary Namespace, along with associated History Events, are transmitted to the replica with a short delay.
...
Lags may also cause recently started Workflows to be temporarily unavailable until the primary Namespace recovers.
