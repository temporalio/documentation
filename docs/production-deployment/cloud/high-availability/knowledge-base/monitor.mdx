...
- **Data synchronization issues:** The replica fails to stay in sync with the primary Namespace due to network or performance problems.
- **Replication lag:** The replica falls behind the primary Namespace, causing it to be out of sync.
- **Network issues:** Loss of communication between the replica and the primary Namespace causes problems.
...
This happens because operations are replicated: they occur on both the primary Namespace and the replica.
