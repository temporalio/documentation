...
description: Temporal Cloud's Namespace with High Availability featuress offer automated failover, synchronized data replication, and high availability for workloads requiring disaster-tolerant deployment and 99.99% uptime.
tags:
  - Temporal Cloud
  - Production
  - High availability
...
Temporal’s replication and automated failover features ensure high availability by automatically switching between active primary Namespaces and their replicas when needed.
By enabling high availability replication, you allow Temporal to copy Namespace metadata and Workflow Executions to a <ToolTipTerm term="replica" tooltip="A replica is a synchronized copy of a Namespace. It ensures redundancy, fault tolerance, and minimal downtime during an incident or outage. A replica can take over operations if the primary Namespace becomes unavailable. It's synchronized up to the point of replication lag, where no further data is available." />.
This redundancy, combined with failover capability, enhances stability during outages.
...
Your application experiences minimal disruption, as Temporal automatically shifts operations to a replicated system.
...
Each Namespace is replicated across <ToolTipTerm term="isolation domains" tooltip="An isolation domain is a defined area within Temporal Cloud's infrastructure. It helps contain failures and prevents them from spreading to other parts of the system. This provides redundancy and fault tolerance." /> or separate regions.
...
Temporal Cloud restricts certain Workflow operations to the primary Namespace:

- You may only update Workflows in the primary Namespace.
- You may only dispatch Workflow Tasks and Activity Tasks from the primary Namespace.
  Because of this, forward progress in a Workflow Execution can only be made in the primary Namespace.

These limits mean that certain requests, such as Start Workflow and Signal Workflow, are processed by and limited to the primary Namespace.
Replicas may receive API requests from Clients and Workers.
They automatically forward these requests to the primary Namespace for execution.
...
During this time requests forward from the now passive (formerly active) primary Namespace to the newly active (formerly passive) replica.

As Workflow Executions progress and are operated on, replication Tasks created in the primary Namespace are dispatched to the replica.
...
Temporal deploys your primary Namespace and its replica across separate <ToolTipTerm term="isolation domains" tooltip="An isolation domain is a defined area within Temporal Cloud's infrastructure. It helps contain failures and prevents them from spreading to other parts of the system. This provides redundancy and fault tolerance." />.
These domains can be in the same region or in different regions, depending on your setup.
Because each region has multiple isolation domains, you can place your replica in the same region as your primary Namespace (same-region replication) or in another region (multi-region replication).

- **Same-region Replication**:
  Temporal replicates Namespaces across isolation domains within one region.
  A good fit when your application is built for one region and you prefer to failover within that region.
...
Temporal detects these issues using regular health checks.
It monitors error rates, latencies, and infrastructure problems, such as request timeouts.
If it finds unhealthy conditions where indicators exceed the allowed thresholds, Temporal automatically switches the primary Namespace to the replica.
...
- During normal operation, the primary Namespace asynchronously copies operations and metadata to its replica, keeping them in sync.
- If the primary Namespace becomes unavailable, Temporal detects the issue through health checks.
  It automatically switches to the replica, using one of its available [failover scenarios](#scenarios).
- The replica takes over the active role.
  Operations continue with minimal disruption.
- When the original primary Namespace recovers, the roles can either switch back (failback, by default) or remain as they are, based on your Namespace settings.
...
After the network partition is resolved and communication between the isolation domains/regions is restored, a conflict resolution algorithm determines whether the primary Namespace or replica remains active.
...
In contrast, with a Temporal Cloud high availability Namespace, only the primary Namespace accepts requests and writes at any given time.
...
Updates made to the primary Namespace may not immediately be reflected in the replica due to <ToolTipTerm term="replication lag" tooltip="The transmission delay of Workflow updates and history events from the primary Namespace to the replica." />, particularly during failovers.
...
After failover, Temporal Cloud creates a new branch history for execution, and begins its <ToolTipTerm term="conflict resolution" tooltip="The process of identifying and resolving conflicts that arise when different parts of a system have inconsistent data or operations. Conflicts can occur when Namespaces and replicas have diverging states. These are often due to network issues, concurrent updates, or replication delays. Conflict resolution ensures that all elements eventually converge to a consistent and correct state, minimizing data inconsistency and system disruption." /> process.
...
This capability is crucial for ensuring Workflow Executions continue forward without losing progress, and for maintaining consistency across replication, even during incidents that caused disruptions in replication.
