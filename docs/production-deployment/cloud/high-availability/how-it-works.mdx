...
By enabling High Availability replication, you allow Temporal to copy Namespace metadata and Workflow Executions to a <Components.ToolTipTerm term="replica"/>.
...
Each Namespace is replicated across <Components.ToolTipTerm term="isolation domains" /> or separate regions.
...
Updates made to the primary may not immediately be reflected in the replica due to <Components.ToolTipTerm term="replication lag" />, particularly during failovers.
...
After failover, Temporal Cloud creates a new branch history for execution, and begins its <Components.ToolTipTerm term="conflict resolution"/> process.
