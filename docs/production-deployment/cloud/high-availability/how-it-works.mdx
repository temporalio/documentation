...
Temporal deploys your primary and its replica across separate <ToolTipTerm term="isolation domains" tooltip="An isolation domain is a defined area within Temporal Cloud's infrastructure. It helps contain failures and prevents them from spreading to other parts of the system. This provides redundancy and fault tolerance." />.
...
In contrast, with a Temporal Cloud high availability Namespace, only the primary accepts requests and writes at any given time.
...
Updates made to the primary may not immediately be reflected in the replica due to <ToolTipTerm term="replication lag" tooltip="The transmission delay of Workflow updates and history events from the primary to the replica." />, particularly during failovers.
