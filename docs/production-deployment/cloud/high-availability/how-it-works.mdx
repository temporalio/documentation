...
By enabling High Availability replication, you allow Temporal to copy Namespace metadata and Workflow Executions to a <Components.ToolTipTerm term="replica" tooltip="A replica is a synchronized copy of a Namespace. It ensures redundancy, fault tolerance, and minimal downtime during an incident or outage. A replica can take over operations if the primary becomes unavailable. It's synchronized up to the point of replication lag, where no further data is available." />.
...
<Components.DiscoverableDisclosure label="Workflow replication restrictions">
...
Temporal deploys your primary and its replica across separate <Components.ToolTipTerm term="isolation domains" tooltip="An isolation domain is a defined area within Temporal Cloud's infrastructure. It helps contain failures and prevents them from spreading to other parts of the system. This provides redundancy and fault tolerance." />.
...
<Components.CaptionedImage src="/img/cloud/high-availability/failover.png" title="On failover, the replica becomes active and the Namespace endpoint directs access to it." />
...
Updates made to the primary may not immediately be reflected in the replica due to <Components.ToolTipTerm term="replication lag" tooltip="The transmission delay of Workflow updates and history events from the primary to the replica." />, particularly during failovers.
