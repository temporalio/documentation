...
| **Temporal<br />tcld&nbsp;CLI**          | At the command line, enter: <br /> <tt>tcld namespace create \ <br /> <Spacer width={4} />--namespace \<namespace_id>.\<account_id> \ <br /><Spacer width={4} />--region \<primary_region> \ <br /> <Spacer width={4} />--region \<replica_region></tt> <br /><br /> Specify the [region codes](/cloud/service-availability) as arguments to the two `--region` flags.<br /> <Spacer width={4} /> - Using the same region replicates to an isolation domain within that region.<br /> <Spacer width={4} /> - Using a different region (within the same continent) replicates across regions. <br /><br /> If using API key authentication with the `--api-key` flag, you must add it directly after the tcld command and before `namespace create` |
...
| **Temporal<br />tcld&nbsp;CLI**          | At the command line, enter: <br /> <tt>tcld namespace add-region \ <br /> <Spacer width={4} />--namespace \<namespace_id>.\<account_id> \ <br /><Spacer width={4} />--region \<replica_region></tt> <br /> <br /> Specify the [region code](/cloud/service-availability) of the region where you want to create the replica as an argument to the `--region` flag:<br /><Spacer width={4} />- Using the current region replicates to an isolation domain within your existing region.<br /><Spacer width={4} />- Using a different region (within the same continent) replicates across regions.<br /><Spacer width={4} />- You cannot create a Multi-region deployment on separate continents.<br /><br /> If using API key authentication with the `--api-key` flag, you must add it directly after the tcld command and before `namespace add-region` <br /><br /> Temporal Cloud sends an email alert once your Namespace is ready for use. |
