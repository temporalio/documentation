...
First, ensure that any Workers or Clients with API keys have been configured to use the [regional endpoint](/cloud/regions) for the Namespace's region.
...
If using API key authentication with the `--api-key` flag to issue this CLI command, you must add it directly after the tcld command and before `namespace delete-region`
