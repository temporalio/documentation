...
1. Create a Pub/Sub topic and make a note of its topic name, for example, "test-
   auditlog".
1. Set up a service account in the same project in Google Cloud that trusts the Temporal internal
...
1. There are two ways to configure the service account to write to the GCS sink. Select **Manual** to configure
   the account manually, or **Deploy with Terraform** to use Terraform.
