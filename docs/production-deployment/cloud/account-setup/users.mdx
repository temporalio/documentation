...
- **Developer**
  - Can create Namespaces
    - Can modify Namespaces they create
    - Has write permissions for all Namespaces they create, allowing them to control [Workflows](/workflows), [Schedules](/workflows#schedule), etc within those Namespaces
  - Has Namespace Admin [permissions](#namespace-level-permissions) on all [Namespaces](/namespaces) they've created
    - Has read-level access to the list of all users within the account
    - Can add and remove users from a Namespace they've created
    - Can assign Namespace-level permissions (admin, write, or read-only) to users for a Namespace they've created
- **Read-Only**
  - Can read limited account information

In addition, there are two roles that the Global Admin can't assign:
...
- **Finance Admin**
  ...
  - Otherwise, has the same permissions as the non-administrative Read-Only role.
