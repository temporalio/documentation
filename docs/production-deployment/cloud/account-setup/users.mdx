...
By default, the first user assigned to your account is granted the **Account Owner** role. If your account does not have an Account Owner, please [contact Support](https://temporalsupport.zendesk.com/) to assign the appropriate individual to this role.
...
This role configures account-level parameters and manages Temporal billing and credit data.
It allows users to perform all actions within the Temporal Cloud account.

:::tip Best Practices

Temporal strongly recommends the following precautions when assigning the AccountOwner role to users:

- Assign the role to at least two users in your organization.
  Otherwise, limit the number of users with this role.
- Associate each Account Owner’s email address with their role so Temporal Cloud support can contact them in urgent situations.
...
You can update a user's account-level [Role](#account-level-roles)  using either the Web UI or tcld..

:::info

- To update an account-level Role, the user must have Global Admin privileges.
