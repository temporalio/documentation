...
## Configure clients to use client certificates {#configure-clients}
...
To connect to a Temporal Namespace using the Temporal CLI and certificates specify your certificates and the TLS server name.

```bash
temporal --tls-ca-path <Path to server CA certificate> --tls-cert-path <Path to x509 certificate> --tls-key-path <Path to private certificate key> --tls-server-name <Override for target TLS server name>
```
