...
Each Namespace Name, such as `rapid-delivery-production`, is unique within the scope of a customer's account.
...
Each Id is a short string of numbers and letters like `abc45`, at least five characters long.
...
3. View the Namespaces listed for the user. Each Namespace includes several details:
   ...
   - _Region_:
     corresponds to the Namespace's [hosting region](/cloud/regions).
   - _Retention_:
...
For example, for the Account Id `abc45` and Namespace Name `rapid-delivery-production`, the Namespace Id is `rapid-delivery-production.abc45`.
...
For example: `https://cloud.temporal.io/namespaces/rapid-delivery-production.abc45`.
