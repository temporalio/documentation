...
Temporal Cloud supports authentication to Namespaces using [API keys](/cloud/api-keys) or [mTLS](/cloud/certificates).
Cloud only supports one authentication method at a time.
...
- For [Namespaces with High Availability features](/cloud/high-availability) use the endpoint format `<namespace>.<account>.tmprl.cloud:7233` regardless of your authentication method, because it allows automated failover without needing to switch your endpoint.
