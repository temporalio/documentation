...
A gRPC endpoint is found on the detail page for each Cloud Namespace.
...
By default, each account is allocated a limit of with ten Namespaces.
...
- [CA certificate](/cloud/certificates#certificate-requirements) for the Namespace, if you are using the mTLS authentication method.
...
1. Select your authentication method between [API keys](/cloud/api-keys) or [mTLS](/cloud/certificates)
1. If using the mTLS authentication method, paste the CA certificate for this Namespace.
...
- By default, each account is allocated a limit of ten Namespaces.
...
The method is set at Namespace creation.
