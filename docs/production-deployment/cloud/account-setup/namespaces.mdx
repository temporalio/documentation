...
### Delete Namespace Protection

To prevent accidental deletion of a Namespace, Temporal Cloud provides a feature to protect against deletion.
We recommend enabling delete protection on Namespaces used in production environments, to ensure that critical data is not unintentionally deleted.

To enable or disable this feature using the Temporal Cloud UI, update the security setting on the Namespace edit page.

To enable or disable this feature using [`tcld`](/cloud/tcld), use the following command.
```
tcld namespace lifecycle set -n <namespace> --enable-delete-protection <true/false>
```
