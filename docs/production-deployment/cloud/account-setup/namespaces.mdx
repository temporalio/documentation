...
We recommend using one namespace per use case, domain, and environment combination when multiple services that are part of the same use case need to communicate with each another via [Signals](/sending-messages#sending-signals) or by starting [Child Workflows](/encyclopedia/child-workflows).
...
Temporal supports authentication to Namespaces using [API keys](/cloud/api-keys) or [mTLS](/cloud/certificates).
We currently only support one authentication method at a time, which is determined at namespace creation time.
If you require migrating from one authentication method to another, please contact [Temporal Support](https://support.temporal.io).

Authentication methods require specific endpoints in order to programmatically access your namespace.
- For the API key authentication method, use the endpoint format `<region>.<cloud_provider>.api.temporal.io:7233`.
- For the mTLS authentication method, use the endpoint format `<namespace>.<account>.tmprl.cloud:7233`.
- For [multi-region Namespaces](/cloud/multi-region) use the endpoint format `<namespace>.<account>.tmprl.cloud:7233` regardless of authentication method, because it allows automated regional failover without needing to switch endpoints.

For information on how to connect to clients using a specific authentication method see the following documentation.
- To use API keys to connect with the [Temporal CLI](/cli), [client SDK](/develop), [tcld](/cloud/tcld), and [Cloud Ops API](/ops), see [Use API keys to authenticate](/cloud/api-keys#using-apikeys).
- To use mTLS to connect with the [Temporal CLI](/cli) and [client SDK](/develop) see [Configure clients to use client certificates](/cloud/certificates#configure-clients).

For accessing Temporal Web UI, use the HTTPS endpoint in the form `https://cloud.temporal.io/namespaces/<namespace>.<account>`; for example, `https://cloud.temporal.io/namespaces/accounting-production.f45a2`.
