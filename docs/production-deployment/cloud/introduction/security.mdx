...
12. Once available, you can use AWS PrivateLink.
    ...
    When using Temporal SDKs, this process varies by language ([Go sample](https://slack.com/archives/CTRCR8RBP/p1658745495171299), [Java sample](https://github.com/temporalio/samples-java/blob/2516af4a3da87f9ce8e2d083a700125a9e8592cf/core/src/main/java/io/temporal/samples/ssl/SslEnabledWorkerCustomCA.java#L83), [Typescript sample](https://learn.temporal.io/tutorials/typescript/background-check/project-setup/#cloud-worker), in .NET by configuring `SslCredentials` >  `TargetHost`, Python by configuring `TLSConfig` > `server_name`, PHP by configuring `ClientOptions` > `tls` > `peer_name`).
