...
### Multi-region Namespace costs

Multi-region Namespaces charges apply after the Namespace setup completes.
To estimate your charges, apply a 2x multiplier to the cost of the Namespace’s Actions and storage.
This roughly estimates your costs.
You won't be charged until Workflows actually start running and data replication begins.

In the case of a pre-existing Namespace, the following charges apply:

* We do not charge for historical actions that completed prior to the Namespace becoming multi-region.
* We charge 2x for Actions of all existing (ongoing) and new Workflows from the point of adding a new region onward.
  These are actively replicated.
* We charge 2x for storage of closed (historical), existing (ongoing), and new Workflows from the point of adding a new region onward.
...
If multi-region Namespace adoption began mid-month, expect to see more Actions than Replicated Actions.
