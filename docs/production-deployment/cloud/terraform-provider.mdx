...
The Terraform Temporal Cloud provider allows you to use Terraform to manage resources for Temporal Cloud. Terraform is a tool used to manage infrastructure as code (IaC). With this provider, you can use Terraform to automate the management of Temporal Cloud resources, like Namespaces, Users, Service Accounts, API Keys and more.

**Remember, once a resource is managed by Terraform, you should only use Terraform to manage that resource.**
...
The GitHub repository for the Terraform provider is [terraform-provider-temporalcloud](https://github.com/temporalio/terraform-provider-temporalcloud/tree/main), where you can report bugs, provide feature requests, and [contribute] (https://github.com/temporalio/terraform-provider-temporalcloud/blob/main/CONTRIBUTING.md) to the provider. We encourage your input as we develop the provider with the community. 

:::note

To see the list of avialable Temporal Cloud resources supported by Terraform provider, visit the resources section of the Terraform documentation in Hashi's [registry] (https://registry.terraform.io/providers/temporalio/temporalcloud/latest/docs).

:::

:::info

Currently, the Terraform Provider only supports Temporal Cloud deployments in AWS regions. Support for Temporal on GCP is in development.
...
- [API Key](/cloud/api-keys): an API Key is required to use the Terraform provider.
  - see [here](https://docs.temporal.io/cloud/api-keys#generate-an-api-key) for instructions on generating an API Key.
...
Terraform is a great way to automate the management of Temporal Namespaces, reagrdless if you want management to be centralized within a platform team or federated to different product teams. The provider allows you to can import, create, update, and delete Namespaces with Terraform.

You must have use an Identity that is assigned a Account Owner, Global Admin, or Developer Account Role permission to manage Namespaces in Temporal Cloud.

:::info Support, stability, and dependency info

Temporal Cloud’s Terraform provider currently does not support [multi-region Namespaces](/cloud/multi-region) or [GCP regions](https://docs.temporal.io/cloud/service-availability#gcp).
...
In this example you are creating the Temporal Cloud Namespace named `terraform`, specifying the AWS region `aws-us-east-1`, and specifying the path to the CA certificate.
...
You can find more examples of Namespace management in the Terraform Provider docs located on HashiCorp's [Terraform Registry](https://registry.terraform.io/providers/temporalio/temporalcloud/latest/docs/resources/namespace). The Terraform Provider docs provide examples of how to generate CA certs within Terraform configuration files as well as examples of creating a Namspace with API Key based authenticattion. 
...
1. Provide a configuraiton placeholder in your Terraform configuraiton.
```yml
  resource "temporalcloud_namespace" "namespace" {
  }
```
...
The Namespace is now a part of the Terraform state and all changes to the Namespace will be, should be, managed by Terraform. Once a resource has been imported into Terraform, outside changes to the resource will create Terraform "drift" errors on subsequent Terraform operations. 

## Manage Temporal Cloud Users with Terraform

Manage Temporal Cloud Users with the same process you use to manage Namespaces with Terraform. The following examples create, update, delete, and import Temporal Cloud Users by using the `terraform apply` command on the Terraform configuration file.

:::note

User Management in Temporal has a few caveats to be aware of:
- The Temporal Account Owner role is not managable by Terraform. While you can Import an Account Owner to Terraform, you cannot create, update, or delete an Account Owner with Terraform.
- Right now, it is not possible to manage a user's access to a Namespace from the Namespace resource. You must manage Namespace access from the User resource. This is true for Service Accounts as well. 
- Account Owners and Global Admins autoamtically gain access to all Namespaces in Temporal. Therefore, you cannot specify Namespace access for these roles.  This is true for Service Accounts as well. 
- To Import a user, you'll need the User's ID which is currently not available in the Temporal Cloud UI. You can get the User ID by running the `tcld user list` command.
:::
...
2. Run the `terraform import` command and pass in the User ID
...
```
The User is now a part of the Terraform state and all changes to the User will be, should be, managed by Terraform.
...
- Service Accounts do not have email addresses, they have names instead. This means you should specifiy a name for a Service Account instead of an email. 
...
You can manage your own, personal API Keys and Service Account API Keys with Terraform. The process and steps to managing an API Key with Terraform are very similar to managing other resrouces with Terraform. You can create, delete, update and import API Keys with Terraform.
One difference between working with API Keys as a Terraform resource compared to other Temporal Cloud resources is the need to access an API Keys secure token output from Terraform. We'll walk through the process of securely accessing the API Key Token in the Create section of this guide.  
...
Replace the  owner_type and owner_id values with your Temporal Cloud Servie Account or other Idnetity information.
...
4. Access the API Key Token securely.
...
:::note
  You can only edit an API Key's name or descrption field. 
...
2. Run the `terraform import` command and pass in the API Key ID
...
```
The API Key is now a part of the Terraform state and all changes to the API Key will be, should be, managed by Terraform.
Import does not import the API Key Token. The Token is only avaialble at the time of API Key creation.
...
Also, feel free to create issues and contribute PRs in the Temporal Terraform [GitHub repository](https://github.com/temporalio/terraform-provider-temporalcloud/tree/main)