...
To delete a Namespace, remove the `temporalcloud_namespace` resource and all dependant resource configurations from your Terraform files and run the `terraform apply` command.
...
To delete a Nexus Endpoint, remove the `temporalcloud_nexus_endpoint` resource configuration from your Terraform files and run the `terraform apply` command.
