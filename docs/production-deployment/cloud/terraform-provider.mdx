...
Terraform is a tool used to manage infrastructure as code (IaC).
With this provider, you can use Terraform to automate the management of Temporal Cloud resources, like Namespaces, Users, Service Accounts, API Keys and more.
...
To see the list of available Temporal Cloud resources supported by Terraform provider, visit the resources section of the Terraform documentation in Hashi's [registry] (https://registry.terraform.io/providers/temporalio/temporalcloud/latest/docs).

:::

:::info

Currently, the Terraform Provider only supports Temporal Cloud deployments in AWS regions.
...
Terraform is a great way to automate the management of Temporal Namespaces, regardless if you want management to be centralized within a platform team or federated to different product teams.
The provider allows you to can import, create, update, and delete Namespaces with Terraform.

You must have use an Identity that is assigned an Account Owner, Global Admin, or Developer Account Role permission to manage Namespaces in Temporal Cloud.
...
The Terraform Provider docs provide examples of how to generate CA certs within Terraform configuration files as well as examples of creating a Namespace with API Key based authentication.
...
2. Run the `terraform import` command from the command line and pass in the Namespace ID.
...
The Namespace is now a part of the Terraform state and all changes to the Namespace will be, should be, managed by Terraform.
...
The following examples create, update, delete, and import Temporal Cloud Users by using the `terraform apply` command on the Terraform configuration file.

:::note

User Management in Temporal has a few caveats to be aware of:

- The Temporal Account Owner role is not manageable by Terraform.
  While you can Import an Account Owner to Terraform, you cannot create, update, or delete an Account Owner with Terraform.
- Right now, it is not possible to manage a user's access to a Namespace from the Namespace resource.
  You must manage Namespace access from the User resource.
  This is true for Service Accounts as well.
...
- To Import a user, you'll need the User's ID which is currently not available in the Temporal Cloud UI.
  You can get the User ID by running the `tcld user list` command.
  :::
...
We'll walk through the process of securely accessing the API Key Token in the Create section of this guide.
