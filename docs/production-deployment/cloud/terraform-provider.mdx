...
Terraform is a great way to automate the management of [Nexus Endpoints](/nexus/endpoints).
The provider allows you to import, create, update, and delete Nexus Endpoints with Terraform.

You must use an Identity with Developer role (or higher) and Namespace Admin permissions on the Endpoint's target Namespace.
This includes the Account Owner, Global Admin, or Developer Account Role.
...
1. Create a Terraform configuration file (`terraform.tf`) to define a Nexus Endpoint.
   ...
   In this example, you create a 3 Namespaces:
   - target Namespace for a Nexus Endpoint - Nexus requests will be routed to a Worker in the target Namespace.
   - caller Namespace(s) that will invoke Nexus Operations through a Nexus Endpoint.

   These 3 Namespaces are referenced in the Nexus Endpoint configuration:
...
1. Search for the Nexus Endpoint you created, using only the Endpoint Name (without an account suffix).
...
Run the `tcld nexus endpoint get` command and pass in your Nexus Endpoint name:

```bash
tcld nexus endpoint get -n "<your-nexus-endpoint>"
```
...
1. Add or remove allowed caller Namespaces by updating the Nexus Endpoint configuration to remove `caller_namespace_2`:
...
1. Apply your configuration.
   ...
   It may take several seconds to update a Nexus Endpoint.
...
1. Run the `terraform import` command from the command line and pass in the Nexus Endpoint Name.
   Your Nexus Endpoint Name is available at the top of the Nexus Endpoint's page in the [Temporal Cloud UI](https://cloud.temporal.io/nexus).

   ```bash
   terraform import temporalcloud_nexus_endpoint my-nexus-endpoint
   ```
