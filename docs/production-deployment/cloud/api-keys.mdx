...
Service accounts have the permission to rotate their own API keys. To use this feature, have your service account create a new API key using [the Cloud Operations API](/production-deployment/cloud/operation-api.mdx) before the current one expires. Note that service accounts cannot delete their own API keys, which helps keep workflow access secure.
...
A: Up to 10 active keys per user and 20 active keys per Service Account.
