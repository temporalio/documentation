...
Temporal Cloud API key authentication provides secure access to the Temporal Service from your applications.
API keys are a common industry standard for simple access control.
They need only one key to authenticate.
[mTLS certificates](https://docs.temporal.io/references/configuration#tls) offer higher security but they require more complex setup and configuration.
API keys simplify deployment and management when mTLS is not needed.
...
Each key is a unique identity.
It is linked to role-based access control (RBAC) settings to ensure secure and appropriate access.
...
For instructions on managing API keys for your organization, see [Global Administrator API key management](#manage-api-keys).
...
- Both normal users and Global Administrators can create, delete, and update API key access using the Cloud UI or `tcld`.
...
- The Global Administrator must [enable API keys access](#manage-api-keys).
- Be a Temporal Cloud user or Service Account.
...
The Global Administrator must enable access to allow the creation and use of API keys.
...
To find an API key, you can filter by API key state and identity type (Global Administrators only).
...
These sections show you how to generate, manage, and remove API keys for a user account.
...
[Login](https://cloud.temporal.io/) to the Temporal Cloud UI and navigate to your [Profile Page → API keys](https://cloud.temporal.io/profile/api-keys).
...
Global Administrators can manage API keys for all Service Accounts in their account and generate API keys for Service Accounts.
This is different from user accounts, which generate their own API keys.
...
[Login](https://cloud.temporal.io/) to the Temporal Cloud UI and go to [API keys settings](https://cloud.temporal.io/settings/api-keys).
...
To use your API key with the Temporal CLI, either pass it with the `--api-key` flag for each command or set it as an environment variable (recommended).
The `tcld` tool will automatically pick up the environment variable.
...
- `--namespace`: Provide the `namespace.accountId` from the top of the Namespace page in the UI.
  - Use the format `<namespace.accountId>`.
