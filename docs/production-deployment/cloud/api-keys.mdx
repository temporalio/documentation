...
API Keys for Client Connections are a new capability that extend the existing API Keys feature to afford users the ability to auth(n/z) into Temporal Cloud's "data plane" from Client Connections (Temporal CLI, SDKs) using an API Key.

> Note: This feature capability changes the UX for Namespace creation/management. With API Keys for Client Connections, the connection method for a Namespace is configurable, either mTLS or API Key during pre-release.

## Key Feature Terms

- **API Key**: Authentication token that can be used in the Temporal CLI, SDKs, tcld, the Cloud Operations API, and the Terraform Provider.
- **gRPC Endpoint**: The gRPC endpoint used in the Temporal CLI and/or SDKs to connect to Temporal Cloud.
  - For an mTLS connection, the endpoint is in the format `<namespace>.<account>.tmprl.cloud:7233`
  - For an API Key connection, the endpoint is in the format `<region>.<cloud provider>.api.temporal.io:7233`
- **gRPC header**: Connection information supplied by the user in the Temporal CLI or SDK when connecting to Temporal Cloud using an API Key. Currently, in most SDKs and the CLI, the gPRC header should include a Bearer Token with the API Key and a Namespace header.
...
While API Keys use in operator tools is a publicly available feature, part of Public Preview, the ability to use an API in the Temporal CLI and SDKs is available to a limited number of Temporal Cloud users through a pre-release. Before an API Key can be used in the Temporal CLI or SDK, Temporal must enable API keys for Client Connections for your account using an internal feature flag.
...
2. Service Account created API Keys - Global Administrators can create, delete, and update API Keys for Service Accounts (in pre-release)
...
By default, API Keys must be enabled by a Global Administrator before use.
...
- Ask Temporal to enable the API Keys for Client Connections pre-release for your Temporal Account
- Create a new Namespace that allows API Key connectivity.
  - For pre-release, Namespaces support either mTLS or API Key connectivity without a way to switch between mTLS or API Keys. In subsequent releases you will have the option to switch between auth methods for your Namespaces, or use both.

## Use API Keys

API Keys can be used in (covered in this pre-release document):
- Temporal CLI v 0.12
- SDKs (latest recommended)

API Keys can also be used in (covered in existing Temporal docs):
...
To use your API Key with the Temporal CLI, you can pass in the API Key to each command using the `--api-key` flag or an environmental variable (best practice).
...
- `--address`: Provide the Namespace's gRPC endpoint located in the Namespace UI in the gRPC endpoint box.
  ...
  - Can be set using an environmental variable
- `--namespace`: Provide the namespace.accountId that appears at the top of the Namespace page in the UI.
  ...
  - Can be set using an environmental variable
- `--grpc-meta "temporal-namespace="`: Provide the namespace.accountId again as grpc-metadata. During pre-release, this field is required for routing to the namespace
- `--tls` for a secure connection with the appropriate options
  - Potentially, with `--tls-disable-host-verification` to disable host verification
  - Can be set using an environmental variable

Here's an example of connecting to Temporal Cloud from the CLI using an environmental variable for the API Key:
