...
For [Multi-region Namespaces](/cloud/multi-region) with API key authentication enabled, use the gRPC endpoint format `<namespace>.<account>.tmprl.cloud:7233`. This allows automated regional failover without needing to switch endpoints.
