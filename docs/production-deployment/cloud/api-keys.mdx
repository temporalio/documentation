...
Temporal Cloud API Keys provide identity, User and [Service Account](/cloud/service-accounts), based authentication.
...
- Cloud Operations Automation: API Keys can be used as the authentication mechanism for Temporal Cloud opeational tools ([Temporal Cloud CLI (tcld)](/cloud/tcld), [the Cloud Ops API](/ops), [Terraform](/production-deployment/cloud/terraform-provider)) to manage your Temporal Cloud account, namespaces, certificates, and user identities. 
...
- Namespace Authentication: API Keys can be used as the Namespace authentication mechanism for executing and managing workflows from the SDK and Temporal CLI as an alternative to using mTLS-based authentication.
  ...
  - If you created a Temporal Account using the Self Serve Signup, API Keys are available for Namespace Authentication.
  - If you have not created a Temporal Account using the Self Serve Signup, please reach out to Temporal to participate in the Pre Release and gain access to this use of API Keys. 

API Keys are an opt-in feature that must be enabled by a Global Administrator for use. For instructions on API Key Management for your organization, see [global Administrator API Key Management](#global-administrator-api-key-management).
...
Global Administrators can monitor, manage, update access, and delete API Keys for any User or Service Account within their account.
...
You can update enable or disable an API Key. After disabling an API Key, the API Key is no longer able to authenticate with Temporal Cloud.
...
When creating new Namespaces a gRPC endpoint will be provided based on the authentication method configured for the Namespace. 
...
To create a Namespace that supports API Key authnetication, specify Allow API Key authentication when creating the Namespace.
