...
Service accounts have the permission to rotate their own API keys. To use this feature, have your service account create a new API key using the [Cloud Ops APIs](/ops) before the current one expires. Note that service accounts cannot delete their own API keys, which helps keep workflow access secure.
