...
- _**Namespace Authentication**_: API Keys can be used as the Namespace authentication mechanism for executing and managing workflows from the SDK and Temporal CLI as an alternative to using mTLS-based authentication.
  ...
  - If you created a Temporal Account using the Self Serve Signup feature, API Keys are available for Namespace Authentication.
  - If you have not created a Temporal Account using the Self Serve Signup feature, please reach out to Temporal to participate in the Pre Release.
...
:::note
Deleting a key removes the ability for that API Key to authenticate into Temporal Cloud. Be aware that if you delete an API Key for a running WF, that WF will fail until a new API Key secret is created and configured.
:::
