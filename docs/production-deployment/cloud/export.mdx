...
Workflow History Export does for Temporal Cloud what [Archival](/self-hosted-guide/archival) does for self-hosted Temporal Server. Archival is not available in Temporal Cloud.

Export runs hourly, 10 minutes after the hour. Individual export time varies: each closed Workflow is expected to be found in the exported file within one to four hours.
