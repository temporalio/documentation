...
UpdateAccount permission (Account Owner and higher roles) is required to view an Audit Log via UI, use the API, or to configure an Audit Log sink.
...
Audit logs in Temporal Cloud are being updated beginning October 2025. This update will include several new features such as a UI to explore audit logs and an API to programmatically query audit logs. 

As part of the work to add these features, we are starting the deprecation notice period for the several fields in audit logs. Below are the fields that will be deprecated and the recommended field to use in place of those that are being deprecated.

user_email (duplicated by principal.name for principals of type ‘user’, other principal types do not have associated emails)

level (duplicated by status)

caller_ip_address (replaced by x_forwarded_for to be more accurate)

details (replaced by raw_details that includes request details)

category (this is no longer used)

These deprecated fields are planned for removal on or after April 1 2026.
...
[
  {
    "$typeName": "temporal.api.cloud.auditlog.v1.LogRecord",
    "operation": "UserLogin",
    "status": "OK",
    "version": 2,
    ...
    "principal": {
      "$typeName": "temporal.api.cloud.auditlog.v1.Principal",
      "type": "user",
      "id": "",
      "name": "user@email.com",
      "apiKeyId": ""
...
An Audit Log sink may be deleted.
...
Audit logs are accessible for the past 30 days using a query API for an Audit Log.
