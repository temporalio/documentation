...
- enabling Activity cancellation without the need to implement bespoke heartbeat logic
- identifying a Worker with high CPU usage fom the Server point of view
...
If you wish to disable Worker heartbeating (features above will not work with this feature disabled) or set heartbeating to be more frequent than every 60 seconds, set the configuration relevant to your SDK.
...
```
Set Worker heartbeats to every 30 seconds

```
runtime = Runtime(
    telemetry=TelemetryConfig(),
    worker_heartbeat_interval=timedelta(seconds=30),
)
```
After setting the configuration, then connect to Temporal Server
