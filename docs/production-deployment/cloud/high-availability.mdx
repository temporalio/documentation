...
* <span style={{ backgroundColor: 'rgb(255, 255, 102)', fontWeight: 'bold' }}>Question: I am using lower-case "high availability" (no hyphen) and lowercase namespace as the original doc did. Is this correct?</span>
* <span style={{ backgroundColor: 'rgb(255, 255, 102)', fontWeight: 'bold' }}>Summary at the end of intro accurate?</span>

Temporal Cloud's high availability namespace feature adds push-button multi-region availability for your Temporal Service.
This feature enables you to choose a standby region for a Temporal Namespace.
The service handles the complexity of replicating data from the active region to the standby region and transfering control to the standby region when needed.

This means your standby region is available and in sync with the active region.
It's always ready to take over in the event of network service and performance issues in the active region.
For these reasons, high availability provides Temporal Cloud's highest levels of regional service reliability.

This feature guide covers the following topics:

- [Why choose high availability?](/cloud/high-availability#high-availability-intro)
- [Understanding failovers](/cloud/high-availability#failovers)
...
:::info
- Customers who wish to test the set-up of their high availability namespace can manually trigger failovers using the Web UI page, the 'tcld' CLI utility, or code their own solutions using our APIs.
...
Replication lag means a [forced failover](/cloud/high-availability#forced-failover) might cause workflows to rollback in progress or cause recently started Workflows to be absent until the region recovers.
Temporal event versioning and [conflict resolution mechanisms](/cloud/high-availability#conflict-resolution) ensure that the eventual history of the Workflow can be replayed by the SDK.
...
Events not yet replicated due to [replication lag](/cloud/high-availability#replication-lag) will undergo [conflict resolution](/cloud/high-availability#conflict-resolution) upon reaching the new active region.
...
See the sections on [triggering a failover](/cloud/high-availability#triggering-failovers), [Worker deployment](/cloud/high-availability#worker-deployment), and [routing](/cloud/high-availability#routing) for more information.
...
* Workflows in a multi-region namespace can only be updated and mutated in the active region.
...
High availability provides an “all-active” experience for Temporal users.
...
When a Workflow Execution is updated in a new region following a failover, events from the previous cluster arriving after the failover cannot be directly applied.
...
<span style={{ backgroundColor: 'rgb(255, 255, 102)', fontWeight: 'bold' }}>I did not include the pricing section; Should it be here?</span>
...
Use the following capabilities to create, upgrade, and discontinue your high availability namespaces.
...
Disabling high availability removes the multi-region availability and automatic failover features that provide Temporal's highest level of regional service recovery.
...
* If you cease services in the middle of the month, your namespace removes high availability features within 1 business day.
* Temporal won't retain replicated data in the standby region once high availability has been disabled.
* After disabling high availability, a wait time of 7 days applies before we can re-enable the feature for a given namespace.
...
After enabling the high availability, you don't need to further configure your Workers.
...
More details are available in the [Routing](/cloud/high-availability#routing) section below.
...
Routing is completely transparent to high availability customers unless they use PrivateLink.
...
<span style={{ backgroundColor: 'rgb(255, 255, 102)', fontWeight: 'bold' }}>Add TTL definition?</span>
Using a CNAME record also allows us to support a lower TTL for the record (configured at 15 seconds).
