...
- For each cluster, provide the server configuration by running `temporal operator cluster describe --address <frontend:7233>` (see [notes](#alternative-commands-for-versions-1281-and-prior) for Temporal server version 1.28.1 and prior).
...
- Option 2 - Deploy one set of Temporal clients and switch the namespace endpoint during migration. It is recommended that you test your worker configuration prior to switching over
  to ensure that there is no disruption of work workflow execution.
