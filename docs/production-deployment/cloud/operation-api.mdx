...
The Temporal Cloud Operations API, or the Cloud Ops API, is an open source, public [gRPC API](https://github.com/temporalio/cloud-api/tree/main) and [HTTP API](https://rest.saas-api.tmprl.cloud/docs/httpapi.html#description/introduction) for programmatically managing Temporal Cloud control plane resources, including [Namespaces](/cloud/namespaces), [Users](/cloud/users), [Service Accounts](/cloud/service-accounts), [API keys](/cloud/api-keys), and others. The Temporal Cloud [Terraform Provider](/production-deployment/cloud/terraform-provider), [tcld CLI](/cloud/tcld), and Web UI all use the Cloud Ops API.

## Develop applications with the Cloud Ops API

You can use the gRPC API or the HTTP API depending on how you need to integrate with your platform.

### Prerequisites

These prerequisites are required for using either gPRC or HTTP.
...
[The HTTP API](https://rest.saas-api.tmprl.cloud/docs/httpapi.html#description/introduction) supports the same operations as the existing [gRPC API](#using-grpc), but it's usable via standard HTTP methods and authentication. This may be a more convenient option if you are writing automation scripts for CI/CD or you can't use gPRC due to network policies, proxies, tooling gaps, or language/runtime constraints. Since it's standard HTTP, it's language agnostic giving you the ability to run cloud operations consistently.

**Note:** This *does not* allow interaction with individual Workflows or Activities via HTTP.

#### Use cases

Some common reasons you might use the HTTP API are to manage Namespaces and users. You can use the HTTP API to:

- [Provision Namespaces](https://rest.saas-api.tmprl.cloud/docs/httpapi.html#tag/namespaces/post/cloudnamespaces) per environment or tenant via pipelines.
- Bootstrap new projects by [creating users](https://rest.saas-api.tmprl.cloud/docs/httpapi.html#tag/users/post/cloudusers), [assigning roles](https://rest.saas-api.tmprl.cloud/docs/httpapi.html#tag/users/post/cloudusers.body.spec), and [creating Namespaces](https://rest.saas-api.tmprl.cloud/docs/httpapi.html#tag/namespaces/post/cloudnamespaces) via custom scripts.
- [Rotate service account keys](https://rest.saas-api.tmprl.cloud/docs/httpapi.html#tag/service-accounts) on a schedule with a job.
- [Audit and report access across orgs](https://rest.saas-api.tmprl.cloud/docs/httpapi.html#tag/groups/get/clouduser-groupsgroupid) with scheduled HTTP requests.
