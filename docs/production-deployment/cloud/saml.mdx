...
1. In the **Attributes & Claims** section, select **Edit**. Configure the following settings:

   - Set **Unique User Identifier (NameID)** to `user.userprincipalname`
   - Set the **NameID format** to `emailAddress`

   Ensure the following claims are present:

   - `email`
   - `name`

   Temporal requires the user's full email address to identify the user. Authentication succeeds only if the email
   address is included in the SAML assertion.
