...
Tags are key-value pairs that can be attached to namespaces in Temporal Cloud to help operators organize, track, and manage namespaces more easily.

### Tag Structure and Limits

- Each namespace can have a maximum of 10 tags
- Each key must be unique for a given namespace (e.g., a namespace cannot have both `team:foo` and `team:bar`)
- Keys and values must be 1-63 characters in length
- Allowed characters: lowercase letters (`a-z`), numbers (`0-9`), spaces representable in UTF-8, and the following characters: `+ - = . _ : / @`
- Tags must be lowercase (requests with uppercase will fail)
...
- Only [**Account Admins** and **Account Owners**](/cloud/users#account-level-roles) can edit tags
- All users with access to a namespace can view its tags

### tcld

```bash
# Add/update or remove existing tags 
tcld namespace update-tags --namespace testns.acct --tag-to-add "key1=value1" --tag-to-add "key2=value2" --tag-to-remove "key3"

# Fetch the namespace object (which contains a map of the tags)
...
```hcl
# Simple example
resource "temporalcloud_namespace_tags" "basic_example" {
...
# Example with namespace resource
...
resource "temporalcloud_namespace_tags" "example_tags" {
