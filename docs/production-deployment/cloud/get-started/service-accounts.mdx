...
For example, an Account Developer with Namespace Admin for `test_ns` can create a Namespace-scoped Service Account scoped to `test_ns`.
...
```
tcld service-account created-scoped -n "test_scoped_sa" --np "test_ns=Admin"
```

This example creates a Namespace Scoped Service Account for the Namespace `test_ns`, named `"sa_scoped_test"`, with an `Admin` Namespace Permission.
