...
### 1. Generate a new end-entity certificate
...
### 2. Deploy the new certificate to the Namespace

- **Verify Connectivity**: Test the new certificate to confirm clients can connect to the Namespace without issues.
- **Update Filters**: If you use certificate filters, add a new filter for the new certificate's attributes (CN, O, OU, or SAN).
  See [How to manage certificate filters in Temporal Cloud](#manage-certificate-filters) for detailed steps.

### 3. Update Workers and Clients

- **Update Clients**: [Configure all clients](https://docs.temporal.io/cloud/certificates#configure-clients-to-use-client-certificates)
  (Temporal CLI, SDKs, or Workers) to use the new certificate and private key alongside the compromised key.
  Update the client configuration as described in [Configure clients to use client certificates](https://docs.temporal.io/cloud/certificates#configure-clients-to-use-client-certificates).

### 4. Revoke the compromised certificate
...
### 5. Confirm Workers and Clients are using the correct certificate

- **Monitor Workers:** Ensure that all Workers and Clients are working properly with the new certificate.

### 6. Remove the compromised certificate from Workers

- **Update Clients**: [Configure all clients](https://docs.temporal.io/cloud/certificates#configure-clients-to-use-client-certificates)
  (Temporal CLI, SDKs, or Workers) to remove the compromised certificate and use only the new certificate and private key.
  Update the client configuration as described in [Configure clients to use client certificates](https://docs.temporal.io/cloud/certificates#configure-clients-to-use-client-certificates).
