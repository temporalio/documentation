...
Temporal does not support or check certificate revocation lists (CRLs). Customers are expected to keep their certifications up to date.
...
- **Create the new certificate**: Before you revoke the compromised certificate, you should create the new certificate
  or update with a new CA used for the Namespace.
...
- **Update Clients**: [Configure all clients](https://docs.temporal.io/cloud/certificates#configure-clients-to-use-client-certificates) (Temporal CLI, SDKs, or Workers) to use the new certificate and private key.
...
- **Monitor Namespace activity**: After implementing the changes, monitor your Namespace for unauthorized access attempts or unusual activity in audit logs.
