...
- _**Cloud operations automation**_: API keys work with Temporal Cloud operational tools, including
  ...
  [the Terraform provider](/production-deployment/cloud/terraform-provider). Use them to manage your Temporal Cloud
...
Use the gRPC Namespace endpoint: `<namespace>.<account>.tmprl.cloud:7233`. This allows automated failover without
needing to configure Worker with the regional endpoints of both the primary Namespace and the replica. In addition,
using the Namespace endpoint also reduces the unavailability window during a failover event.
