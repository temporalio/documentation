...
- The version in the Namespace belongs to this Temporal Service, i.e.
  `(version in namespace) % (shared version increment) == (this Temporal Service's initial version)`
...
3. `initialFailoverVersion` in each Temporal Service has to assign a different value.
