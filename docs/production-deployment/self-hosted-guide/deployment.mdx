...
The Temporal Server is a standalone Go application that can be [imported](/references/server-options) into another project as a library.
This is useful for testing, development, and building customized servers.

You can pass custom plugins or other customizations through the [Server Options](/references/server-options), then build and run a binary that contains your customizations.

See [Embedded server](/self-hosted-guide/embedded-server) for details on running Temporal in-process with SQLite, or the [samples-server repository](https://github.com/temporalio/samples-server) for examples of server extensibility (authorization, metrics, TLS).
