...
This page provides instructions for deploying a Temporal Service for production. If you are deploying a Temporal Service
for development, you can use the Temporal CLI to [start a local Temporal Service](/cli/start-dev-server).
...
3. Run the `docker compose up` command. This uses the default configuration from the `docker-compose.yaml` file, which
   ...
   The Temporal Service will be available at `http://localhost:8233`. The Temporal Web UI will be available at
   `http://localhost:8080`.
...
4. (Optional) Review
   ...
   try them out. The configurations include different databases, different visibility stores, and TLS settings.
...
In Docker, you could run each service in its own container, using the `SERVICES` flag to specify the service. Refer to
the [Temporal Server Docker image documentation](https://hub.docker.com/r/temporalio/server) for the environment
variables supported by the Temporal Docker images.
...
The `temporalio/server` image includes only production-essential components. It does not have the `temporal` CLI,
`dockerize`, `curl`, or `bash` tools.
...
When deploying with Helm charts (version 0.73.1 or later), the following configuration options control the dockerize to
sprig migration:
...
- **Required Helm chart version:**
  [temporal-0.73.1](https://github.com/temporalio/helm-charts/releases/tag/temporal-0.73.1) **or later**
