...
description: Create or rename custom Search Attributes with the Temporal CLI on Temporal Cloud; contact Support to delete them. Use Temporal CLI to create or remove them when self-hosting.
...
To manage custom Search Attributes on Temporal Cloud, use the [Temporal CLI](/cli/command-reference/cloud/namespace#search-attribute).
...
[`temporal cloud namespace search-attribute create`](/cli/command-reference/cloud/namespace#search-attribute-create).
For example, to add a custom Search Attribute named `CustomSA` of type `Keyword` to your Namespace, run:

```
temporal cloud namespace search-attribute create \
