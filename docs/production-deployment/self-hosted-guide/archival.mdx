...
Archival is supported in [Multi-region Namespaces](/namespaces#multi-region-namespace) (Namespaces that span multiple clusters).
When Archival is running in a Multi-region Namespace, it first runs on the active cluster; later it runs on the standby cluster. Before archiving, a history check is done to see what has been previously archived.
