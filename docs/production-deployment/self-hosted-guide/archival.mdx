...
sidebar_position: 9
description: Learn how to setup the self-hosted Temporal Service Archival feature.
...
Workflow Execution Event Histories are backed up after the [Retention Period](/temporal-service#retention-period) is reached.
...
Archival is supported in [Global Namespaces](/namespaces#global-namespace) (Namespaces that span multiple temporal-service).
When Archival is running in a Global Namespace, it first runs on the active Temporal Service; later it runs on the standby Temporal Service. Before archiving, a history check is done to see what has been previously archived.
