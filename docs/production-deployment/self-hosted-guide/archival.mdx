...
Workflow Execution Event Histories are backed up after the [Retention Period](/clusters#retention-period) is reached.
...
- **Provider:** Location where the data should be archived.
  Supported providers are S3, GCloud, and the local file system.
- **URI:** Specifies which provider should be used.
  The system uses the URI schema and path to make the determination.
...
- **Local file system**: The [filestore archiver](https://github.com/temporalio/temporal/tree/master/common/archiver/filestore) archives data in the file system of the Temporal server's host.
  In the case of [temporal helm-charts](https://github.com/temporalio/helm-charts), the archive data is stored in the `history` pod.
  APIs do not function with the filestore archive.
...
Archival is supported in a [Multi-region Namespace](/namespaces#multi-region-namespace) (Namespaces that span multiple Temporal Clusters).
When Archival is running in a Multi-region Namespace, it first runs on the active Temporal Cluster; later it runs on the standby Temporal Cluster.
Before archiving, a history check is done to see what has been previously archived.
...
Many common errors that exist between archivers are defined in [common/archiver/constants.go](https://github.com/temporalio/temporal/blob/master/common/archiver/constants.go).
