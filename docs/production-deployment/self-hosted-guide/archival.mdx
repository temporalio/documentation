...
- **Which provider to use:** S3, GCloud, local file system, or custom.
...
Configure Archival in [`config/development.yaml`](https://github.com/temporalio/temporal/blob/main/config/development.yaml#L93):
...
If you don't specify an Archival URI during Namespace creation, the Namespace uses `namespaceDefaults.archival.history.URI` from `config/development.yaml`.
...
For retention maximums, check [Temporal Service Retention Period limits](/temporal-service/temporal-server#retention-period) for your server version.
