...
This guide will provide an example of configuring [Prometheus](https://prometheus.io/) and [Grafana](https://grafana.com/) to work with the observability metrics emitted from Temporal SDKs.
...
This example shows a response the server metrics endpoint, provided by the Temporal dev server, and two SDK metrics endpoints, as defined in the Prometheus configuration.
...
We provide community-driven Grafana dashboards that can be used for monitoring Tempoarl Server and SDK metrics in a [dashboards](https://github.com/temporalio/dashboards/) repo.
