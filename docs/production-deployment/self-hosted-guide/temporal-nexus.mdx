...
:::warning Abrupt region loss

Replication is asynchronous, so any state not replicated before the loss is lost.
A Namespace still active on the lost Cluster stays stuck until it's failed over to
a surviving Cluster.
