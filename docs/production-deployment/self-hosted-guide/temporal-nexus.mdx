...
A Nexus Operation started before a Cluster failover completes on the new active
...
sequenceDiagram
    ...
    haB-xcaB: completion attempt fails (retryable)
    haB->>caB: retry delivers the completion
