...
:::note Abrupt region loss

Replication is asynchronous, so state not replicated before the loss is lost. A
Namespace still active on the lost Cluster resumes once it's failed over to a
surviving Cluster.
