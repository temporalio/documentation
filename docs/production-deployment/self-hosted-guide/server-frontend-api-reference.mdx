...
The Temporal Server includes a native HTTP API that translates HTTP requests to gRPC calls.
The HTTP API runs on a separate port from the gRPC API (7233) and must be explicitly enabled.

:::note

The HTTP API is still considered unstable and may have breaking changes.

:::

### Enable the HTTP API

To enable the HTTP API on a specific port with the dev server, use the `--http-port` flag:

```sh
temporal server start-dev --http-port 7243
```

For self-hosted deployments, set the `httpPort` in your frontend service configuration:
