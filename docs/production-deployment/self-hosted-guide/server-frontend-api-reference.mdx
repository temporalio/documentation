...
description:
  Easily interact with the Temporal Server via Client SDKs or CLI, or use the gRPC and HTTP APIs for Workflow
  operations. Access code examples and API docs at api-docs.temporal.io.
...
The Temporal Server includes a native HTTP API that translates HTTP requests to gRPC calls. The HTTP API runs on a
separate port from the gRPC API (7233) and must be explicitly enabled.

### Enable the HTTP API

To enable the HTTP API on a specific port with the local development server, use the `--http-port` flag:

```sh
temporal server start-dev --http-port 7243
```

For self-hosted deployments, set [`httpPort`](/references/configuration#rpc) in your frontend service configuration:
...
With the HTTP API enabled, the OpenAPI specs are available at:

- [`localhost:7243/openapi.yaml`](http://localhost:7243/openapi.yaml) — OpenAPI v3 (YAML)
- [`localhost:7243/swagger.json`](http://localhost:7243/swagger.json) — OpenAPI v2 / Swagger (JSON)

### Make a request

```sh
curl localhost:7243/api/v1/namespaces/default/workflows
```
