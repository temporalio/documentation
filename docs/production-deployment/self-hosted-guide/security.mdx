...
For information about Temporal Cloud security features, see our [Cloud security page](/evaluate/cloud/security)
:::
...
`GetToken` is called on every outbound cross-cluster RPC.
...
The Temporal Service does not cache the returned token.
Implementations are responsible for caching and refresh.
A provider that fetches tokens from an external service must cache internally, otherwise every outbound RPC triggers a network round-trip to that service.
OAuth 2.0 client libraries typically handle this caching, and a `TokenProvider` can often be a thin wrapper around one.
