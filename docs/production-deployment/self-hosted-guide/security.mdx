...
Configure your `Authorizer` using the [`temporal.WithAuthorizer`](/references/server-options#withauthorizer) and your `ClaimMapper` using the
[`temporal.WithClaimMapper`](/references/server-options#withclaimmapper) server options.
