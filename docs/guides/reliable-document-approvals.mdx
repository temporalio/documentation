...
The Workflow Definition must be deterministic because Temporal uses *history replay* to reconstruct state after Worker restarts. If the code generates different commands on replay, the SDK raises a `NondeterminismError`.
...
- A running Temporal Server — either the [Temporal CLI](/cli) development server (`temporal server start-dev`) or [Temporal Cloud](https://temporal.io/cloud)
...
- [Signals](/develop/python/workflows/message-passing#signals) — the mechanism for sending data to a running Workflow
- [Queries](/develop/python/workflows/message-passing#queries) — the mechanism for reading Workflow state without affecting execution
- [Workers](/workers) — the processes that execute Workflow and Activity code
- [Task Queues](/task-queue) — the named queues that route work to Workers
