...
Do not enable [Worker Versioning](/production-deployment/worker-deployments/worker-versioning#definition) for worker-specific Task Queues — versioning is not needed and will quickly exhaust the [Task Queue limit](/cloud/limits#max-task-queues-in-deployment-version-limits).
