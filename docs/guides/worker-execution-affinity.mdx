...
Task Queues in Temporal are dynamically created when first referenced. With this pattern a unique Task Queue is created per Worker instance (for example, `file-processing-abc123`) that only that Worker polls.
...
- **Go SDK** has a built-in [Worker Sessions API](/develop/go/workers/sessions) that handles Worker-specific routing automatically
...
**Recommended practice:** Set a short `heartbeat_timeout` (for example, 30s) to detect crashes quickly, and a short `schedule_to_start_timeout` (for example, 1m) to stop waiting on dead queues.

**Recommended practice:** Set appropriate `schedule_to_start_timeout` values to detect Worker failures quickly (for example, 5 minutes for file processing).
...
:::caution Worker Versioning

Do not enable [Worker Versioning](/production-deployment/worker-deployments/worker-versioning#definition) for worker-specific Task Queues — it will unnecessarily exhaust the [per-version Task Queue limit](/evaluate/cloud/limits#max-task-queues-in-deployment-version-limits). A per-worker Task Queue's lifetime is tied to the Worker itself. When the Worker is updated to a new version, it gets a new Task Queue and the old one is gone. So versioning this queue does not add any value.
...
- **Worker scaling:** When scaling down, ensure Workers complete in-progress Workflows before [termination](/encyclopedia/workers/worker-shutdown)
...
**Deployment guidance:**
- Deploy multiple instances of this Worker (for example, 5-10 instances)
...
1. **Heartbeat Timeout** (fastest): Activities periodically send heartbeats to signal they're alive. If a Worker crashes mid-execution, no Heartbeats are sent, and the Activity fails within seconds (for example, 30s) rather than waiting for the full Task duration.

2. **Schedule-to-Start Timeout** (medium): If a Worker crashes before picking up a Task from its queue, this timeout detects it within minutes (for example, 5 min) instead of waiting for the full Start-To-Close Timeout. This is critical for identifying dead Workers early.
...
2. **Performance improvement:** Reduced execution time by 80%+ for Workflows processing large files (for example, 10GB video file no longer transferred between Workers)
...
- [Temporal Documentation - Task Routing](/task-routing)
