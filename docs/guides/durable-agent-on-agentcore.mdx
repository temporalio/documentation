...
The sample sets `MODEL_NAME` to `nova-lite` and maps it to the Bedrock model identifier `amazon.nova-lite-v1:0` when it
configures `StrandsPlugin`. The Workflow refers to the registered model by name, so model construction and AWS access
remain in the Worker process.
