...
`TemporalAgent` adapts the Strands agent loop to run in Workflow code. The
[Temporal Strands plugin](/develop/python/integrations/strands-agents) schedules each model call as an Activity.
`activity_as_tool` makes `execute_code` another Activity when the model selects that tool.
...
`AGENT_RUNTIME_ARN` ends with `/runtime/<runtime-id>`. It identifies the Runtime resource. The IAM role in Step 4 uses
