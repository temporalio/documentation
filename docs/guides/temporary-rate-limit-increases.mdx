...
**Note:** Latency to the caller can be further reduced via [the Early Return pattern paired with an SDK call to update-with-start](/sending-messages#update-with-start).
...
- [Temporal Capacity Modes](/cloud/capacity-modes)
- [Temporal Best Practices](/best-practices)
