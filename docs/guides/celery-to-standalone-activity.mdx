...
[**Standalone Activities**](/standalone-activity) are Temporal's [job queue](/evaluate/features/job-queue). A Standalone Activity is an Activity you start directly from a Temporal Client, without wrapping it in a Workflow. You get Temporal's durability, retries, timeouts, and visibility for an individual unit of work, which maps almost one-to-one onto a Celery task. Because there is no Workflow to run a single Activity, Standalone Activities also use fewer resources than orchestrating one Activity through a Workflow.
...
- The Temporal Python SDK, version 1.33.0 or higher (installed in Step 2).
- The Temporal CLI, version 1.9.1 or higher (installed in Step 2).
...
Install the Temporal Python SDK (version 1.33.0 or higher) with `pip`:

```bash
pip install "temporalio>=1.33.0"
```

Next, install the Temporal CLI (version 1.9.1 or higher). On macOS or Linux with [Homebrew](https://brew.sh/), run:
...
Verify the CLI version, since Standalone Activities require 1.9.1 or higher:

```bash
temporal --version
```

Confirm the printed version is at least 1.9.1. With the tools installed, you can start a local Temporal Service.
...
async def main():
    ...
    async for info in client.list_activities(query=query):
        print(f"{info.activity_id} | {info.activity_type} | {info.status.name}")
