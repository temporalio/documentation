...
- Ruby 3.3 or higher installed on your machine.
...
```text
temporal-standalone/
├── welcome_email_helpers.rb # Shared job logic
...
Consider a typical Sidekiq job that sends a welcome email. First, extract its business logic into `welcome_email_helpers.rb`; both the existing Sidekiq job and the new Activity use these same helpers:

```ruby
# welcome_email_helpers.rb
# A tiny stand-in for your user model.
User = Struct.new(:user_id, :email)

module WelcomeEmailHelpers
...
Its `welcome_email_job.rb` can use that shared logic without changing the work it performs:

```ruby
# Sidekiq version — welcome_email_job.rb
require_relative "welcome_email_helpers"

class WelcomeEmailJob
  include Sidekiq::Job
  include WelcomeEmailHelpers
...
The `User` struct and the two mock helpers let these files run end to end without a database or mailer. When you migrate your own job, keep its existing shared business logic (or swap these helpers for your real implementations); the Activity itself does not need special handling. Now you need a Worker to run it.
...
A couple of details are worth calling out. The `id` you provide is a business identifier you choose (an order number, a user identifier); Temporal uses it, along with the ID reuse and conflict policies, to guarantee that concurrent Activities cannot have the same ID, which is a built-in form of deduplication. Every Activity also requires a timeout — `start_to_close_timeout` (in seconds) caps how long one attempt may run, a safety net Sidekiq leaves to you.

## Step 7: Start an Activity in place of `perform_async`

`perform_async` is Sidekiq's core call: it enqueues a job and returns immediately. The Temporal equivalent is `start_activity`, which durably enqueues the Activity and hands back a handle you can use later. In this step, you will start an Activity without blocking on it.
...
The `query` uses the same [List Filter](/list-filter) syntax as Workflow visibility, so you can filter by attributes such as `ActivityType` and `ExecutionStatus` — for example, `"ActivityType = 'SendWelcomeEmail' AND ExecutionStatus = 'Running'"`. Each `info` also exposes `status`, `schedule_time`, `close_time`, and `execution_duration` if you need them. These calls return only Standalone Activities; Activities running inside Workflows are excluded. The Temporal CLI offers the same views with `temporal activity list` and `temporal activity count`.
