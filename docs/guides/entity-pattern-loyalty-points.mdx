...
**Workflow code determinism.** Activities can include non-deterministic code, but the Workflow's code must remain deterministic. Use `workflow.now()` instead of `datetime.now()`, `workflow.random()` instead of `random`, and `workflow.uuid4()` instead of `uuid.uuid4()`. All I/O must be performed in Activities.
...
### Phase 7: Ensure Workflow code determinism
