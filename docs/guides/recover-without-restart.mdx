---
id: recover-without-restart
title: Recover business processes without restarting
description: Build a multi-step business process that pauses when it hits a permanent failure, then resumes from where it stopped without restarting.
...
![A blocked Workflow showing the failure details and a form to send a corrective Signal](assets/recover-without-restart/keep-business-moving-list-view.png)
...
![Architectural comparison between a traditional implementation and the Temporal-based solution](assets/recover-without-restart/keep-business-moving-architectural-comparison.png)
...
Temporal addresses this with [`ApplicationFailure.nonRetryable()`](/references/failures#application-failure), which instructs the SDK to skip the Retry Policy and propagate the error to the Workflow code without delay.
The Workflow then has full control over how to handle the failure: it can log the error, update Search Attributes for visibility, and suspend until a [Signal](/develop/typescript/workflows/message-passing#signals) arrives.

Custom [Search Attributes](/search-attribute) provide the mechanism for routing blocked Workflows to the correct resolution resource.
...
Temporal [Queries](/develop/typescript/workflows/message-passing#queries) provide synchronous read access to the current state of any running Workflow.
...
**Note:** This guide uses the TypeScript SDK but the pattern applies to any Temporal SDK.
...
Temporal Activities automatically recover from transient failures — network timeouts, temporary service unavailability, rate limits — through their built-in [Retry Policy](/encyclopedia/retry-policies) and [timeout management](/develop/typescript/activities/timeouts#activity-timeouts).
...
The project includes a single-page operations dashboard at [`public/index.html`](https://github.com/temporal-sa/validated-pattern-keep-business-moving/blob/main/public/index.html) that exercises the web service endpoints you built above. Start it alongside the Worker:
...
![Dashboard showing 3 blocked Workflows and 1 completed](assets/recover-without-restart/keep-business-moving-list-view.png)
...
![Sending a corrective Signal to fix an unknown employer](assets/recover-without-restart/keep-business-moving-fix-employer.png)
...
![Workflow completed after fixing the employer name](assets/recover-without-restart/keep-business-moving-success.png)
...
![Event History in the Temporal Web UI showing the Signal and Activity completions](assets/recover-without-restart/keep-business-moving-event-history.png)
...
- [Temporal Search Attributes](/search-attribute) — Reference for creating and using custom Search Attributes for Workflow visibility.
- [Temporal Visibility](/visibility) — Overview of the Visibility subsystem and List Filter Query syntax.
- [ApplicationFailure Reference](/references/failures#application-failure) — API reference for `ApplicationFailure` including retryable and non-retryable error handling.
