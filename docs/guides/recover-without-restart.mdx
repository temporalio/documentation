...
The Workflow then has full control over how to handle the failure: it can log the error, update Search Attributes for visibility, and suspend until a [Signal](/develop/typescript/message-passing#signals) arrives.

Custom [Search Attributes](/visibility#search-attribute) provide the mechanism for routing blocked Workflows to the correct resolution resource.
...
Temporal [Queries](/develop/typescript/message-passing#queries) provide synchronous read access to the current state of any running Workflow.
...
Temporal Activities automatically recover from transient failures — network timeouts, temporary service unavailability, rate limits — through their built-in [Retry Policy](/encyclopedia/retry-policies) and [timeout management](/develop/typescript/failure-detection#activity-timeouts).
...
- [Temporal Signals](/develop/typescript/message-passing#signals) — Guide to defining and sending Signals to running Workflow Executions.
- [Temporal Queries](/develop/typescript/message-passing#queries) — Guide to defining and handling synchronous read-only Queries on Workflows.
- [Temporal Search Attributes](/visibility#search-attribute) — Reference for creating and using custom Search Attributes for Workflow visibility.
