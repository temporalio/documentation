...
- **Required software and tools**:
    ...
    - Temporal CLI 1.6.2 for the local development server ([install](/cli))
...
- **Required concepts**:
    - Familiarity with Temporal Workflows and Activities ([docs](/workflows)).
    - Child Workflows and parent close policies ([docs](/child-workflows)).
    - Signals and Queries ([docs](/encyclopedia/workflow-message-passing)).
    - Workflow Definition determinism ([docs](/workflow-definition#deterministic-constraints)).
...
sequenceDiagram
    autonumber
    actor App as Application Workflow
    participant Sem as Semaphore (helper)
    participant Permit as PermitSlotWorkflow (child)
    participant Temporal as Temporal Service

    App->>Sem: acquire(lease=10m, backoff=5s)
    Sem->>Temporal: start_child_workflow(id="permit:gpu-pool:2")
    Temporal-->>Sem: WorkflowAlreadyStartedError (slot held)
    Sem->>Temporal: start_child_workflow(id="permit:gpu-pool:3")
    Temporal-->>Sem: ChildWorkflowHandle (pinned to this run)
    Temporal->>Permit: dispatch run(PermitSlotInput)
    Sem-->>App: yield "3"
    Note over App: execute gated work
    App->>Sem: release on context exit
    Sem->>Temporal: signal "release" on the child handle
    Temporal->>Permit: deliver release Signal
    Permit-->>Temporal: complete ("released")
    Note over Permit,Temporal: slot ID becomes reusable
```
...
- [Temporal Python SDK Documentation](/develop/python). SDK reference for Workflows, Signals, child Workflows, and Worker setup.
- [Child Workflow Executions](/child-workflows). How parent and child Workflows interact, including parent close policies.
- [Workflow Message Passing](/encyclopedia/workflow-message-passing). Reference for Signals and Queries.
