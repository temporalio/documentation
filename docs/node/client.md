...
- `workflowId`: Specify unique Workflow id to use when starting, subject to [WorkflowIdReusePolicy](https://nodejs.temporal.io/api/interfaces/client.workflowoptions/#workflowidreusepolicy). If not specified, a UUID is generated, which you can access with `handle.workflowId`. Most users should do just fine with the autogenerated workflowId.
