...
Server commands allow you to start and manage the [Temporal Server](/temporal-service/temporal-server) from the command line.

Currently, `cli` server functionality extends to starting the Server.

## start-dev

The `temporal server start-dev` command starts a local [Temporal Server](/temporal-service/temporal-server).
