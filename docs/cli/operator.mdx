...
description: Efficiently use the Temporal CLI to manage operator actions on Namespaces, Search Attributes, and Temporal Services. Create, update, list, and remove resources.
toc_max_heading_level: 4
keywords:
  - cli reference
  - Temporal Service
  - Temporal Service health
  - Temporal Service list
  - Temporal Service remove
  - Temporal Service upsert
...
Operator commands enable actions on [Namespaces](/namespaces), [Search Attributes](/visibility#search-attribute), and [Temporal Services](/temporal-service).
...
Temporal Service commands enable actions on [Temporal Services](/temporal-service).

Temporal Service commands follow this syntax:
`temporal operator [command] [subcommand] [command options]`.

### describe

The `temporal operator cluster describe` command shows information about the [Temporal Service](/temporal-service).
...
The `temporal operator cluster health` command checks the health of the [Frontend Service](/temporal-service#frontend-service).
A successful execution returns a list of [Temporal Service](/temporal-service) metrics.
...
The `temporal operator cluster list` command prints a list of all remote [Clusters](/temporal-service) on the system.
...
The `temporal operator cluster remove` command removes a remote [Temporal Service](/temporal-service) from the system.
...
The `temporal operator cluster system` command provides information about the system the [Temporal Service](/temporal-service) is running on.
This information can be used to diagnose problems occurring in the [Temporal Server](/temporal-service#temporal-server).
...
The `temporal operator cluster upsert` command allows the user to add or update a remote [Temporal Service](/temporal-service).
...
Namespace commands perform operations on [Namespaces](/namespaces) contained in the [Temporal Service](/temporal-service).
...
The Namespace can be created on the active [Temporal Service](/temporal-service), or any named Temporal Service within the system.
...
Other settings, such as [retention](/temporal-service#retention-period) and [Visibility Archival State](/temporal-service#visibility), can be configured according to the application's needs.
...
- [--active-cluster](/cli/cmd-options#active-temporal-service)

- [--address](/cli/cmd-options#address)

- [--cluster](/cli/cmd-options#active-temporal-service)
...
The `temporal operator namespace list` command lists all [Namespaces](/namespaces) on the [Server](/temporal-service#frontend-service).
...
Namespaces can be assigned a different active [Temporal Service](/temporal-service).
...
Any [Archives](/temporal-service#archival) that were previously enabled or disabled can be changed through this command.
