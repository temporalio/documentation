...
`temporal cloud login` allows you to perform administrative and operational tasks on Temporal Cloud. It does not provide
the CLI with information about the Temporal Service you want to communicate with. Commands such as
`temporal workflow list`, which interact with a Temporal Service, still require the `--address` and `--namespace` flags
as well as API key credentials.
