...
| `--partitions-legacy` | No | **int** Query partitions 1 through `N`. Experimental/Temporary feature. Legacy mode only. | `1` |
...
| `--task-queue-type-legacy` | No | **string-enum** Task Queue type (legacy mode only). Accepted values: workflow, activity. | `workflow` |
...
| `--reachability-type` | No | **string-enum** Reachability filter. `open`: reachable by one or more open workflows. `closed`: reachable by one or more closed workflows. `existing`: reachable by either. New Workflow Executions reachable by a Build ID are always reported. Accepted values: open, closed, existing. | `existing` |
...
| Flag | Required | Description | Default |
|------|----------|-------------|--------|
| `--max-sets` | No | **int** Max return count. Use 1 for default major version. Use 0 for all sets. | `0` |
| `--task-queue`, `-t` | Yes | **string** Task Queue name. |  |
...
|------|----------|-------------|--------|
| `--build-id` | Yes | **string** Target Build ID. |  |
| `--percentage` | No | **int** Traffic percent to send to target Build ID. | `100` |
| `--rule-index`, `-i` | No | **int** Insertion position. Ranges from 0 (insert at start) to count (append). Any number greater than the count is treated as "append". | `0` |
| `--yes`, `-y` | No | **bool** Don't prompt to confirm. |  |
...
| `--percentage` | No | **int** Divert percent of traffic to target Build ID. | `100` |
| `--rule-index`, `-i` | Yes | **int** Position of the assignment rule to be replaced. Requests for invalid indices will fail. |  |
| `--yes`, `-y` | No | **bool** Don't prompt to confirm. |  |
