...
The `temporal workflow signal` command is used to send a [Signal](/sending-messages#sending-signals) to a [Workflow Execution](/workflow-execution) by [Workflow Id](/workflows#workflow-id) or [List Filter](/list-filter).
