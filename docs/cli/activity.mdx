...
The `temporal activity update-options` command updates options for an [Activity Execution](/activities#activity-execution).
The Activity must already be running on a valid [Workflow](/workflows).
...
For long-running activities, activities in paused state will send a cancellation with "activity_paused" set to 'true' in response to 'RecordActivityTaskHeartbeat'.
...
The `temporal activity reset` command resets an [Activity Execution](/activities#activity-execution) specified by its ID or type.
