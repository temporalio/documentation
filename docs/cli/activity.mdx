...
If the activity may be executing (i.e. it has not yet timed out), the reset will occur the next time it times out.
If it has failed or timed out and is waiting for a retry, the reset will apply immediately.
