...
Activity ID to unpause. Can only be used without --query. Either `activity-id` or `activity-type` must be provided, but not both.

**--activity-type**, **-g** _string_

Activity Type to unpause. Either `activity-id` or `activity-type` must be provided, but not both.
