...
The Temporal CLI, evolving from its precursor [tctl](/tctl-v1), serves as a command-line interface that provides developers with direct access to a Temporal Service.
...
The Temporal CLI includes a distribution of a Temporal Service, comprised of the [Temporal Server](/temporal-service#temporal-server), SQLite persistence, and the [Temporal Web UI](/web-ui).
...
For a full list of available commands, run `temporal` or see [Available commands](#command-set).
