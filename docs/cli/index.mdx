...
The Temporal CLI, evolving from its precursor `tctl`, provides developers with direct CLI access to a Temporal Service.
...
The Temporal CLI encapsulates complex operations into simpler command-line inputs, making it an indispensable component of a developer’s toolkit when working with the Temporal ecosystem.
...
When upgrading from `tctl` to the Temporal CLI, make sure to update your environment variables and use updated commands.
For details, see [CLI release notes](https://github.com/temporalio/cli/releases/).

:::

The Temporal CLI includes a distribution of a Temporal Cluster.
...
```shell
temporal operator namespace create -n foo
```
...
Configure system environment with the Temporal CLI environmental variables.
The following table lists and describes the environment variables you can set for the Temporal CLI.
...
1. Make sure there is a place to store custom completions for Fish:

   ```fish
   mkdir -p ~/.config/fish/completions
   ```

2. Arrange for the completions to be loaded when needed (note: the file name must be `temporal.fish` or the completions will not be found):

   ```fish
   echo 'eval "$(temporal completion fish)"' >~/.config/fish/completions/temporal.fish
   ```

3. Restart your shell to pick up the new completions.
