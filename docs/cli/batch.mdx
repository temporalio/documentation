...
Batch commands change multiple [Workflow Executions](/workflows#workflow-execution) by providing a [List Filter](/temporal-service#visibility) and the type of Batch Job to execute.
...
Batch Jobs can be returned for an entire Temporal Service or a single Namespace.
