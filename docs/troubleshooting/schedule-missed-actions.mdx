...
description: Diagnose why a Schedule did not fire by alerting on the missed catchup window metric, then narrow down to the affected Schedule with ListSchedules and DescribeSchedule.
...
## Alert on missed catchup window {/* #alert-on-missed-catchup-window */}
...
## Investigate which Schedule missed an Action {/* #investigate-which-schedule-missed-an-action */}

Once the alert fires, narrow your search down to the affected Schedule in two steps.

### 1. List Schedules in the Namespace {/* #1-list-schedules-in-the-namespace */}
...
### 2. Describe each Schedule {/* #2-describe-each-schedule */}
