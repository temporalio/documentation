...
[Worker Controller Instance (WCI)](/serverless-workers#how-invocation-works) may not be detecting Tasks on the Task
Queue. The WCI is a system Workflow that runs in the same Namespace as your Worker Deployment. One WCI Workflow runs per
Worker Deployment Version that has a compute provider configured. The WCI monitors bound Task Queues and invokes the
Lambda function when Tasks arrive.
