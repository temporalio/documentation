...
description: "Temporal BlobSizeLimitError occurs when the size of a blob exceeds the 2MB single request limit or 4MB Event History transaction limit. Causes performance issues and excessive resource usage. Resolve by reducing blob size, using compression, or breaking down large payloads into smaller batches."
