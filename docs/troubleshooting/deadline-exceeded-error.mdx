...
OSS users can verify that the Frontend Service is connected and running by using `temporal`.

```
temporal operator --address frontendAddress:frontendPort cluster health
```
...
```
temporal operator --address [SERVER_ADDRESS] cluster health
```
