...
description: "Troubleshoot the `Failed reaching server: last connection error` caused by an expired TLS certificate with this guide, which covers verification, renewal, and update of TLS certification, as well as self-signed certificate management and additional resources."
...
```command
temporal namespace describe --namespace <namespace_id> --address <namespace_grpc_endpoint> --tls-cert-path <path-to-mTLS-pem-file> --tls-key-path <path-to-mTLS-key-file>
```
