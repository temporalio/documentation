...
If you are using a self-signed certificate, run the following `temporal` command:

```command
temporal --namespace <namespace_id> --address <namespace_grpc_endpoint> --tls-cert-path <path-to-mTLS-pem-file> --tls-key-path <path-to-mTLS-key-file> namespace describe
```
