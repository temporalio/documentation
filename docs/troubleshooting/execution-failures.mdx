...
1. **Identify the affected Executions.** Find the Workflow Id and Run Id in Worker logs because the metric does not carry a Workflow Id. In the Temporal UI, you can also find affected Executions by querying the `TemporalReportedProblems` Search Attribute, which the Temporal Service sets on Executions experiencing repeated Workflow Task failures.
1. **Find and fix what made the completion large.** The limit applies to the sum. Identify what the Workflow produced in a single step, then change the Workflow to produce less in each Workflow Task:
   - **A large fan-out.** Scheduling many Activities, Child Workflows, or Timers in one Workflow Task can make the completion too large. Break the fan-out into smaller batches across multiple Workflow Tasks.
   - **Accumulated Signals or Updates.** Handling a large number of buffered Signals or Updates in one Workflow Task can make the completion too large. Limit how many senders send at once, or batch their requests.
   - **Large Payloads in aggregate.** Payloads can each be under the Payload limit but make the completion too large when combined. Store the data in [External Storage](/external-storage) and pass a reference through Event History instead.
