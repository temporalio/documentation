...
- This guide assumes you are already familiar with the Vercel AI SDK. If you aren't, please refer to the
...
- Ensure you have set up your local development environment by following the
  ...
  done, leave the Temporal Development Server running.
...
1. Create a connection to the MCP servers using the `experimental_createMCPClient` function from the `@ai-sdk/mcp`
   ...
   The example above uses `StdioClientTransport` for local development and testing. For production deployments, use
   `StreamableHTTPClientTransport` to connect to a centralized MCP server over HTTP. You can register multiple MCP
   servers by providing multiple factory functions in `mcpClientFactories`.
