...
`Connection` class from `@temporalio/client` and use
...
configuration to the `Connection.connect` method to create a Temporal Client using the `staging` profile as follows.
After loading the profile, you can also programmatically override specific connection options before creating the
client.
...
a `NativeConnection` object instead of a `Connection` object. The `NativeConnection` class is imported from
