...
Temporal TypeScript SDK support for Standalone Nexus Operations is at
...
To execute a Standalone Nexus Operation, first create a Nexus service client using `client.nexus.createServiceClient()`,
...
Use `handle.result()` to wait until the Operation completes and retrieve its result. This works for both
...
Use `client.nexus.list()` to list Standalone Nexus Operation Executions that match a [List Filter](/list-filter) query.
The result is an async iterator that yields operation metadata entries.

Note that `client.nexus.list()` is called on the base `Client`, not on the Nexus service client.
...
Use `client.nexus.count()` to count Standalone Nexus Operation Executions that match a [List Filter](/list-filter) query.
