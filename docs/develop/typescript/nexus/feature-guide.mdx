...
All calls must complete within the [Nexus request timeout](/evaluate/cloud/limits#nexus-operation-request-timeout).
...
For custom interceptor logic beyond tracing (for example, logging, authorization), see [Nexus interceptor registration](/develop/typescript/workers/interceptors#nexus-interceptor-registration).
