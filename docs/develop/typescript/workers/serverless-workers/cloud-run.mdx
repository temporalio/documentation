---
id: cloud-run
title: Serverless Workers on GCP Cloud Run - TypeScript SDK
sidebar_label: GCP Cloud Run
description: Run a Temporal Worker on a GCP Cloud Run worker pool using the TypeScript SDK.
slug: /develop/typescript/workers/serverless-workers/cloud-run
...
Cloud Run needs no SDK-specific package, unlike [AWS Lambda](/develop/typescript/workers/serverless-workers/aws-lambda), which uses `@temporalio/lambda-worker`.
...
`loadClientConnectConfig()` returns `connectionOptions` and `namespace`. Pass `connectionOptions` to `NativeConnection.connect()` and `namespace` to `Worker.create()`, as shown above.
