...
Two container details are specific to the TypeScript SDK, because its Worker runs on a Rust core rather than on Node.js alone.

The Rust core reads TLS roots from the operating system's certificate store, and the slim Node.js images ship without one.
