...
- If the code has a call to patched, and there is a marker after that spot in the execution, or if there is no marker for that patch ID anywhere in the event history, it returns false.
