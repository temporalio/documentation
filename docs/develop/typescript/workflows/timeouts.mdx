...
- [Fail Workflows deliberately](#workflow-failure)
...
In fact, *any* other exceptions that are raised from your TypeScript code in a Temporal Activity will be converted to an `ApplicationFailure` internally.
...
class InvalidChargeError extends Error {
    constructor(message: string) {
        ...
        Object.setPrototypeOf(this, InvalidChargeError.prototype);
    }
}
...
One reason to use the Temporal `ApplicationFailure` class is because it allows you to set an additional `nonRetryable` parameter.
