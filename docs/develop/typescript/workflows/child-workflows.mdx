...
You should block progress on the Parent Workflow until the [ChildWorkflowExecutionStarted](/references/events#childworkflowexecutionstarted) Event is logged to the Event History to ensure the Child Workflow Execution has started. If the Parent makes the `ExecuteChildWorkflow` call and then immediately completes, the Child Workflow Execution doesn't start. In TypeScript, you can make sure your Child Workflow started by awaiting [`executeChild`](https://typescript.temporal.io/api/namespaces/workflow#executechild).
