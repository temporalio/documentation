...
The `getExternalWorkflowHandle` method helps ensure that Workflow code remains deterministic.
Recall that one aspect of deterministic Workflow code means not directly making network calls from the Workflow.
...
Instead, to communicate between Workflows, we use `getExternalWorkflowHandle` to both ensure that Workflow code remains deterministic and also that these interactions are recorded as Events in the Workflow's Event History.
