...
Recall that one aspect of a deterministic Workflow Definition means not directly making network calls from the Workflow.
...
Instead, to communicate between Workflows, we use `getExternalWorkflowHandle` to both ensure that the Workflow Definition remains deterministic and also that these interactions are recorded as Events in the Workflow's Event History.
...
To use `async` handlers safely, understand what can go wrong.
