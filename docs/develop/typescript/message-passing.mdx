...
// Workflow code, and when calling the Query in Client code.
...
export async function greetingWorkflow(): Promise<string> {
  ...
  wf.setHandler(setLanguage, (newLanguage: Language) => {
...
```typescript
currentLanguage = await handle.query(getLanguage);
```
...
- Send [`WorkflowHandle.startUpdate`](https://typescript.temporal.io/api/interfaces/client.WorkflowHandle/#startUpdate) to receive an [`WorkflowUpdateHandle`](https://typescript.temporal.io/api/interfaces/client.WorkflowUpdateHandle) as soon as the Update is accepted or rejected.
  ...
  - `async` Update handlers normally perform long-running async activities.
...
When you don't have access to the Workflow Definition or it isn't written in Typescript, you can still use non-type safe APIs and dynamic method invocation.
  ...
  wf.setHandler(setLanguageUsingActivity, async (newLanguage) => {
    ...
    if (!(newLanguage in greetings)) {
      ...
      await lock.runExclusive(async () => {
        // Simulate an async call to a greeting service
...
Here are two important use cases for `workflow.condition`:
