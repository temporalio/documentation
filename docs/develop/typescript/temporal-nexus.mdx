...
A Nexus Service handler is defined using the `nexus-rpc`'s [`serviceHandler`](https://nexus-rpc.github.io/sdk-typescript/functions/serviceHandler.html) function. {/* Added _/}
Nexus Service handlers are typically defined in the same Worker as the underlying Temporal primitives they abstract.
A Service handler must provide Operation handlers for each Operation declared by the Service. {/_ Added */}
...
export async function helloCallerWorkflow(
  name: string,
  language: LanguageCode,
): Promise<string> {
...
async function cancellableCallerWorkflow() {
  ...
  const nexusOperationHandle = await cancellableScope.run(async () =>
    nexusClient.startOperation('hello', { name, language }, {
