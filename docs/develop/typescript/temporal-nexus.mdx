...
Use `getClient()` from `@temporalio/nexus` to get the Client connected to the same Namespace as the Worker:

```ts
import { getClient } from '@temporalio/nexus';

export const myServiceHandler = nexus.serviceHandler(myService, {
  signalWorkflow: async (ctx, input: SignalInput): Promise<SignalOutput> => {
    const client = getClient();
    const handle = client.workflow.getHandle(input.workflowId);
    // Signal a Workflow
    await handle.signal('mySignal', input.value);
    // Query a Workflow
    const result = await handle.query<string>('myQuery');
    return { result };
  },
});
```
