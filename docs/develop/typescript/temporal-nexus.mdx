...
- `@temporalio/nexus`'s [`NexusOperationFailure`](https://typescript.temporal.io/temporalio.exceptions.NexusOperationFailure.html): this is the error thrown inside a Workflow when a Nexus operation fails for any reason. Use the `cause` attribute on the exception to access the cause chain.
