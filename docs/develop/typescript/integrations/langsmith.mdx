...
Temporal gives your agent code [durable execution](https://docs.temporal.io/temporal#durable-execution). LangSmith adds
...
In TypeScript the Client and the Worker are configured independently, so add the same plugin instance to your
`Client` as well. This links client-side operations, like starting a Workflow, to the Workflows they trigger.
...
Use the same `projectName` on both the Worker and the Client so their traces land in the same LangSmith project.
