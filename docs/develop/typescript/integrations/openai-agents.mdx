...
Set `modelParams.streamingTopic` on the plugin (in both the Worker and the Client), host a `WorkflowStream` in the Workflow, and run with `{ stream: true }`:
...
The streaming Activity also returns the full event list to the Workflow, which consumes that list instead of the live stream so replay stays deterministic. Subscribe from an external client with `WorkflowStreamClient` from `@temporalio/workflow-streams/client`; see the [`@temporalio/workflow-streams`](https://github.com/temporalio/sdk-typescript/tree/main/contrib/workflow-streams) docs for the subscriber API.
