...
slug: /develop/typescript/data-encryption
...
This section will summarize the difference between a Payload Converter and Payload Codec.
...
When the Data Converter receives a value for conversion, it passes through each Payload Converter in sequence until the converter that handles the data type does the conversion. You will explore more in detail now.
...
A Composite Data Converter can be comprised of custom rules that you created, and it can also
...
The order in which the Payload Converters are applied is important. During
...
Custom Payload Converters can be used to change how application objects get serialized to binary Payload. To handle custom data types that are not natively JSON-serializable (e.g., `BigInt`, `Date`, or binary data), you can create a custom Payload Converter. A Custom Payload Converter is responsible for converting your custom data types to a payload format that Temporal can manage.
...
2. Configure the Data Converter: To send values that are not [JSON-serializable](https://en.wikipedia.org/wiki/JSON#Data_types) like a `BigInt` or `Date`, provide a custom [Data Converter](https://typescript.temporal.io/api/interfaces/common.DataConverter/) to the Client and Worker. By doing this, Temporal knows to use the Custom Converter for serializing and deserializing your Workflow and Activity inputs and outputs.
