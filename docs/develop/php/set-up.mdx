...
<p>Then verify the extension is loaded:</p>
<CodeSnippet language="bash">
php -m | grep grpc
</CodeSnippet>
</>
...
The Temporal PHP SDK requires the gRPC extension for client operations. 
Install it using PECL:

**Note:** After installation, you may need to restart your terminal or web server. 
Use `php --ini` to find your php.ini location if needed.
...
<p>The Temporal PHP SDK includes RoadRunner for running workers. 
You'll create a configuration file for it.</p>
...
Create a `.rr.yaml` file in your project directory - you'll configure it in the next step.
...
### 0. Configure Composer Autoloading

First, configure Composer to autoload your classes. 
Add the following to your `composer.json` file above the "require" section:
