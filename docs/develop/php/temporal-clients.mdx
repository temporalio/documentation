...
When developing in PHP, the Task Queue name is a `string`.
It defaults to `"default"`.
While setting a meaningful Task Queue name is recommended for better observability, you can run Workflows without setting this option.
...
The following example demonstrates how to set the Workflow options and the Task Queue name and build a new Workflow stub:
...
Each Worker listens to one Task Queue.
To set the Task Queue name when creating a Worker:
...
The Task Queue must match the one used by the Client so it can poll for Activity and Workflow Tasks.

:::info

Workers automatically fail any Tasks that are not registered with them that appear on their Task Queue.
A Task failure doesn't cause its associated Workflow Execution to fail.
