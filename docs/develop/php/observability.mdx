...
Logs in replay mode are omitted unless the `enableLoggingInReplay` Worker option is set to true.
...
By default, PHP SDK uses a `StderrLogger` that outputs log messages to the standard error stream.
