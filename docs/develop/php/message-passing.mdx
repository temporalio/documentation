...
A [Workflow Task Failure](/references/failures#errors-in-workflows) causes the server to retry Workflow Tasks indefinitely. What happens to your Update request depends on its stage:
  - If the request has been admitted by the server but not accepted or rejected by the Workflow, you receive a [`WorkflowUpdateResultException`](https://php.temporal.io/classes/Temporal-Exception-Client-WorkflowUpdateResultException.html).
  - If the request has been rejected by the workflow, you receive a  [`WorkflowUpdateException`](https://php.temporal.io/classes/Temporal-Exception-Client-WorkflowUpdateException.html).
