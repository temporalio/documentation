...
[send an Update](#send-update-from-client) that checks whether an already-running Workflow with that ID exists:
...
- WorkflowStub created from [`WorkflowOptions`](https://php.temporal.io/classes/Temporal-Client-WorkflowOptions.html).
...
Same as for [Update Workflow](#send-update-from-client).
...
To wait for the Update result, run the Update with the wait stage set to [`LifecycleStage::StageCompleted`](https://php.temporal.io/classes/Temporal-Client-Update-LifecycleStage.html#enumcase_StageCompleted).
