...
Replay testing takes one or more existing [Workflow Histories](https://docs.temporal.io/workflow-execution#event-history) that ran against a previous version of Workflow code and runs them against your *current* Workflow code, verifying that it is compatible with the provided history.
