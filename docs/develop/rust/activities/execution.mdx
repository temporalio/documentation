...
description: Spawn an Activity Execution from a Workflow with the Temporal Rust SDK, set the required Activity Timeouts, and get the Activity's result.
...
In Rust, calling `.await` on the Activity invocation returns the result. If you need more control (for example, parallel execution), you can create multiple Activity futures and await them selectively.
