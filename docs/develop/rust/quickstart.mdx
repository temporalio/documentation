...
import { CallToAction, SetupSteps, SetupStep, CodeSnippet } from '@site/src/components';
...
- `temporalio-workflow` - Referenced by the code the Workflow macros generate, so it must be a direct dependency even though your own code never names it
