...
`run()` borrows the Worker mutably, so take a shutdown handle before starting it.
Calling the handle initiates shutdown, which stops polling for new Tasks and lets in-flight Tasks finish.
