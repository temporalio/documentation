...
description: Develop a basic Workflow and Activity Definition using the Temporal Ruby SDK, run a Worker Process, and set up Dynamic Workflows and Activities.
...
Temporal Workflows [must be deterministic](https://docs.temporal.io/workflow-definition#deterministic-constraints), which includes
...
Use [`Temporalio::Workflow.logger`](https://ruby-doc.temporal.io/) instead of a standard Ruby `Logger`. The SDK logger automatically suppresses log messages during replay and appends Workflow details to every log entry:

```ruby
Temporalio::Workflow.logger.info("Starting workflow for #{name}")
```

For logger configuration, see [Observability: Logging](/develop/ruby/observability#logging).

#### Random numbers

Use [`Temporalio::Workflow.random`](https://ruby-doc.temporal.io/) to get a deterministic `Random` instance seeded per Workflow Execution. Never use `rand` or `Random.new` directly:
...
Use [`Temporalio::Workflow.now`](https://ruby-doc.temporal.io/) instead of `Time.now`. The SDK returns the time of the last Workflow Task, which is consistent across replays:
...
If your goal is to always take action when something new is happening, exclude `Temporalio::Workflow::Unsafe.replaying_history_events?` instead. Your code will run during read-only operations like queries and update validators.  This is what the SDK's built-in logger and tracing interceptors use internally.
