...
Use [`Temporalio::Workflow::Unsafe.replaying?`](https://ruby-doc.temporal.io/) to guard code that should only run on the first execution, such as emitting metrics or sending external notifications.
