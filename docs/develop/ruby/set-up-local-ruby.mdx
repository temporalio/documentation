...
<SetupStep code={
  <>
    ...
    <p><strong>Command Line (gem install):</strong></p>
    <CodeSnippet language="bash">
...
**Gemfile approach:** This approach is used in Ruby projects that manage dependencies with Bundler. To install it, add the line to your project's Gemfile and run `bundle install` in your terminal. Bundler will download and install the gem and its dependencies.

**Command Line approach:** Install directly using gem install.
...
<p>Download for your architecture:</p>
<ul>
...
<p>Extract the archive and add <code>temporal.exe</code> to your PATH.</p>
...
<p>Extract the archive and add the <code>temporal</code> binary to your PATH by copying it to a directory like <code>/usr/local/bin</code>.</p>
<CodeSnippet language="bash">
temporal server start-dev
</CodeSnippet>
<p><strong>Optional:</strong> To retain data between runs, use:</p>
<CodeSnippet language="bash">
...
<style>
{`.success-section { background: rgba(34, 197, 94, 0.08); border: 1px solid rgba(34, 197, 94, 0.2); border-radius: 0.75rem; padding: 1.25rem; margin: 1.5rem 0; } .success-section h4 { margin-top: 0; margin-bottom: 1rem; color: var(--ifm-color-success-dark); font-weight: 600; display: flex; align-items: center; gap: 0.5rem; } .success-icon { stroke: var(--ifm-color-success-dark); flex-shrink: 0; } .success-section p { margin-bottom: 1rem; font-size: 0.95rem; line-height: 1.5; color: var(--ifm-color-emphasis-800); } .success-section ul { margin-bottom: 0; padding-left: 1.5rem; } .success-section li { margin-bottom: 0.5rem; font-size: 0.95rem; color: var(--ifm-color-emphasis-800); } [data-theme='dark'] .success-section { background: rgba(34, 197, 94, 0.12); border-color: rgba(34, 197, 94, 0.3); }`}
</style>
</>
}>
...
<CallToAction href="/develop/ruby/core-application">
