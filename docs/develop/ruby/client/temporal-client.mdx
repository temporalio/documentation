...
description:
  Connect to a development Temporal Service or Temporal Cloud from Ruby, set a Task Queue and Workflow Id, and get
...
To set a Task Queue in Ruby, specify the `task_queue` keyword argument on `start_workflow` or `execute_workflow`.
...
To set a Workflow Id in Ruby, specify the `id` keyword argument on `start_workflow` or `execute_workflow`.
