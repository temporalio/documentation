...
The observability guide covers the many ways to view the current state of your [Temporal Application](/encyclopedia/architecture/temporal-architecture#client-application).
This includes viewing [Workflow Executions](/workflow-execution) tracked by the [Temporal Platform](/encyclopedia/architecture/temporal-architecture#the-temporal-server), as well as inspecting state at any point during execution.
...
Logging uses the Ruby standard logging APIs. You can find the log levels supported in [their official documentation](https://ruby-doc.org/stdlib-2.4.0/libdoc/logger/rdoc/Logger.html). The Temporal SDK core normally uses `WARN` as its default logging level.
