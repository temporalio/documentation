...
- Fairness counters for the most active fairness keys are preserved across server restarts. Less active keys may temporarily lose their relative position after a restart, but disruption is generally not noticeable in practice.
