...
Priority is available for both self-hosted Temporal instances and Temporal Cloud.

To enable Priority in self-hosted Temporal, set the dynamic config `matching.useNewMatcher` to `true` on a Task Queue, Namespace, or globally. Priority is enabled by default in Temporal Cloud.
...
Task Queue Fairness lets you distribute Tasks based on _fairness keys_ and _fairness weights_ within a Task Queue. Each fairness key creates its own "virtual queue", allowing you to organize Tasks into logical groups like tenants, applications, or workload types.

These virtual queues operate using a round-robin dispatch mechanism, meaning the system cycles through each fairness key in turn when selecting the next Task to dispatch. This prevents any single fairness key from hogging Worker capacity, even if one key has a much larger backlog than the others.

By default, each fairness key is weighted equally in the round-robin. This behavior can be customized by assigning a _fairness weight_ to a key, but only one weight should be assigned per fairness key within a Task Queue. Multiple weights on the same key will result in unspecific behavior.
