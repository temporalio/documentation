...
Priority is enabled by default in Temporal Cloud. To enable Priority in self-hosted Temporal, set the dynamic config `matching.useNewMatcher` to `true` on a Task Queue, Namespace, or globally. For example, to enable Priority on the dev server:
