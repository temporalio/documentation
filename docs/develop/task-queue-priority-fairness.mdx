...
You can use Priority and Fairness individually or combine them to express Fairness within a Priority tier.

## Task Queue Priority

**Task Queue Priority** lets you control the execution order of Workflows, Activities, and Child Workflows based on assigned priority values within a _single_ Task Queue. Each priority level acts as a sub-queue that separates Tasks so that high priority Tasks can cut in front of low priority Tasks.
