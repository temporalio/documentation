...
- Backlogged tasks are durable and unaffected by server restarts. Fairness ordering is largely preserved across restarts for the most active keys; less active keys may briefly dispatch new tasks ahead of their existing backlog until ordering normalizes.
