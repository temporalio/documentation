...
- Fairness counters for up to 100 active fairness keys are preserved across server restarts. Keys outside the top 100 may temporarily lose their relative position after a restart, but disruption is generally not noticeable in practice.
