...
on the relevant Task Queues or Namespaces. For example, to enable Fairness on the dev server:

```
temporal server start-dev --dynamic-config-value matching.enableFairness=true
```
