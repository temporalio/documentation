...
Priority is enabled by default in Temporal Cloud. To enable Priority in self-hosted Temporal, set the [dynamic config](/temporal-service/configuration#dynamic-configuration) `matching.useNewMatcher` to `true` on a Task Queue, Namespace, or globally. For example, to enable Priority on the dev server:

```
temporal server start-dev --dynamic-config-value matching.useNewMatcher=true
```
...
To enable Fairness for a Namespace in Temporal Cloud, navigate to the Namespace's Overview page in the UI and activate the Fairness toggle. Note that CLI support is coming soon.
