...
import { EnlargeImage, SdkTabs, AnnotatedCode } from '@site/src/components';
...
Each fairness key has a default _fairness weight_ of 1.0. You can assign a different weight to a key. When two groups have backlogged Tasks, a group with a weight of 2.0 receives approximately twice as many dispatches as a group with a weight of 1.0.
...
fairness key to make sure _tenant-big_ doesn't dominate dispatch and block the others. In this case,
...
2. **Fairness key within a tier (weighted).** Within a priority tier, Tasks are dispatched according to the weights of their fairness keys. When both groups are backlogged, a key with weight 2.0 receives approximately twice as many dispatches as a key with weight 1.0.
...
<EnlargeImage
  ...
  alt="Flowchart of how Priority and Fairness combine: a poller checks priority levels in order, then Fairness selects a backlogged group according to its weight"
/>
...
- With Worker Versioning, each version has a separate backlog. Fairness applies within each version's backlog.
...
- Tasks that synchronously match an available poller can bypass backlog ordering. [Eagerly dispatched Tasks](/develop/worker-performance#eager-task-execution) also bypass matching.
