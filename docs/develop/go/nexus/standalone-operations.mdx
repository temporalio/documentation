...
create the Nexus Endpoint and start the sample Worker (`go run nexus-standalone-operations/worker/main.go`):
...
resp, err := c.ListNexusOperations(context.Background(), client.ListNexusOperationsOptions{
    Query: "Endpoint = 'nexus-standalone-operations-endpoint'",
})
if err != nil {
