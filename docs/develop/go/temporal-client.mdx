...
To use an [API key](/cloud/api-keys) with the Temporal Go SDK, you will need to provide additional connection information:

- When you create your _API Key,_ be sure to save the value.
- You can see the _namespace account id_ combination in your URL or anywhere that you manage namespaces.
  It follows the format `<namespace_id>.<account_id>`.
- The _endpoint_ will vary. The most common endpoint used is the gRPC regional endpoint, which you can find by selecting the Connect button on the namespace details page.
  It follows the format: `<region>.<cloud_provider>.api.temporal.io:7233`.
...
Once you have the necessary info, to create an initial connection, use this code:
