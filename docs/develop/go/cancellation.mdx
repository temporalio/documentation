...
Some scenarios for this could be you want to completely ignore the cancellation and continue Activity
execution, including Heartbeating, or you would like to send one final Heartbeat after Cancellation.
...
`context canceled` error will be returned from the call. However, do note that the 
