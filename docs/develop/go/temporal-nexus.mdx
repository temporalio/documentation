...
Use `temporalnexus.GetClient(ctx)` to get the Client that the Worker was initialized with:
...
var MyOperation = nexus.NewSyncOperation("my-operation", func(ctx context.Context, input MyInput, options nexus.StartOperationOptions) (MyOutput, error) {
	c, err := temporalnexus.GetClient(ctx)
	if err != nil {
		return MyOutput{}, nexus.HandlerErrorf(nexus.HandlerErrorTypeInternal, "failed to get client: %v", err)
	}
	// Signal a Workflow
	err = c.SignalWorkflow(ctx, input.WorkflowID, "", "my-signal", input.Value)
	// Query a Workflow
	val, err := c.QueryWorkflow(ctx, input.WorkflowID, "", "my-query")
	// ...
})
```
