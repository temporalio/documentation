...
- Credentials with permission to write objects on components that store payloads, and to read objects on components that
  retrieve them. Because the drivers check whether an object already exists before uploading, storing also requires read
  permission. Components that only retrieve payloads do not need write permission.
...
2.  Configure the driver on `ExternalStorage` and pass it in your Client options. This step is the same for every
    ...
    times a Run passes a payload around.
