...
During Workflow Task processing, returning an error from `Encode` or `Decode` can fail the Workflow Execution, depending on the operation.
To apply the Worker's configured [`WorkflowPanicPolicy`](https://pkg.go.dev/go.temporal.io/sdk/worker#WorkflowPanicPolicy), panic from the Payload Codec instead.
