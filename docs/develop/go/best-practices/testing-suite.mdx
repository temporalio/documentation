...
If an Activity references its context, you need to mock that context when testing in isolation. You can do that by using [`NewTestActivityEnvironment`](https://pkg.go.dev/go.temporal.io/sdk@v1.44.1/internal#WorkflowTestSuite.NewTestActivityEnvironment) from the [`WorkflowTestSuite`](https://pkg.go.dev/go.temporal.io/sdk@v1.44.1/internal#WorkflowTestSuite).
...
In addition to automatic skipping, you can manually control time progression in the Go test environment.

Use `env.RegisterDelayedCallback()` to explicitly move time forward. This is useful when you want fine-grained control over when Timers fire.
