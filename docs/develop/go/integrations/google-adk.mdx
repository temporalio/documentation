...
Tools run **in-workflow by default** (the idiomatic Temporal model: the Workflow Definition can't touch the network, clock, or
disk directly — anything that does goes through an Activity). Opt a tool into an Activity with `googleadk.ActivityAsTool`, or
