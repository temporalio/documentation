...
Nexus handler servier to run the actual Nexus operation. You can mock the Nexus operation or
override its implementation with the Nexus mock environment.
...
type in this case must be `*nexus.HandlerStartOperationResultAsync` with an `OperationID`, which can
...
If your Workflow executed multiple Nexus asynchronous operations, you can mock each of them with
different operation IDs, and register the completion results using the corresponding operation ID.
