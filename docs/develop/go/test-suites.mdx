...
- **Unit:**
  - Running a piece of Workflow or Activity code (a function or method) and mocking any code it calls.
  - Running a piece of Nexus operation and mocking the handler.
...
## Testing Nexus Operations {#test-nexus}

A Nexus operation can be tested with a mock Nexus environment, which provides a way to mock the
Nexus operation. This behavior allows you to test the Nexus operation in isolation by calling it
directly without needing to create a Nexus handler service to run the Nexus operation.

### Mock and override Nexus operations

Let's take a look at a test that simulates a Nexus operation call. Assume that the Nexus operation
is called `sample-operation`, the input type is `SampleInput`, the output type is `SampleOutput`,
and it belongs to the Nexus service `sample-service`.
...
The example above simulates a call to a Nexus synchronous operation, indicated by the returned value
type `*nexus.HandlerStartOperationResultSync[T]`. Since `OnNexusOperation` needs to know the
operation's name, input and output, `nexus.NewOperationReference` creates an operation reference
that represents the operation without knowing the implementation (basically, it represents the
signature of the operation). You may also use the operation itself instead of creating the operation
reference if you have it available.

Let's take an example of simulating a call to an Nexus asynchronous operation.
...
func (s *UnitTestSuite) Test_SimpleWorkflow_NexusAsyncOperation() {
        s.env.OnNexusOperation(
                ...
                &nexus.HandlerStartOperationResultAsync{
                        OperationID: "sample-operation-id",
                },
                nil, // error if you want to simulate an error in the ExecuteOperation call
        )
        err := env.RegisterNexusAsyncOperationCompletion(
                "sample-service",
                "sample-operation",
                "sample-operation-id", // must match the OperationID above
                SampleOutput{},
                nil,            // error if you want to simulation an error in the operation
                2*time.Second,  // delay to simulate how long the operation takes
        )
        s.NoError(err)
        s.env.ExecuteWorkflow(SimpleWorkflow, "test_nexus_operation")
...
In the example above, the `OperationID` in the `*nexus.HandlerStartOperationResultAsync` object can
be any string of your choice, and it is used in the `RegisterNexusAsyncOperationCompletion` call to
register the result of the corresponding asynchronous operation. With this, you can mock multiple
asynchronous Nexus operations and register the results for each of them in the same workflow.

If you need to run some custom logic for the Nexus operation, you can do as follows.
