...
For most calls, the `WorkflowID` option is optional; and it is invalid to set the `RunID` option.
For `WithStartOperation`, a `WorkflowID` is required.
...
For example, specifying a `CronSchedule` results in an error.
Refer to the API documentation for further details.

The `ExecuteWorkflow` returns once an Update result is available; or when a provided context times out.
