...
Each tracing interceptor uses its library’s native propagation mechanism (e.g., OpenTelemetry’s `TextMapPropagator` with W3C TraceContext format) to serialize trace spans into Temporal headers. The SDK carries these headers across Workflow, Activity, and Child Workflow boundaries, so the tracing library can reconstruct the call graph.  
...
To build custom context propagation (e.g., tenant IDs, auth tokens), see [Context Propagation](/develop/go/context-propagation).
