...
The Go SDK provides tracing interceptors for [OpenTelemetry](https://pkg.go.dev/go.temporal.io/sdk/contrib/opentelemetry), [OpenTracing](https://pkg.go.dev/go.temporal.io/sdk/contrib/opentracing), and [Datadog](https://pkg.go.dev/go.temporal.io/sdk/contrib/datadog/tracing). Create a tracing interceptor and pass it to [ClientOptions](https://pkg.go.dev/go.temporal.io/sdk/internal#ClientOptions):
...
The interceptor automatically propagates trace context across Workflow, Activity, and Child Workflow boundaries using Temporal headers. You can also register interceptors through a [Plugin](/develop/plugins-guide#interceptors) if you’re building a reusable library.
...
You can also register context propagators through a [Plugin](/develop/plugins-guide#context-propagators) if you are building a reusable library.
