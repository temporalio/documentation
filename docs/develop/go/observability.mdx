...
### Context Propgation Over Nexus Operation Calls

Nexus does not use the above header structure as it uses a Temporal agnostic protocol under the hood that is built to
connect arbitrary systems. For propagating context over Nexus Operation calls, the context can be serialized into a
`nexus.Header` - a wrapper on top of `map[string]string` with helpers to `Set` and `Get` values, normalizing keys into
lower case form. Due to this different format, and the fact that Nexus calls may go to external parties, the
`ContextPropagator` interface does not apply to Nexus headers and must be explicitly propagated via interceptors as
shown in the [Nexus Context Propagation
sample](https://github.com/temporalio/samples-go/tree/main/nexus-context-propagation).
