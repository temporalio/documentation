...
```go
opts.ClientOptions.ConnectionOptions.TLS = &tls.Config{
    // Custom TLS settings, e.g., RootCAs for a private CA
}
```
...
To collect this telemetry, attach the [AWS Distro for OpenTelemetry Lambda layer](https://aws-otel.github.io/docs/getting-started/lambda/lambda-go) to your Lambda function.
