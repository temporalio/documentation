...
For new metrics setups that use OpenTelemetry, use the
[OpenTelemetry v2 integration](/develop/go/integrations/opentelemetry-v2#export-temporal-sdk-metrics). It
configures Temporal SDK metrics through the Go SDK Plugin API and is currently in
...
For new tracing setups that use OpenTelemetry, use the
[OpenTelemetry v2 integration](/develop/go/integrations/opentelemetry-v2). It uses the Go SDK Plugin API to
propagate OpenTelemetry trace context across Temporal calls and is currently in
[Pre-release](/evaluate/development-production-features/release-stages#pre-release).
