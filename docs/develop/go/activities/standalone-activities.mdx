...
New to Standalone Activities? Start with the [Standalone Activities Go Quickstart](/develop/go/activities/standalone-activities-quickstart)
to define an Activity, run a Worker, and execute your first Standalone Activity. This guide picks up where the
Quickstart leaves off.
...
The code samples in the [Quickstart](/develop/go/activities/standalone-activities-quickstart) use
`envconfig.MustLoadDefaultClientOptions()`, so the same code works against Temporal Cloud - just configure the
...
Then run the Worker and starter code as shown in the
[Quickstart](/develop/go/activities/standalone-activities-quickstart).
