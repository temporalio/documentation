...
## Monitoring Task Queue metrics {#task-queue-metrics}
...
### Task Queue types and metrics {#task-queue-types}
...
- **Nexus Task Queue** ([Public Preview](/evaluate/development-production-features/release-stages#public-preview)): A queue that holds Temporal Nexus Tasks.
  Temporal Nexus Tasks are used for units of work passed between Namespaces.
  Workers configured with the Temporal Nexus Service poll the Nexus Task Queue for available tasks and handle them by initiating Workflow Executions in the target Namespace.
  Each Temporal Nexus Task contains the context required by the target Workflow Definition.
...
- [`ApproximateBacklogAge`](#ApproximateBacklogCount)
...
Workflow Task Queue types provide only partial information.
Tasks sent to [Sticky](https://docs.temporal.io/workers#sticky-execution) queues are not included in the returned values.
Since Tasks remain valid for only a few seconds in Sticky Queues, this inaccuracy becomes less significant as the backlog grows in size.
:::
...
### Retrieve Worker information with the Temporal CLI {#retrieve-worker-info}
...
It may significantly change or be removed in a future release.
Determining Task reachability incurs a non-trivial computing cost.

:::

#### Uncover Worker availability and capacity issues {#worker-capacity-issues}
...
You can adjust the number of Workers to improve Workflow Execution performance and manage your fleet size.
For example, a large backlog of Tasks with too few Workers will slow down Workflow Execution completions and reduce processing efficiency.
Adding more Workers speeds up completion rates and improves throughput.
An empty backlog indicates low Worker utilization, allowing you to reduce your fleet and associated costs.

The metric values provided by `temporal task-queue describe` can assist you in managing your Worker fleet deployment:

- `ApproximateBacklogAge` shows how long Tasks have been waiting for dispatch.
  If this time becomes excessive, consider deploying more Workers to enhance Workflow Execution efficiency.
...
- Calculate the demand per Worker by dividing the number of backlogged Tasks (`ApproximateBacklogCount`) by the number of Workers.
  Compare this against the backlog consumption rate (`TasksDispatchRate`) and dispatch latency (`ApproximateBacklogAge`) to assess if your task processing rate meets your expectations.

- The backlog increase or decrease rate (`BacklogIncreaseRate`) indicates the changing demands on your Worker fleet.
  As the backlog increase rate grows, you may need to deploy additional Workers until you achieve a balanced state.
