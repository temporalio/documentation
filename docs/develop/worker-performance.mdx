...
Each SDK provides [default slot counts](/develop/worker-tuning-reference#compute-defaults-by-sdk), but Temporal recommends actively tuning these values for your workload.
See [Worker runtime performance tuning](#worker-performance-tuning) for guidance.
