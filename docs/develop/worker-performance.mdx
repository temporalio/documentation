...
A Worker's performance characteristics are affected by the following elements.

### Executor slots {#slots}

A **Worker Executor Slot**, represents the capacity of a Temporal Worker to execute a single concurrent Task.
...
Slot types support Activity, Workflow, Nexus, or Local Activity Tasks.
...
- **Fixed Size Slot Suppliers**:
  ...
  When you need the absolute best performance, pre-determine your hardware and environment characteristics.
...
- Resource-based slot suppliers supersede the existing `maxConcurrentXXXTask` style Worker options.
  Using both styles will cause runtime crashes of your Temporal applications.
...
**Task pollers** play a crucial role in the Temporal architecture by efficiently distributing work to Workers to support scalable, resilient Workflow Execution.
A Temporal Task poller is a Worker component.
...
### Latency timers {#latency-timers}

A **Temporal latency timer** measures the time taken for various operations within the Temporal system.
These timers help monitor and optimize the performance of Temporal Workflows and services.
They help identify bottlenecks, detecting backlogs, and support efficient Workflow execution.


### Worker caches {#caches}

Worker caches in Temporal are an optimization mechanism that improve the performance and efficiency of Workflow execution.
A WorkerCache is held by each Worker to store cached data.
It contains pointers for data shared with other Workers and its own values for instance-specific use.
...
Temporal's [`worker_task_slots_available`](/references/sdk-metrics#worker_task_slots_available) gauges can report the number of available executor “slots” that are currently available and unoccupied for a Worker type.
...
`maxConcurrentWorkflowTaskExecutionSize` and `maxConcurrentActivityExecutionSize` define the number of total available Workflow Task and Activity Task slots for a Worker.
...
A **Worker Tuner** provides a single instance that provides slot suppliers for different slot types (Activity, Workflow, Nexus, or Local Activity Tasks).
A tuner assigns different suppliers to each slot type.
For example, it might vend a fixed assignment slot supplier for Workflows and use an resource-based supplier for Activities.
...
- When Task completion latency is a concern, avoid resourced-based auto-tuning slot suppliers.
...
Slot throttling is a mechanism used in Temporal's Worker configuration to control the rate at which new slots for concurrent tasks are made available for processing.
