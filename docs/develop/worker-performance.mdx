...
Eager Workflow Start is available in [Public Preview](/evaluate/development-production-features/release-stages#public-preview).

:::

Eager Workflow Start (EWS) is currently in Public Preview, with the goal of reducing the time it takes to start a Workflow.
...
Eager Workflow Start is opt-in, and requires the Starter and the Worker to share a Client located in the same process.
To share the Client between Worker and Starter:

1. Start the Worker first in non-blocking mode: for example with the Go SDK [using Start instead of Run, and a handler for Worker errors with the option OnFatalError](https://github.com/temporalio/samples-go/tree/main/eager-workflow-start)
2. Set the `request_eager_start` (or similar name) to true in the Start Workflow call.
...
Instead, when the Starter and Worker are collocated and aware of each other, they can interact while bypassing the server, saving a few time-intensive operations.
