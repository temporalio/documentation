...
Activity slot counts depend on per-Activity resource consumption. Profile your Activities under load and adjust the count to maximize throughput without exhausting Worker resources.

#### When to use resource-based slot suppliers

Resource-based slot suppliers are suited to a narrow set of use cases.
They can never perform as well as a properly configured fixed-size tuner, and they have foot-guns that make them tricky to use effectively.
Reserve resource-based suppliers for the following scenarios:
