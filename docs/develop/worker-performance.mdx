...
Eager Workflow Start is an opt-in feature that is disabled by default. It requires the Starter and the Worker to share a Client located in the same process and setting the `request_eager_start` (or similar name) to true in the Start Workflow call.
