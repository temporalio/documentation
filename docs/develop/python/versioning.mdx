...
Suppose you have a Workflow that has an Activity called `pre_patch_activity`:
...
1. Use [Patched](https://python.temporal.io/temporalio.workflow.html#patched) to patch in new code and run it alongside the `PrePatchActivity` code.
2. Once all `PrePatchActivity` Workflows have finished executing and surpassed the Retention Period, remove the old code and apply [DeprecatePatch](https://python.temporal.io/temporalio.workflow.html#deprecate_patch).
...
After ensuring that all Workflows started with `pre_patch_activity` code have finished and surpassed the Retention Period, you can remove the old code branch and [deprecate the patch](https://python.temporal.io/temporalio.workflow.html#deprecate_patch).
...
### 3. Safe Deployment of post_patch_activity {#deploy-new-code}

You can safely deploy `post_patch_activity` without a patch once all Workflows with non-deprecated my-patch or earlier are finished and have surpassed Retention Period.
