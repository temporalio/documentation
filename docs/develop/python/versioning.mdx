...
This video examines into the behavior of the `patched()` function:
...
If the Marker Event occurs after the execution point in the event history,
...
Recapping the unexpected behaviors that may occur during a Replay:

If the execution hits a call to `patched()`, but that patch ID isn't *at or before
that point* in the event history, you may not realize that
the event history *after* the current execution location matters. 
...
using old code. For the rest of the execution, they
will either:
...
the newest code*. It only does that if not replaying or if replay is
