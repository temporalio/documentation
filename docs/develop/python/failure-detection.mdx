...
In each Temporal SDK, error handling is implemented idiomatically, following the convetions of the language.
...
In fact, *any* exceptions that are raised from your Python code in a Temporal Activity will be converted to an `ApplicationError` internally.
...
This works differently in a Workflow than raising errors from Activities.
In an Activity, all raised errors are converted to Temporal `ApplicationError`.
In a Workflow, any errors that are raised other than an explicit Temporal `ApplicationError` will only fail that particular [Workflow Task](https://docs.temporal.io/tasks#workflow-task-execution) and be retried automatically.
