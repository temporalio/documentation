...
2. Configure the driver on your `DataConverter` and pass the converter to your Client and Worker:
   ...
   By default, payloads larger than 256 KiB are offloaded to external storage. You can adjust this with
...
If you need a storage backend other than what the built-in drivers allow, you can implement your own storage driver. The
following example shows a complete custom driver implementation that uses local disk as the backing store:
