...
- `name()` returns a unique string that identifies the driver. This name is embedded in the reference payload stored in
  Event History and is used to map a claim back to the correctly configured driver and its configuration parameters.
  Changing the name after payloads have been stored breaks retrieval.
...
For more complex selection logic, use a callable that reads from the `StorageDriverStoreContext`. The following example
uses a feature flag to gradually roll out external storage to a subset of Workflows:
