...
If you are inheriting methods from both `client.Interceptor` and `worker.Interceptor`, you should not pass your Interceptors directly to the `Worker()` constructor — instead, just pass it to `Client.connect()`.
This will allow a Worker to use them from the underlying Client.
In other words, only inherit from `worker.Interceptor` and pass the methods to the `Worker()` if you are not using Client methods.
