...
Interceptors can also perform arbitrary modifications to data before it is received by the SDK's "real" implementation, e.g. for context propagation.
...
- `execute_workflow()`
    - Note that `execute_workflow()` is considered an Inbound Workflow call unlike `start_workflow()` because it blocks and waits for a result to be returned.
- `handle_query()`
- `handle_signal()`
...
- `execute_activity()`
    - This is technically the only Inbound Activity call, with the same distinction as `execute_workflow()`.

#### [Outbound Activity calls](https://python.temporal.io/temporalio.worker.ActivityOutboundInterceptor.html)
- `info()`
- `heartbeat()`

Technically, the first of these categories is a Client call, and the remaining 4 are Worker calls.
...
Ihe intercepted method will perform its function then call the same method on the next Interceptor in the chain.
