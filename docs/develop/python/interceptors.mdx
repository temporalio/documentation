...
These methods run on the Client interacting with the Workflow.  For example:
...
These methods run on the Worker in the Workflow definition and intercept inbound calls like starts/executes and messages.

:::warning Workflow interceptors and replay

These methods execute during [replay](/develop/python/testing-suite#replay). Use replay-safe APIs for logging, randomness, and time in these interceptors.
See [Develop Workflow logic](/develop/python/core-application#workflow-logic-requirements) for important details.

Also, if you want to write a generic code shared by all inbound Workflow call handlers, but want to avoid doing something during the read-only operations, check `workflow.unsafe.is_read_only()`.
...
These methods run on the Worker in the Workflow definition and intercept outbound calls to Temporal APIs like starting Activities and Timers.
...
This interceptor runs on the Worker in the Activity context, at the beginning.
...
These methods run on the Worker in the Activity context when it's calling built-in methods.
