...
Workflow Streams are principally intended for keeping outside observers updated on the progress being made by a Workflow and its associated Activities.
...
A `WorkflowStream` instance is a many-to-many communication channel hosted inside a Workflow: multiple publishers (the Workflow itself, its Activities, other Workflows, etc.) may append events, and multiple subscribers may consume them. Workflow Streams target modest fan-out — typically on the order of tens of publishers and subscribers per Workflow, not high-fanout distribution to thousands of clients. Temporal's [Signals and Updates](/develop/python/workflows/message-passing) provide the underlying message passing primitives; the `temporalio.contrib.workflow_streams` module packages the batching, offset tracking, topic filtering, and continue-as-new hand-off that turn those primitives into a usable stream.
...
Cross-language client support is on the roadmap. Only the Python client is available presently.
...
- A complete LLM streaming example on this page (Activity publishes deltas, FastAPI SSE consumer with retry handling): [Stream LLM output](#stream-llm-output).
...
Temporal's standard message-passing model provides the implementation, with no new transport involved:
...
A `WorkflowStream` is hosted inside a Workflow, so the first design choice is whether one Workflow handles both the work and the stream, or whether a separate Workflow exists only to host the stream. This is a lifecycle question, not a naming convention.

**Host the stream on the Workflow that does the work** when the events come from work that Workflow orchestrates: an agent run, an order pipeline, a chat session. The stream's lifecycle aligns with the work, starting when the run starts and ending when the run returns. The Workflow Id used to start the work is the same one subscribers attach to. This is the common shape for AI agents and most progress-streaming use cases, where the stream is a natural side channel of the work the Workflow performs.
...
The library ships with the Temporal Python SDK; import it from `temporalio.contrib.workflow_streams`. Enable streaming by constructing a `WorkflowStream` from your Workflow's `@workflow.init` method:
...
- It creates the in-memory event log that holds published entries
...
`@workflow.init` is the only hook guaranteed to run before any Signals or Updates can be delivered, so stream handlers must be registered there to avoid missing early publishes. (If you've only seen `@workflow.run` before, think of `@workflow.init` as the Workflow's constructor.) Calling `WorkflowStream(...)` outside `@workflow.init`, or constructing more than one stream on the same Workflow, raises `RuntimeError`.
...
Pass `force_flush=True` on a publish to ship the current buffer immediately without waiting for the next interval. The call returns as soon as the flusher is woken; it does not wait for delivery to the Workflow or to subscribers:
...
`publish()` is non-blocking and applies no backpressure. From an Activity or other client, it appends to the client's in-memory buffer and returns; from inside a Workflow, it appends synchronously to the in-memory log (no buffer, nothing to flush). Subscribers pull from the Workflow's log on their own schedule, so subscriber slowness does not propagate back to publishers. If a publisher emits faster than batches can ship to the server, the buffer grows: the process consumes more memory, the stream's view of real-time falls further behind the publisher, and at the limit Signals cannot land fast enough to keep up.

If your application needs to bound this (to cap memory, to keep the stream close to real-time, or to enforce a policy when production outpaces shipping), provide it upstream of `publish()`. The right policy is application-specific (block, drop, error, sample), and Workflow Streams does not impose a choice.
...
Subscribing from inside the host Workflow is intentionally unsupported. Reading the stream the Workflow itself produces would cross the abstraction boundary that keeps the Workflow's view of progress independent of side-channel events; see [How it works at a glance](#how-it-works-at-a-glance) for why the Workflow acts as a conduit rather than a consumer.
...
The iterator handles re-polling, pagination when a poll response hits the ~1 MB cap, and Workflow-side log truncation transparently. There are no extra signals or exceptions for normal callers to handle. See the [API reference](https://python.temporal.io/temporalio.contrib.workflow_streams.html) if you need the underlying details.

Closing a stream cleanly requires a deliberate pattern; see [Closing the stream](#closing-the-stream) below.
...
The headline use case fits the publish/subscribe shapes above. An Activity calls the model and publishes deltas as they arrive; the Workflow kicks off the Activity and emits its own lifecycle events; an HTTP endpoint subscribes and forwards deltas to the browser over Server-Sent Events (SSE).
...
async def stream_completion(prompt: str) -> str:
    ...
    async with client:
        deltas = client.topic("delta", type=TextDelta)
        complete = client.topic("complete", type=TextComplete)
        retry = client.topic("retry", type=dict)
        ...
        full: list[str] = []
        first = True
        async for chunk in openai_client.chat.completions.stream(prompt=prompt):
            # force_flush only on the first delta so the user sees something
            # immediately; subsequent deltas batch at the 200 ms interval.
            deltas.publish(TextDelta(text=chunk.text), force_flush=first)
            first = False
...
```python
# api.py — FastAPI SSE endpoint with a retry-aware reducer
...
async def chat_stream(chat_id: str):
    ...
    async def events():
        ...
        async for item in stream.subscribe(
            ...
            if item.topic == "retry":
                # Earlier attempt's deltas are stale — tell the browser to reset.
                accumulator.clear()
                yield "event: reset\ndata:\n\n"
...
- The Activity is the publisher because it owns the non-deterministic LLM call. The Workflow processes only the Activity's *return value* and emits lifecycle events but does not read the stream itself (see [Workflow as conduit](#workflow-as-conduit)).
...
- `force_flush=True` is used only on the first delta and on the `RETRY` sentinel, where latency matters. Subsequent deltas batch at the 200 ms `batch_interval`. Per-delta `force_flush=True` would generate one Signal per token; for a long completion that's measurable history pressure. See [Architecture](#architecture) for tuning.
...
The trade-off is direct. Each batched publish becomes one Signal event in the Workflow's history, and each poll becomes two Update events. A more responsive UI means more events per second, which approaches Temporal's per-run history limits faster. For long-running streams, plan a [Continue-As-New](#continue-as-new) policy from the start.
...
- **`force_flush=True`** (per publish). Wakes the flusher immediately so the buffer ships without waiting for the next interval. Use it on the *first* delta of a response so the user sees something fast, and on punctuated events like `RETRY` or `STATUS_CHANGE` where latency matters. Do not make it the default mode: per-token `force_flush=True` on a 500-token completion produces 500 publish Signals, which is meaningful but tractable; per-character `force_flush=True` is not.

### Other knobs

You usually do not need to touch these, but they are available when the basic knobs are not enough:
...
- **`poll_cooldown`** (subscriber-side, default 100 ms). Minimum interval between polls when the previous poll returned no progress. Hot subscribers skip the cooldown when the previous poll hit the 1 MB response cap, so this knob mainly governs subscriber load on idle Workflows. Hold a single iterator and consume from it rather than opening and closing subscriptions in a loop, which multiplies the Update events.
- **`max_retry_duration`** (default 10 minutes) and **`publisher_ttl`** (default 15 minutes). Failure-recovery limits. The defaults satisfy the constraint `max_retry_duration < publisher_ttl`; if you tune one, tune the other. See [Delivery semantics](#delivery-semantics) for the full failure model.
...
- **`max_retry_duration`** (default 10 minutes). A `WorkflowStreamClient` retries a failed batch for up to this long. If the duration elapses with the batch still pending (for example, during a sustained network partition), the client gives up, the pending batch is dropped, and a `TimeoutError` is raised.
    ...
    Two consequences are worth knowing. First, the client advances its internal sequence past the dropped batch before giving up, so subsequent batches are guaranteed to use a fresh sequence and will not be silently deduplicated against a (possibly delivered) timed-out batch. The batch is at-most-once on timeout, but downstream publishes resume cleanly. Second, the timeout raises from inside the background flusher loop, which terminates that task; until you call `await client.flush()` or exit the `async with` block, subsequent publishes accumulate in the buffer with no flusher to ship them.
...
The codec chain (the same one configured on your client) runs once on the Signal or Update envelope, not per item, so codec transforms apply symmetrically to Workflow-side and client-side publishes with no double processing (e.g. encryption, compression).
...
- **Publishers send Signals.** `WorkflowStreamClient` batches publishes inside its async context manager and ships each batch as a single `__temporal_workflow_stream_publish` Signal. The default batch interval is two seconds. `force_flush=True` skips that timer for an immediate send. Each batch becomes one Signal event in the Workflow's history, so high-frequency `force_flush=True` from a hot loop will inflate history; tune `batch_interval` accordingly.
- **Subscribers send Updates.** `subscribe()` issues a long-poll `__temporal_workflow_stream_poll` Update that returns when entries past the requested offset are available or when the polling cooldown elapses with no progress. Poll responses are capped at roughly 1 MB; subscribers repoll immediately when the server caps a response, so high-throughput producers should expect a steady stream of small batches rather than occasional large ones.
...
- **Mid-stream barriers need explicit flush.** `await client.flush()` is the only way to be sure prior publishes have reached the server before subsequent work runs. `force_flush=True` wakes the flusher but doesn't wait, so use it for latency, not for confirmation.
