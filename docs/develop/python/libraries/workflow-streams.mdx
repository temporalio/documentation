...
Workflow Streams are principally intended for keeping outside observers updated on the progress being made by a Workflow and its associated Activities. Example applications include updating a user interface to show the progress being made by an AI agent, or providing updates on in-flight payment or order processing work. They are not designed for ultra-low-latency applications such as real-time voice streaming, and they target modest fan-out: typically on the order of tens of publishers and subscribers per Workflow, not high-fanout distribution to thousands of clients.
...
**Wire-level handlers.** The three handlers registered when you construct a `WorkflowStream` are `__temporal_workflow_stream_publish` (the Signal that receives batched publishes), `__temporal_workflow_stream_poll` (the long-poll Update that subscribers use), and `__temporal_workflow_stream_offset` (the Query that reports the current head offset). Poll responses are capped at roughly 1 MB; subscribers repoll immediately when the server caps a response, so high-throughput producers see a steady stream of small batches rather than occasional large ones.
...
The headline use case fits the publish/subscribe shapes documented above. An Activity calls the model and publishes deltas as they arrive; the Workflow kicks off the Activity and emits its own lifecycle events; an HTTP endpoint subscribes and forwards deltas to the browser over Server-Sent Events (SSE).
