...
Workflow Streams is built for the case where an observer can show up late — after a page refresh, server restart, or closed laptop — and still see what happened while they were away. Three patterns recur when applying the library. You can find the surrounding rationale in [this blog post](see [TODO: blog post link]).
