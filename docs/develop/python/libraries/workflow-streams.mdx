...
When the source of events is an Activity — for example, an Activity streaming output from an LLM — make the Activity publish events directly to the Workflow's stream. The Workflow publishes its own lifecycle events and processes only the *complete result* returned by each Activity; it doesn't read the stream itself.

This separation keeps the Workflow's deterministic progress independent of side-channel events, which may include partial output from an Activity attempt that ended up retrying. Surface retry transitions to consumers as their own event (for example, a `RETRY` event published with `force_flush=True`) and let the consumer decide how to present them. See [Delivery semantics](#delivery-semantics) for the precise guarantees.
...
:::note Custom handlers that read stream state

If you add a synchronous `@workflow.signal` or `@workflow.update` handler that reads `WorkflowStream` state, a publish Signal that lands in the same activation as a call to your handler may be observed in its pre-publish form. Define such handlers as `async` and `await asyncio.sleep(0)` before reading state — that yield lets pending publishes apply with no Temporal timer or history events. (Don't substitute `workflow.sleep(0)`, which schedules a timer.) Most handlers are already safe: anything that `await`s before reading state, the module's own poll handler, and Workflow-internal publishes.
...
`WorkflowStreamClient.subscribe()` will exit cleanly on its own when the host Workflow reaches a terminal state, so callers don't need to wrap the iterator for that case. To close a streamed Workflow cleanly — and to let consumers recognize a *semantic* end of stream rather than just "the Workflow is gone" — two pieces work together:
...
2. **The Workflow holds its run open briefly after publishing the terminator** so subscribers' next poll fetches it before the Workflow returns. Items published in the same Workflow task that returns from `@workflow.run` are abandoned and the next subscriber poll lands on a completed Workflow. A short `await workflow.sleep(timedelta(milliseconds=500))` before returning is enough to let one more poll come through.
...
With both pieces in place, subscribers receive the terminator, break out of their `async for`, and stop polling. By the time the Workflow exits there are no in-flight poll handlers, so the SDK doesn't warn about unfinished handlers. As a fallback for runs that crashed before publishing a final event, the iterator's clean exit on terminal status surfaces the missing terminator; call `await temporal_client.get_workflow_handle(order_id).describe()` afterward to inspect the Workflow's status.
...
The `| None` on the `stream_state` field type is required: `prior_state` is `None` on a fresh start and a `WorkflowStreamState` instance after a rollover. Use the concrete type, not `Any`, so the default data converter can rebuild the dataclass on the new run.
...
**Streaming surfaces what durable execution hides.** Durable execution presents your Workflow code with a clean view of progress: Activity retries are absorbed and only the successful result is visible. Streaming sits *below* that boundary. Consider an Activity that publishes three `TEXT_DELTA` events, fails, and is retried. The Workflow itself sees only the second attempt's return value, but every event from both attempts is now in the stream. That is what gives you visibility into in-progress work — a model's output as it arrives, an Activity's progress as it runs, a pipeline's intermediate state — and it is the inevitable consequence of surfacing events before they have been settled into a successful outcome. Waiting for a failure-safe recovery point would defeat the purpose.

Deduplicating, hiding, or labelling retry-attempt events is the application's job. The conventional pattern is for an Activity that detects it's on a retry attempt to publish a `RETRY` event with `force_flush=True`, and for the consumer to clear or annotate prior-attempt output when it sees one. The [Workflow as conduit](#workflow-as-conduit) pattern keeps the Workflow's own state independent of these retried events; treat the stream as an append-only log of attempts and let an idempotent consumer reducer reconcile them — overwriting on terminal events like `STATUS_CHANGE` or `TEXT_COMPLETE`, or resetting an accumulator on a sentinel like `AGENT_START` before deltas resume.
