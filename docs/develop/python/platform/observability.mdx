...
That gap shows up during an incident.
When several Namespaces degrade at the same time, the cause is usually either the Temporal Service or a deploy by one team.
Grouped only by Temporal tags, the two look the same, and the investigation starts by asking whether the Temporal Service is at fault.

A `team` tag separates the two cases on the dashboard itself:
