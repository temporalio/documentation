...
Temporal gives your agent code durable execution: it retries failed work, persists state across crashes, and resumes
from where it left off. LangSmith adds the observability side, so you can inspect LLM inputs and outputs, follow a
...
Also add the plugin to any Temporal Client you use on the Client side (typically a starter or API that calls into
...
and so on) must run inside an Activity. That makes Activities the right place to add LangSmith runs too. When you
...
prompt, completion, token counts, and latency. You get all of that by wrapping the client once:
...
The [LangSmith tracing sample](https://github.com/temporalio/samples-python/tree/main/langsmith_tracing) puts all of
