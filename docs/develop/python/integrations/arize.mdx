...
Run the starter and the Worker as separate processes, as the sample does. The bridge is Public Preview in the
Temporal SDK; the sample's `telemetry.py` includes a small logging filter for a known, harmless
"Failed to detach context" message that the bridge produces when an agent span outlives a Workflow Task.
