...
to the [`pydantic_data_converter`](/develop/python/best-practices/data-handling/data-conversion), so Pydantic types
...
`StrandsPlugin` composes cleanly with the [OpenTelemetry plugin](/develop/python/observability#tracing). Register
