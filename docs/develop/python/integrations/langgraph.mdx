...
- **Has non-deterministic behavior** — anything that can return different results on re-execution (random numbers,
  current time, external data). Workflow code must be deterministic.
...
| Conditional edge functions (`add_conditional_edges`) | Workflow | Always runs in the Workflow. Must be **deterministic code** and **async** (sync functions trigger `run_in_executor`, which is not allowed in the Temporal sandbox). |
