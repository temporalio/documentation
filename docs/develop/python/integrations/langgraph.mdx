...
For LangChain Deep Agents built with `create_deep_agent(...)` rather than as raw LangGraph graphs, see the
[Deep Agents integration](/develop/python/integrations/deepagents), which hooks model and tool calls instead of graph
nodes.
