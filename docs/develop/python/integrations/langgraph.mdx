...
[`lint_langgraph_workflows.py`](https://github.com/temporalio/documentation/blob/main/sample-apps/python/langgraph_workflow_lint/lint_langgraph_workflows.py)
static checker.
...
- **Time/date calls** — `datetime.now()`, `time.time()`, `time.sleep()`
- **Randomness** — `random.randint()`, `uuid.uuid4()`, `secrets.*`
- **Network I/O** — `requests`, `httpx`, `aiohttp`, `urllib`, `http.client`
- **File I/O** — `open()`, `pathlib.Path.read_text()`, `shutil`, `glob`
- **OS/subprocess** — `os.environ`, `subprocess.run()`, `os.system()`
- **Other** — `print()`, `multiprocessing`, `socket`
