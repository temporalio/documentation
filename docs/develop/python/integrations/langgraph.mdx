...
[LangGraph plugin samples](https://github.com/DABH/samples-python/tree/langgraph-samples/langgraph_plugin) (on a PR
branch — see [samples-python#289](https://github.com/temporalio/samples-python/pull/289)). Refer to the samples for the
complete code.
...
Python 3.11 or newer is required for the Functional API (`@entrypoint` / `@task`) and for `interrupt()`. On older
Python versions the plugin loads but emits a warning, and those features will not work because LangGraph relies on
`contextvars` propagation through `asyncio.create_task()`, which is only available from Python 3.11 onward.
...
`execute_in` cannot be set in `default_activity_options` — you must set it on each node or task individually.
...
See the [human-in-the-loop samples](https://github.com/DABH/samples-python/tree/langgraph-samples/langgraph_plugin/graph_api/human_in_the_loop) for
complete working examples using both Graph and Functional APIs.

## Samples

The [LangGraph plugin samples](https://github.com/DABH/samples-python/tree/langgraph-samples/langgraph_plugin)
demonstrate all supported patterns across both APIs (samples are on a PR branch — see
[samples-python#289](https://github.com/temporalio/samples-python/pull/289)):
...
| Continue-as-new | Yes | Yes | Preserve completed task results across continue-as-new with `cache()` |
| LangSmith tracing | Yes | Yes | Tracing with the Temporal LangSmith plugin |
