...
Use `nexus.client()` to get the Client that the Worker was initialized with. In this example, the workflow ID is derived from the client ID, with the method "get_workflow_id" converting a given client ID (in this case, the client is passing in a user ID) to generate a workflow ID from it.
