...
- About validators:
  ...
  - The SDK automatically provides a validator decorator named `@<update-handler-name>.validator`.
    ...
    You can create your own validator with the same constraints by marking a method with the `@update_workflow_status.validator` decorator.
  - For more flexibility, use code-based checks at the start of the Update handler instead of validators.
    Raise an `ApplicationError` for any issues that would prevent the Update from running.
    This approach adds rejected Update requests to your Event History if exceptions occur.
...
A Worker must be online and polling the Task Queue to process a Signal.
...
- Send [`start_update`](https://python.temporal.io/temporalio.client.WorkflowHandle.html#start_update) to receive an [`UpdateHandle`](https://python.temporal.io/temporalio.client.WorkflowUpdateHandle.html) as soon as the update is accepted or rejected.
  ...
  - `start_update` wait until the Worker has accepted or rejected the Update, not until all activities are complete.
...
You can use wait conditions ([`workflow.wait_condition`](https://python.temporal.io/temporalio.workflow.html#wait_condition)) to set a function that prevents the code from proceeding until the condition returns `True`.
...
* Waiting in a handler until it appropriate to continue.
...
### Add handler conditions

When using handlers, it's common to add early Workflow wait conditions to ensure the handler is ready to start.
You can also use wait conditions anywhere else in the handler to wait for a specific status to become `True`.
This flexibility helps you manage wait conditions throughout the handler's execution.
...
Workflow wait conditions let your handlers complete before a Workflow finishes.
...
When using Signal, the only exception that will result from your requests during its excecution is `RPCError`.
...
A dynamic Workflow, Activitiy, Signal, Update, or Query is a kind of unnamed item.
Normally, these items are registered by name with the Worker and invoked at runtime.
When an unregistered or unrecognized Workflow, Activity, or message handler arrives with a recognized method signature, the Worker can use a pre-registered dynamic stand-in.
This helps when a static named version does not exist or when the name for an existing item is unknown.
