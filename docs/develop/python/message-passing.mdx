...
At times, Signal or Update handlers require something in the main Workflow method to be done first before they should proceed.
Wait conditions are high beneficial in these circumstances.
Using  ([`workflow.wait_condition`](https://python.temporal.io/temporalio.workflow.html#wait_condition)) allows you to specify a function that must return True before the handler can proceed.

For example, in an `async def` handler you might require a `ready_for_update_to_execute` method to be successful before starting the Update:
...
You can use wait conditions anywhere in your handlers, not just at the start and for whatever reason you need to block.
The condition you're waiting for can be changed by anything -- another handler, a child coroutine spawned by the main workflow method, and so forth.

Remember: a Signal or Update handler can execute before the main Workflow method has started.

#### Use `asyncio.Lock` to prevent concurrent handler execution {#control-handler-concurrency}

At times, your code won't work properly if multiple handler instances are in progress [concurrently](/encyclopedia/workflow-message-passing#message-handler-concurrency).
...
Coordinating access with `asyncio.Lock` transforms this into a safe and maintainable approach.
It ensures that only one coroutine is executing a particular section of code at a time.
...
Finish handlers address this problem.

To prevent this, use [`workflow.wait_condition`](https://python.temporal.io/temporalio.workflow.html#wait_condition) to ensure [`all_handlers_finished`](https://python.temporal.io/temporalio.workflow.html#all_handlers_finished) returns `True` before proceeding:
...
* **The client can't contact the server**: You'll encounter a [`temporalio.service.RPCError`](https://python.temporal.io/temporalio.service.RPCError.html) on which the `status` attribute is [`RPCStatusCode`](https://python.temporal.io/temporalio.service.RPCStatusCode.html) `UNAVAILABLE` (after some retries; see the `retry_config` argument to [`Client.connect`](https://python.temporal.io/temporalio.client.Client.html#connect)).

* **The workflow does not exist**: You'll get an [`temporalio.service.RPCError`](https://python.temporal.io/temporalio.service.RPCError.html) on which the `status` attribute is [`RPCStatusCode`](https://python.temporal.io/temporalio.service.RPCStatusCode.html) `NOT_FOUND`.
...
* **No Workflow Workers are polling the Task Queue**: Your request will be retried by the SDK Client indefinitely.
    ...
    This raises [`temporalio.client.WorkflowUpdateRPCTimeoutOrCancelledError`](https://python.temporal.io/temporalio.client.WorkflowUpdateRPCTimeoutOrCancelledError.html).

* **Update failed**: You'll encounter a [`temporalio.client.WorkflowUpdateFailedError`](https://python.temporal.io/temporalio.client.WorkflowUpdateFailedError.html). There are two ways this can happen:
...
* **The handler caused the Workflow Task to fail**:
  A [Workflow Task Failure](/references/failures#errors-in-workflows) causes the server to retry Workflow Tasks indefinitely. What happens to your Update request depends on its stage:
    - If the request hasn't been accepted by the server, you receive a `FAILED_PRECONDITION` [`temporalio.service.RPCError`](https://python.temporal.io/temporalio.service.RPCError.html).
...
<div style={{backgroundColor: '#ffff00',padding: '0px 15px',borderRadius: '5px',border: '1px solid #cccccc',display: 'inline-block'}}>**PICK UP HERE**</div>

*. **Workflow finished while the Update handler execution was in progress**
