...
At times, [concurrent message-passing](/encyclopedia/workflow-message-passing#message-handler-concurrency) code won't work properly if multiple handler instances are in progress.
...
*. **The Workflow finished while the Update handler execution was in progress**:
    You'll get a [`temporalio.service.RPCError`](https://python.temporal.io/temporalio.service.RPCError.html) exception with a "workflow execution already completed" message.
wefwef
