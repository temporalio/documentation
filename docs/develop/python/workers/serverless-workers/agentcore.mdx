...
The `async_task` decorator reports the Runtime as busy while the handler is running, so AgentCore cannot tell when a
Worker that is still polling has no Temporal work. Without an application idle policy, the Runtime can remain active
until its maximum lifetime of eight hours. To release capacity sooner, have the Runtime handler detect when the Worker
has no useful work.
