...
`ActivityTracker` is custom code from the AgentCore sample. It uses an
[Activity inbound Interceptor](/develop/python/workers/interceptors) to count Activity work and decide when the Worker
can stop.
