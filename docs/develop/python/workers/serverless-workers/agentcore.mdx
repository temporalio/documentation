...
`ActivityTracker` is one example. It is custom code from the AgentCore sample. It uses an
[Activity inbound Interceptor](/develop/python/workers/interceptors) to count running Activities. This is appropriate
when Activities represent work that should keep the Worker available, such as the model and tool calls in the AgentCore
sample.
