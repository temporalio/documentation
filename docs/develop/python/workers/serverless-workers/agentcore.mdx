...
sample runs model and tool calls as Activities. It is not a universal definition of idleness.

If your workload does not have a useful Activity-based signal, use a fixed Worker lease instead. Start a lease when
AgentCore invokes the Runtime. When it expires, stop polling and drain the Worker:
...
A shorter lease releases capacity sooner but causes more Worker starts. A longer lease keeps capacity available longer
but can leave it idle. After the Worker drains, the next Task that needs capacity causes the WCI to invoke another
Runtime session.
