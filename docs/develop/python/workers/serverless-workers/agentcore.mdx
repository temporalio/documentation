...
Activity running. A long-running Activity keeps the count above zero, so the Worker continues polling. After that
Activity completes, the tracker waits another 60 seconds before it retires the Worker. The two-minute
`graceful_shutdown_timeout` is a safety limit for any Activity still in flight when shutdown starts.
