...
Normally, your Workflow constructor method won't have any parameters.
However, if you use the `[WorkflowInit]` attribute on your constructor, you can give it the same Workflow parameters as your `[WorkflowRun]` method.
...
Here's an example.
Notice that the constructor and `RunAsync` must have the same parameters with the same types:
