...
- `ITemporalClient.GetWorkflowHandle`
...
Using asynchronous calls allows you to `await` Activities, Child Workflows, `Workflow.DelayAsync` Timers, `Workflow.WaitConditionAsync` wait conditions, etc.
...
The `Workflow.WaitConditionAsync` call waits until your condition is met:
...
Coordinating access with `Workflows.Mutex`, a mutual exclusion lock, corrects this code.
...
Consider a Signal, where you might send something like  `handle.SignalAsync(wf => wf.MySignalMethodAsync("my_arg"))`.
This is a type-safe compiler-checked approach that guarantees a method exists.
There's also a non-type-safe string-based form: `handle.SignalAsync("some-name", new object?[] { "my_arg" })`.
...
For example, you make a Signal dynamic by adding `Dynamic = true` to the `[WorkflowSignal]` attribute.

The handler must accept a string input for the name, and a `Temporalio.Converters.IRawValue[]` for the arguments.
Use a [Workflow.PayloadConverter](https://dotnet.temporal.io/api/Temporalio.Workflows.Workflow.html#Temporalio_Workflows_Workflow_PayloadConverter)  to convert `IRawValue` objects to your required type.
For example:
...
This sample creates a dynamic Signal.
...
When registered, any Signals sent to the Workflow without a defined handler will be delivered to the dynamic `WorkflowSignal`.
Note that you can only register one each of a dynamic Signal, Query, or Update per Workflow Execution.
Dynamic handlers can be implemented in both regular and dynamic Workflow implementations.


<div style={{backgroundColor: '#ffff00',padding: '0px 15px',borderRadius: '5px',border: '1px solid #cccccc',display: 'inline-block'}}>**.NET source did not include dynamic Workflow coverage.**</div>
...
<div style={{backgroundColor: '#ffff00',padding: '0px 15px',borderRadius: '5px',border: '1px solid #cccccc',display: 'inline-block'}}>**Needs .NET.**</div>
...
<div style={{backgroundColor: '#ffff00',padding: '0px 15px',borderRadius: '5px',border: '1px solid #cccccc',display: 'inline-block'}}>**Needs .NET and an appropriate example with a payload converter.**</div>
...
<div style={{backgroundColor: '#ffff00',padding: '0px 15px',borderRadius: '5px',border: '1px solid #cccccc',display: 'inline-block'}}>**.NET source did not include dynamic Activity coverage.**</div>
...
<div style={{backgroundColor: '#ffff00',padding: '0px 15px',borderRadius: '5px',border: '1px solid #cccccc',display: 'inline-block'}}>**.NET material required here**</div>
