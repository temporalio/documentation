...
Signals can be used to change the state and control the flow of a Workflow Execution. 
...
1. **[Defining the Signal](#define-a-signal)** - You specify the name and data structure used by Temporal Clients when sending the Signal.
2. **[Handling the Signal](#handle-a-signal)** - You write code that will be invoked when the Signal is received from a Temporal Client.
...
### Handle Signal {#handle-a-signal}

**How to handle a Signal using the .NET SDK.**
