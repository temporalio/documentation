...
Use the `ConnectAsync()` static method on the `Temporalio.Client.TemporalClient` class to create and connect to a Temporal Client to the Temporal Cluster.
...
Use the `ConnectAsync()` static method on the `Temporalio.Client.TemporalClient` class to create and connect to a Temporal Client to the Temporal Cluster.
Specify the `Tls` property of the connection options to connect to a Temporal Cluster with TLS enabled.
