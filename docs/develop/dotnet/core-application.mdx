...
Use [`Workflow.Unsafe.IsReplaying`](https://dotnet.temporal.io/api/Temporalio.Workflows.Workflow.Unsafe.html) to guard code that should only run on the first execution, such as emitting metrics or sending external notifications.
