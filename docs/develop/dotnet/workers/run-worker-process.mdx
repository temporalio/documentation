...
`ExecuteAsync()` runs until the cancellation token is cancelled, so pass a token you control rather than `CancellationToken.None` in a real process.
...
Cancel the token passed to `ExecuteAsync()` to shut the Worker down.
It stops polling for new Tasks and waits for in-flight Tasks to finish, up to `GracefulShutdownTimeout`.
