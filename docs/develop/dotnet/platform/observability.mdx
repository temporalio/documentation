...
The observability feature guide covers the many ways to view the current state of your [Temporal Application](/encyclopedia/architecture/temporal-architecture#client-application).
This includes the ways to view which [Workflow Executions](/workflow-execution) are tracked by the [Temporal Platform](/encyclopedia/architecture/temporal-architecture#the-temporal-server) and the state of any specified Workflow Execution, either currently or at points of an execution.
