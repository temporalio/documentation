...
Use `NexusOperationExecutionContext.Current.TemporalClient` to get the Client that the Worker was initialized with:

```csharp
[NexusOperationHandler]
public IOperationHandler<SignalInput, SignalOutput> SignalWorkflow() =>
    OperationHandler.Sync<SignalInput, SignalOutput>(
        async (ctx, input) =>
        {
            var client = NexusOperationExecutionContext.Current.TemporalClient;
            var handle = client.GetWorkflowHandle(input.WorkflowId);
            // Signal a Workflow
            await handle.SignalAsync("MySignal", new[] { input.Value });
            // Query a Workflow
            var result = await handle.QueryAsync<string>("MyQuery");
            return new(result);
        });
```
