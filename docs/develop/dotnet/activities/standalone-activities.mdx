...
Omit `runId` to target the latest run for that Activity Id:

```csharp
// Without a known result type
var handle = client.GetActivityHandle("my-activity-id");

// With a known result type
var typedHandle = client.GetActivityHandle<string>("my-activity-id");
```
...
Use a typed handle (or `GetResultAsync<T>()`) so the result type is known:

```csharp
var handle = client.GetActivityHandle<string>("my-activity-id");
var result = await handle.GetResultAsync();
```
