...
Every Operation is written with a [Temporal Operation Handler](/nexus/temporal-operation-handler). Mark a method
`[TemporalOperation]`; that method is the Operation handler and receives three things: a
...
Nexus Service handlers also support dependency injection through the generic-host Worker. See [NexusDependencyInjection sample](https://github.com/temporalio/samples-dotnet/tree/main/src/NexusDependencyInjection).
