...
To run the examples on this page against the [.NET sample](https://github.com/temporalio/samples-dotnet/tree/main/src/NexusStandaloneOperations),
create the Nexus Endpoint and start the sample Worker (`dotnet run worker`):
...
```csharp
var nexusClient = client.CreateNexusClient<IHelloService>("nexus-standalone-operations-endpoint");
...
```csharp
var nexusClient = client.CreateNexusClient("nexus-standalone-operations-endpoint", "HelloService");
