...
mkdir temporaliodotnethelloworld
cd temporaliodotnethelloworld

dotnet new sln -n temporaliodotnethelloworld
...
dotnet sln temporaliodotnethelloworld.sln add Workflow/Workflow.csproj Worker/Worker.csproj Client/Client.csproj
...
dotnet add Client/Client.csproj package Temporalio`}
    ...
    You'll add the Temporal .NET SDK package and project references in the next section.
...
# Build the solution (restores packages automatically)
dotnet build
```
...
└── temporaliodotnethelloworld.sln
```
...
A Worker polls a Task Queue, that you configure it to poll, looking for work to do. Once the Worker dequeues the a Workflow or Activity task from the Task Queue, it then executes that task.
