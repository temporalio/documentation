...
The exception is `getApprovalStatus`, which is [not implementable in the pre-release](/nexus/temporal-operation-handler) because Query is not yet available as sync messaging.
