...
`getApprovalStatus` belongs in the contract, but Query is not available as sync messaging on the Nexus-aware Client in the pre-release, so this Operation cannot be implemented yet. See [Temporal Operation Handler](/nexus/temporal-operation-handler).
...
- [Temporal Operation Handler](/nexus/temporal-operation-handler) for the sync side effect and async backing distinction.
