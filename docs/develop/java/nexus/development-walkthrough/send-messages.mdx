...
Look at the contract from [step 1](/develop/java/nexus/development-walkthrough/define-the-data-contract#plan-the-operations) and `attachApprovalContext` carries more than it seems to need: the item id, the requester, the amount, *and* the note. `remindApprover` gets by with just an approval id.
