...
Update-with-Start — starting a Workflow and running an Update against it atomically — is not available in any SDK yet. See the [per-SDK support table](/nexus/sdk-v2#per-sdk-support).
...
`requestApproval` delivers the decision to whoever awaited it. There is no Operation that attaches to an already-running approval and waits for its result, because Get Workflow Result as an async backing is [not yet available](/nexus/sdk-v2#per-sdk-support) in any SDK.
