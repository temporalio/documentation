...
You may see older examples using `WorkflowRunOperation.fromWorkflowMethod` or a synchronous `OperationHandler`. Both still work and are not being removed, but they are de-emphasized in favor of `TemporalOperationHandler`. See [Coming from the earlier handler APIs](/nexus/temporal-operation-handler#coming-from-the-earlier-handler-apis).
