...
create the Nexus Endpoint and start the sample handler Worker (`./gradlew -q :core:execute -PmainClass=io.temporal.samples.nexusstandalone.handler.HandlerWorker`):
...
./temporal nexus operation execute \
  --endpoint nexus-standalone-operation-endpoint \
...
```bash
./temporal nexus operation result --operation-id my-greet-op
```
...
```bash
./temporal nexus operation list --query 'Endpoint = "nexus-standalone-operation-endpoint"'
```
...
```bash
./temporal nexus operation count --query 'Endpoint = "nexus-standalone-operation-endpoint"'
```
