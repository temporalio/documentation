...
<div style={{backgroundColor: '#ffff00',padding: '0px 15px',borderRadius: '5px',border: '1px solid #cccccc',display: 'inline-block'}}>**EXTERNAL WORKFLOW SENT FOR DOUBLE-CHECK TO JAVA EXPERT. QUINN IS ON VACATIONN.**</div>

A Workflow can send a Signal to another Workflow, known as an _External Signal_.
Establish an `ExternalWorkflowStub` instance in your _current_ Workflow with [`Workflow.newExternalWorkflowStub`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/workflow/Workflow.html#newExternalWorkflowStub(java.lang.Class,io.temporal.api.common.v1.WorkflowExecution)), passing a running Workflow Id.
...
<div style={{backgroundColor: '#ffff00',padding: '0px 15px',borderRadius: '5px',border: '1px solid #cccccc',display: 'inline-block'}}>**CORRECT? QUINN IS ON VACATIONN.**</div>

The Client executees the `setCustomer` Signal handler and blocks before calling the `@WorkflowMethod` `greet`.

Here is the Workflow interface for the `GreetingWorkflow` called in the previous example.
The `setCustomer` method is annotated with`@SignalMethod`:
...
<div style={{backgroundColor: '#ffff00',padding: '0px 15px',borderRadius: '5px',border: '1px solid #cccccc',display: 'inline-block'}}>**MISSING FROM PYTHON? Dan.**</div>

If a Workflow Execution with the given Workflow Id is already running, the Client sends the Signal.
If not, the Client starts a new Workflow and sends the Signal immediately after.

<div style={{backgroundColor: '#ffff00',padding: '0px 15px',borderRadius: '5px',border: '1px solid #cccccc',display: 'inline-block'}}>**PAUSE POINT FOR WEEKEND. WRAPPING UP OTHER MATTERS.**</div>
...
This involves passing method names instead of methods to [`WorkflowClient.newUntypedWorkflowStub`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowClient.html#newUntypedWorkflowStub(java.lang.String,io.temporal.client.WorkflowOptions)) / [`WorkflowStub.query`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowStub.html#query(java.lang.String,java.lang.Class,java.lang.Object...)) / [`WorkflowStub.signal`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowStub.html#signal(java.lang.String,java.lang.Object...)) / [`WorkflowStub.update`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowStub.html#update(java.lang.String,java.lang.Class,java.lang.Object...)) / [`WorkflowStub.startUpdate`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowStub.html#startUpdate(io.temporal.client.UpdateOptions,java.lang.Object...)), and using the non type-safe APIs [`newUntypedWorkflowStub​`](https://www.javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowClient.html#newUntypedWorkflowStub(java.lang.String,io.temporal.client.WorkflowOptions)) and [`Workflow.newUntypedExternalWorkflowStub`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/workflow/Workflow.html#newUntypedExternalWorkflowStub(java.lang.String)).
...
You can avoid this by using [`Workflow.await`](https://www.javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/workflow/Workflow.html#await(java.time.Duration,java.util.function.Supplier)) to wait for `Workflow.isEveryHandlerFinished` (URL TODO) to return `true`:
```java
...
See [Exceptions in message handlers](/encyclopedia/workflow-message-passing#exceptions) for a non–Java-specific introduction to this topic.
...
1. **The client can't contact the server**
    ...
    You'll get an [`WorkflowServiceException`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowServiceException.html) on which the `cause` is a [`StatusRuntimeException`](https://grpc.github.io/grpc-java/javadoc/io/grpc/StatusRuntimeException.html) with `status` `UNAVAILABLE` (after some retries).

1. **The workflow does not exist**

    You'll get an [`WorkflowNotFoundException`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowNotFoundException.html).

### Problems when sending a Signal {#signal-problems}

For Signal, these `WorkflowException`s are the only types of exception that will result from the request.
...
1. **Update failed.**
    ...
    You'll get [`WorkflowUpdateException`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowUpdateException.html). There are two ways this can happen:

    1. The Update was [rejected](#handle-update) by an Update validator (defined in the Workflow alongside the Update handler).
    ...
    1. The Update failed after having been accepted.
        ...
        - The Workflow author threw an `ApplicationFailure`
    ...
    1. **The handler caused the Workflow Task to fail.**
        ...
        If it had not yet been accepted by the server then you'll get a `FAILED_PRECONDITION` [`WorkflowServiceException`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowServiceException.html).
        On the other hand, if it had been [accepted](#send-update) by the server, then it is durable, and as soon as the Workflow becomes healthy again (perhaps as a result of a code deploy) then you can use an [`UpdateHandle`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/UpdateHandle.html) to fetch the update result.


    1. **Workflow finished while the Update handler execution was in progress**

        You'll get a [`WorkflowServiceException`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowServiceException.html) "workflow execution already completed"`
...
1. **There is no Workflow Worker polling the Task Queue**
    ...
    You'll get an [`WorkflowServiceException`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowServiceException.html) on which the `cause` is a [`StatusRuntimeException`](https://grpc.github.io/grpc-java/javadoc/io/grpc/StatusRuntimeException.html) with `status` `FAILED_PRECONDITION`.

1. **Query failed.**

    You'll get [`WorkflowQueryException`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowQueryException.html).
    Any exception in a Query handler will cause this.
    Note that this differs from Signal and Update, for which exceptions can cause Workflow Task Failure.
