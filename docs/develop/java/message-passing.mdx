...
<div style={{backgroundColor: '#ffff00',padding: '0px 15px',borderRadius: '5px',border: '1px solid #cccccc',display: 'inline-block'}}>**I can't seem to find an equivalent for this first one. Taking a guess.'**</div>
- [`Client.newUntypedWorkflowStub`](https://javadoc.io/static/io.temporal/temporal-sdk/1.25.1/io/temporal/client/WorkflowClient.html#newUntypedWorkflowStub(java.lang.String))
...
Although an asynch Signal handler can initiate similar network tasks, using an Update handler allows the client to receive a result or error once the Activity completes.
...
You can use a wait condition with [`Workflow.await`](https://www.javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/workflow/Workflow.html#await(java.time.Duration,java.util.function.Supplier)) to set a function that prevents the code from proceeding until the condition returns `true`.
...
Coordinating access using `WorkflowLock` corrects this code.
...
- **Update failed**: You'll receive a `WorkflowUpdateException` exception.
  There are two ways this can happen:
    ...
    - The Update was [rejected](#handle-update) by an Update validator defined in the Workflow alongside the Update handler.
...
- **There is no Workflow Worker polling the Task Queue**:
  You'll receive a [`WorkflowServiceException`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowServiceException.html) on which the `cause` is a [`StatusRuntimeException`](https://grpc.github.io/grpc-java/javadoc/io/grpc/StatusRuntimeException.html) with `status` of  `FAILED_PRECONDITION`.
...
Add `dynamic=True` to the handler annotation (for example, `@workflow.signal(dynamic=True)`) to make a handler dynamic.
...
Adding `implements DynamicWorkflow` to the main Workflow method makes the Workflow Definition eligible to participate in dynamic invocation.
...
1. **The client can't contact the server**
    ...
    You'll get an [`WorkflowServiceException`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowServiceException.html) on which the `cause` is a [`StatusRuntimeException`](https://grpc.github.io/grpc-java/javadoc/io/grpc/StatusRuntimeException.html) with `status` of `UNAVAILABLE` (after some retries).
...
1. **There is no Workflow Worker polling the Task Queue**
    ...
    You'll get an [`WorkflowServiceException`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/client/WorkflowServiceException.html) on which the `cause` is a [`StatusRuntimeException`](https://grpc.github.io/grpc-java/javadoc/io/grpc/StatusRuntimeException.html) with `status` of `FAILED_PRECONDITION`.
