...
Import the `io.temporal.envconfig` package to set connection options for the Temporal Client using environment variables.
The `ClientConfigProfile.load` method will automatically load all environment variables. For a list of all available
...
within another Workflow Execution, use either the [Child Workflow](/develop/java/child-workflows) or External Workflow
APIs.

See the [Customize Workflow Type](/develop/java/core-application#workflow-type) section to see how to customize the name
...
See [`SignalwithStart`](/develop/java/message-passing#signal-with-start) to start a Workflow Execution to receive a
...
[`setCronSchedule`](/develop/java/schedules#cron-schedule) Workflow option in the Client code.
