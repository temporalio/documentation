...
The code will just mock the implementation of the NexusService class with the handler methods, but will need those methods stubbed in for the testing framework.
...
public class NexusServiceMockTest {
    // Set up mocks for the service and return values.
    ...
    public TestWorkflowRule testWorkflowRule =
      TestWorkflowRule.newBuilder()
          // Set the Nexus service to the mock service
          .setNexusServiceImplementation(mockNexusService)
          // The caller types still need to be registered
          .setWorkflowTypes(HelloCallerWorkflowImpl.class)
          .build();
    ...
    public void testHelloWorkflow() {
        ...
        helloResult = new NexusService.HelloOutput("Hello World!");
        testWorkflowRule.getTestEnvironment().start();

        // Calling the workflow does not change from the prior example
        HelloCallerWorkflow workflow =
            testWorkflowRule
                .getWorkflowClient()
                .newWorkflowStub(
...
public class NexusServiceMockTest {
    ...
    //A method to set up the mocks correctly for the Nexus service and the handlers
    private NexusServiceImpl createMockNexusService() {
        NexusServiceImpl mock = mock(NexusServiceImpl.class);
        when(mock.hello()).thenReturn(OperationHandler.sync((ctx, details, input) -> helloResult));
        return mock;
    ...
    public void testHelloWorkflow(
      TestWorkflowEnvironment testEnv, Worker worker, HelloCallerWorkflow workflow) {
      // Set the mock value to return the desired result
      helloResult = new NexusService.HelloOutput("Hello World!");

      // Execute the workflow which will run the mocked service, this is exactly the same as the prior JUnit5 example
      String greeting = workflow.hello("World", NexusService.Language.EN);
      assertEquals("Hello World!", greeting);
    }
}
