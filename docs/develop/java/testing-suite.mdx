...
For example, in the time-skipping mode, Timers, which include sleeps and conditional timeouts, are fast-forwarded except when Activities or Nexus Operations are running.
...
#### Skip time in Nexus Operations {#skip-time-in-nexus-operations}

Learn to skip time in Nexus Operations in the SDK of your choice.
