...
- Nexus Service beans present in Spring context whose implementations are annotated with `io.temporal.spring.boot.SampleNexusServiceImpl`
...
:::note
`io.temporal.spring.boot.ActivityImpl` and `io.temporal.spring.boot.SampleNexusServiceImpl` should be applied to beans, one way to do this is to annotate your Activity implementation class with `@Component`
:::
