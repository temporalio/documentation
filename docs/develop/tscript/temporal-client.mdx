...
description: Master the Temporal Typescript Client with our comprehensive guide that covers everything from initialization to Workflow Execution.
keywords:
...
The Temporal Client is a key part of a Temporal SDK that allows you to communicate with the Temporal Service.

You create Temporal Workflows by writing code in your programming language of choice, such as using the Typescript SDK.
This code interacts with Temporal's APIs through the Temporal Client.

The Temporal Client is crucial to starting, executing, monitoring, and managing Workflows ensuring they are durable and recover from failures seamlessly.

Particularly for long running and complex Workflows, it acts as the bridge for communication between your applications and the Temporal Service.
