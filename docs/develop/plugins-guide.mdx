...
- Temporal's TypeScript SDK ships with an [OpenTelemetry Plugin](https://github.com/temporalio/sdk-typescript/blob/main/packages/interceptors-opentelemetry/src/plugin.ts)
...
The TypeScript SDK bundles all Workflow code from a single module. This prevents TypeScript Plugins from providing
built-in Workflows the way other SDKs can. Plugin users must import and re-export any Plugin-provided Workflows from
their own Workflow module so the bundle includes them.

Workflow interceptors are also bundled, but they aren't limited to a single module. `SimplePlugin` appends any
`workflowModules` paths from `workerInterceptors` to the bundle's `workflowInterceptorModules` list, so Plugins can
offer Workflow interceptors directly. Pass plugins to `Worker.create()` or `bundleWorkflowCode()` to include them:
