...
For a Plugin to offer Workflow interceptors, the Workflow bundle must include the module containing the interceptors.

The `SimplePlugin` handles this automatically by appending any `workflowModules` paths from `workerInterceptors` to the
bundle's `workflowInterceptorModules` list.

Passing plugins to `Worker.create()` or `bundleWorkflowCode()` will include the Workflow interceptor module alongside
your Workflow code in the bundle:
