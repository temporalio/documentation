...
Though, unlike [Activities](/docs/go-activities), Workflow timeouts are available primarily to protect the system from "runaway" Workflows that may end up consuming too many resources, and not intended to be used as a part of the business logic.
...
The only required Workflow options parameter is the name of a [Task Queue](/docs/go-task-queues).
Read the [Task Queues concept page](/docs/go-task-queues) for a better overview.
...
If you are using [Signals](/docs/go-signals) with the Go SDK, you should make sure to do an asynchronous drain on the Signal channel or the Signals will be lost.
