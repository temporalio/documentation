...
A Retry Policy is a collection of attributes that instructs the Temporal Server how to retry a failure of a [Workflow Execution](/next/workflows#workflow-executions) or an [Activity Task Execution](/next/tasks#activity-task-execution).
(Retry Policies do not apply to [Workflow Task Executions](/next/tasks#workflow-task-execution), which always retry indefinitely.)
...
- **Activity Execution**: When an Activity Execution is spawned, it is associated with a default Retry Policy, and thus Activity Task Executions are retried by default.
  When an Activity Task Execution is retried, the Cluster places a new [Activity Task](/next/tasks#activity-task) into its respective [Activity Task Queue](/next/tasks#task-queues), which results in a new Activity Task Execution.
...
- A [Temporal Cron Job](/next/workflows#cron-jobs) or some other stateless, always-running Workflow Execution that can benefit from retries.
...
- For a Workflow Execution with a Retry Policy, if the Workflow Execution fails, the Workflow Execution will [Continue-As-New](/next/workflows#continue-as-new) and the associated Event is written to the Event History.
