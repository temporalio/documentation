...
- **Thrift/TChannel** → **[Protobuf](https://developers.google.com/protocol-buffers)/[gRPC](https://grpc.io/)** ⚡ :
  ...
  - All communication over the wire can be encrypted via [TLS](/references/configuration/#tls). Note: Support for TLS in `tctl` and `temporal-web` is still in-progress.
