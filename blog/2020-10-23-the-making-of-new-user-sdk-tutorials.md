...
The first challenge is that the information we inherited lagged behind in fully describing Temporal's capability and feature set. One of the reasons for this is that documentation is typically offered a secondary level of prioritization; Faced with limited resources, as long as the product ships with some minimal viable information and access to product experts who can answer questions, then at least most advanced users can get it going. While Temporal now prioritizes documentation, this was not always true from where Temporal originated as Cadence.

The second challenge was that in the time since Temporal forked from Cadence there have been many core changes to the system, terminology, and SDKs. Back in early September, many of these changes had yet to be propagated throughout the docs as well. So, not only was there missing information but some of the existing information was just plain incorrect.
...
We decided that the best thing for us to focus on was a great first experience for new users. We wanted something that would enable a developer to start using the product right away but also instill an understanding of the value it provides. We wanted to cultivate that "aha!" moment.
...
2. In the same way that the code must be optimized for the docs, the docs must also be optimized for the code. In essence, the docs are being “driven” and "defined" by the underlying code. And if no one has coined the term yet, I think the credit for “code-driven documentation” should go to our Head of Product, Ryland Goldstein as he pinged me early one morning to share that epiphany with me.

We decided to embrace [Snipsync](https://github.com/temporalio/snipsync) and solve the challenges that it introduced.
...
We actually already repositories of code samples for the [Go SDK](https://github.com/temporalio/samples-go) and [Java SDK](https://github.com/temporalio/samples-java). And while we desired to see more samples, there were already quite a few of them in each. But, we discovered that shared sample repositories tended to have two issues that made them less ideal for synchronizing with docs.
...
Once it became clear that a standard approach wasn't going to cut it, we approached our co-founder and CEO, Maxim Fateev. We asked him to give us a demonstration that he typically uses to introduce engineers to Temporal for the first time. The scenario represents a money transfer from one bank account to another and he demonstrates what happens if one of the steps in the transfer fails. The money transfer sample was a great way to introduce the values of Temporal. For if you understand the ramifications of losing money from a failed financial transaction, several values of Temporal become immediately apparent:
...
For someone that is new to Temporal the attraction doesn’t come from using the SDK to print “Hello World!”, but instead comes from witnessing the inherent benefits that Temporal offers by simply running simulations using a pre-built application.
