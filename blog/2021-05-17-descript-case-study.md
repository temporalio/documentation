...
- **Session Routing for Stateful Execution**: You can finetune the execution of sequential activities on the same worker, using [Go SDK Sessions](https://docs.temporal.io/docs/go/how-to-use-worker-sessions-in-go), which Descript will need (but doesn't yet use).
