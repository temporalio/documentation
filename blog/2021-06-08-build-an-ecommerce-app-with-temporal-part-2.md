...
For example, since [Temporal automatically retries failed activities](/docs/content/what-is-a-retry-policy), it would automatically retry the `SendAbandonedCart()` Activity for up to 5 times if `SendAbandonedCart()` returns an error.
