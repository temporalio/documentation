...
- **Don't write a literal `rect rgb(...) ... end` color** for a sequence-diagram highlight block. It renders as a plain `<rect class="rect" fill="rgb(...)">` with no `style` attribute, so `src/css/custom.css` already overrides it globally (search `.rect`) to a theme-aware color — no per-diagram color needed, and message text drawn over it no longer needs a contrast compromise.
