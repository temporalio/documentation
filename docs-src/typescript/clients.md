---
id: clients
title: Clients in TypeScript
sidebar_label: Clients
description: Clients are embedded in your application code, and connect to Temporal Server via gRPC. They are the only way to schedule new Workflow Executions with Temporal Server.
tags:
  - client
  - typescript
---
...
**Clients are embedded in your application code and connect to Temporal Server via gRPC**.
They are the only way to schedule new Workflow Executions with Temporal Server.

- Clients can run in any Node.js application, for example, in a serverless function, Express.js API route handler or CLI/script run.
- The primary use of Clients is to start new Workflow Executions (including [Cron Workflows](#scheduling-cron-workflows)).
  Given a `workflowId`, a Client can also get a Handle to a running Workflow Execution or retrieve/wait for its result.
...
- Advanced users can also use the [`WorkflowService`](https://typescript.temporal.io/api/classes/client.Client#workflowservice) exposed by a `Client` to make **raw gRPC calls**, for example to use features that have no equivalent, high-level APIs.

Clients are separate from Workers, but communicate with them through Task Queues to start Workflow Executions.
...
The following code is a `Client` example, from our Hello World sample:
...
Create a [`Client`](https://typescript.temporal.io/api/classes/client.Client) with the requisite gRPC [`Connection`](https://typescript.temporal.io/api/classes/client.Connection):
...
If you omit the connection and just call `new Client()`, it creates a default connection that will work locally. Just remember you will need to configure your Connection and Namespace when [deploying to production](/typescript/security#encryption-in-transit-with-mtls).

## Start a Workflow Execution

When you have a Client, you can schedule the start of a Workflow with `client.workflow.start`, specifying `workflowId`, `taskQueue`, and `args` and returning a Workflow Handle (see below) immediately after the Server acknowledges receipt.

```ts
// // STEP ONE: client.workflow.start
...
const result = await client.workflow.execute(example /*...*/); // start and immediately wait for Workflow to complete and get result
```
...
Calling `client.workflow.start` (or `client.workflow.execute`) merely sends a Command to Temporal Server to schedule a new Workflow Execution on the specified Task Queue; it does not actually start until a Worker (that has a matching Workflow Type) polling that Task Queue picks it up.

You can test this by executing a Client command without a matching Worker.
...
This queuing mechanic makes your application tolerant to outages and horizontally scalable, but can be confusing to newcomers if they expect that calling `client.workflow.execute(YourWorkflow)` directly executes the Workflow code on the same machine as the Client.
...
**Workflow Handles** are returned after you start a Workflow (or retrieve an existing one with `client.workflow.getHandle`) and are bound to a single Workflow instance. They represent already-started Workflow Executions, and let you `signal`, `query`, `describe`, `cancel`, or `terminate` their instance:
...
const result = await client.workflow.execute(example /*...*/); // Alternative API for starting and immediately waiting for Workflow completion
```
...
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `client`              | Readonly accessor to the underlying Client.                                                                                               |
...
Using a Workflow Handle isn't necessary with `client.workflow.execute` by definition.
