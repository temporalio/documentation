...
The following table lists commonly used dynamic configuration keys that can be used to control the numner of requests to the Temporal Server.
...
###### QPS limits to Persistence database
...
You can change some default values for Activity and Workflow [Retry Policies](/concepts/what-is-a-retry-policy) at the Cluster level.
