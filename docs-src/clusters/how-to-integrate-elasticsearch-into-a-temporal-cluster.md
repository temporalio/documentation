...
1. Add the `advancedVisibilityStore: es-visibility` key-value pair to the `persistence` section.
   [There are several development_es.yaml files](https://github.com/temporalio/temporal/tree/master/config) in the `temporalio/temporal` repo you can look at for example usage.
