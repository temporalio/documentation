...
You can install [tctl](/tctl-v2) in the following ways.
...
- Run locally together with Temporal Server in [docker-compose](https://github.com/temporalio/docker-compose): `docker exec temporal-admin-tools tctl YOUR COMMANDS HERE`
  - To invoke [tctl](/tctl-v2) as though it is installed locally (such as `tctl namespace describe`), set an alias: `alias tctl="docker exec temporal-admin-tools tctl"`
...
**Note:** To use [tctl](/tctl-v2), you must have a Temporal Server running.

To see help for [tctl](/tctl-v2) commands, enter the following commands.
