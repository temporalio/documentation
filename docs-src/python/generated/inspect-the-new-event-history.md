...
# To get this test to pass, we must get an updated Event History JSON file.<<<<<<< HEAD[Start a new Workflow](/python/backgroundcheck-boilerplate-start-workflow) and after it is complete [download the Event History as a JSON object](/python/chapter-durable-execution/retrieve-event-history).
