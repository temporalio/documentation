...
1. *Admission* - Upon submission to an instance of the Temporal WorkflowService, an update is validated by the server against server-configured limits to resource utilization (e.g. concurrent request limits, RPS limits, etc).
...
1. *Validation* - Update requests undergo further validation through an optional request validation phase that can be associated with an update handler at registration time.
...
1. *Execution* - Accepted Update requests proceed to the execution phase.
...
1. *Completion* - The update handler can return a result value or some language-appropriate error/exception to indicate that it has completed.
