...
So, for example, a Workflow Execution that returns a result of size 2.1MiB (which is above 2MiB - 1KiB limit) will be allowed to compete successfully, but that value will not be available as a last completion result.
